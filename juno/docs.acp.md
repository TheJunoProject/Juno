...
This document describes how the Juno ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`juno acp` exposes an ACP agent over stdio and forwards prompts to a running
Juno Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Juno Gateway
...
- ACP client spawns `juno acp` and speaks ACP messages over stdio.
...
`juno acp` resolves the Gateway URL and auth from CLI flags or config:
