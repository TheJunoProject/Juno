...
```bash
export JUNO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.juno/juno.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env JUNO_PARALLELS_DISCORD_TOKEN \
...
- Use the `juno` wrapper for guest `message send/read`; `node juno.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/juno-parallels-smoke.*`
...
- host inbound nonce appears in `juno message read` output
