---
name: juno-release-maintainer
description: Maintainer workflow for Juno releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Juno Release Maintainer
...
- Core `juno` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Juno/Resources/Info.plist`
...
- Every Juno release ships the npm package and macOS app together.
- The production Sparkle feed lives at `https://raw.githubusercontent.com/juno/juno/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `juno` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `juno YYYY.M.D-beta.N`
...
```bash
  JUNO_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/juno-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `JUNO_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `JUNO_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Juno npm release path before creating the tag.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `JUNO_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  - `90m` for `pnpm test:docker:all`
  - Parallels caps from the `juno-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `juno update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `juno/juno` and the real mac preflight in
  `juno/releases-private` for every release.
...
- Juno publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` is an explicit mode on
  `.github/workflows/juno-npm-release.yml`, but it still needs a valid
...
- `.github/workflows/macos-release.yml` in `juno/juno` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `juno/releases-private/.github/workflows/juno-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `juno/releases-private/.github/workflows/juno-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@juno/juno-release-managers` before publish continues.
- Mac publish uses
  `juno/releases-private/.github/workflows/juno-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `juno/juno`
  automatically when `JUNO_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Juno package
...
- `@juno/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/juno-npm-release.yml` with `preflight_only=true`
...
10. Start `.github/workflows/macos-release.yml` in `juno/juno` and wait
...
14. Start `.github/workflows/juno-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@juno/juno-release-managers`.
16. If the stable release was published to `beta`, start
    `.github/workflows/juno-npm-release.yml` again after beta validation
...
17. If the stable release was published directly to `latest` and `beta` should
    follow it, start `.github/workflows/juno-npm-release.yml` again with
...
- Use `juno-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
