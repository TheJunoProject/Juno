...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Juno to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "juno.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.JUNO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.JUNO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set JUNO_APNS_TEAM_ID and JUNO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.JUNO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.JUNO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.JUNO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set JUNO_APNS_PRIVATE_KEY_P8 or JUNO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading JUNO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
