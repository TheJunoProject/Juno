...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\Juno\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/juno/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/juno/juno.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/juno/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/juno/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the juno executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\juno.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/juno-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/juno-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\juno-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/juno/custom.js", "gateway"])).toBe(false);
