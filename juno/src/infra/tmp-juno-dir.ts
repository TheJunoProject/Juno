...
export const POSIX_JUNO_TMP_DIR = "/tmp/juno";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredJunoTmpDirOptions = {
...
export function resolvePreferredJunoTmpDir(
  options: ResolvePreferredJunoTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "juno" : `juno-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[juno] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Juno temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Juno temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_JUNO_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_JUNO_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_JUNO_TMP_DIR)) {
      return POSIX_JUNO_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_JUNO_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_JUNO_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_JUNO_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_JUNO_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_JUNO_TMP_DIR;
