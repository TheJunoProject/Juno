...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__juno_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "juno-root");
...
function setPackageRoot(root: string, name = "juno") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./juno-root.js").resolveJunoPackageRootSync,
  asyncResolver: typeof import("./juno-root.js").resolveJunoPackageRoot,
  opts: Parameters<typeof import("./juno-root.js").resolveJunoPackageRootSync>[0],
...
vi.mock("./juno-root.fs.runtime.js", () => ({
  openJunoRootFsSync: mockFsModule(),
  openJunoRootFs: mockFsPromisesModule(),
}));

describe("resolveJunoPackageRoot", () => {
  let resolveJunoPackageRoot: typeof import("./juno-root.js").resolveJunoPackageRoot;
  let resolveJunoPackageRootSync: typeof import("./juno-root.js").resolveJunoPackageRootSync;

  beforeAll(async () => {
    ({ resolveJunoPackageRoot, resolveJunoPackageRootSync } =
      await import("./juno-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "juno");
        const pkgRoot = path.join(project, "node_modules", "juno");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "juno");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "juno.mjs")));
    ...
    {
      name: "falls through from a non-juno moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-juno");
    ...
    {
      name: "returns null for non-juno package roots",
      setup: () => {
        const pkgRoot = fx("not-juno");
        setPackageRoot(pkgRoot, "not-juno");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "juno.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "juno");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveJunoPackageRootSync,
      resolveJunoPackageRoot,
      opts,
      expected,
    );
  });
