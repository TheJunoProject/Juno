...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "JUNO_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "JUNO_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "JUNO_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "JUNO_BUNDLED_SKILLS_DIR=./attacker-skills",
  "JUNO_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "juno-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.JUNO_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.juno/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.juno/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".juno");
        process.env.JUNO_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "juno", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "JUNO_STATE_DIR=./evil-state",
            "JUNO_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.JUNO_STATE_DIR).toBe(stateDir);
        expect(process.env.JUNO_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "JUNO_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "JUNO_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "JUNO_LIVE_GEMINI_KEY=sk-gemini-live",
            "JUNO_LIVE_OPENAI_KEY=sk-openai-live",
            "JUNO_GATEWAY_TOKEN=attacker-token",
            "JUNO_GATEWAY_PASSWORD=attacker-password",
            "JUNO_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks JUNO_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.JUNO_STATE_DIR).toBeUndefined();
        expect(process.env.JUNO_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (JUNO_AGENT_DIR, JUNO_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, JUNO_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "JUNO_AGENT_DIR=./evil-agent",
            `JUNO_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "JUNO_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.JUNO_AGENT_DIR;
        delete process.env.JUNO_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.JUNO_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.JUNO_AGENT_DIR).toBeUndefined();
        expect(process.env.JUNO_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.JUNO_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks JUNO_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "JUNO_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.JUNO_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.JUNO_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "JUNO_PINNED_PYTHON=./attacker-python",
            "JUNO_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.JUNO_PINNED_PYTHON;
        delete process.env.JUNO_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.JUNO_PINNED_PYTHON).toBeUndefined();
        expect(process.env.JUNO_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "JUNO_PINNED_PYTHON=/trusted/python",
            "JUNO_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.JUNO_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.JUNO_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "JUNO_LIVE_GEMINI_KEY=sk-gemini-live",
            "JUNO_LIVE_OPENAI_KEY=sk-openai-live",
            "JUNO_GATEWAY_TOKEN=trusted-token",
            "JUNO_GATEWAY_PASSWORD=trusted-password",
            "JUNO_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.JUNO_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.JUNO_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.JUNO_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.JUNO_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.JUNO_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.JUNO_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.JUNO_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "JUNO_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when JUNO_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.JUNO_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "juno", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.JUNO_STATE_DIR).toBe(customStateDir);
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("juno/juno");
