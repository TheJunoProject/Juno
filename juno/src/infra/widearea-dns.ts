...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.JUNO_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*juno-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "juno";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "juno");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "juno-gw");
  ...
  records.push(`_juno-gw._tcp IN PTR ${instanceLabel}._juno-gw._tcp`);
  records.push(`${instanceLabel}._juno-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._juno-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; juno-content-hash: ${contentHash}\n${contentBody}`;
}
