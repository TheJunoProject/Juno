...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "juno.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "juno.plugin.json");
...
function statusCommandSpawn() {
  return [process.execPath, "juno.mjs", "status"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "juno-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "juno.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "juno-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "juno.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "juno.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...
          JUNO_FORCE_BUILD: "1",
          JUNO_RUNNER_LOG: "1",
          JUNO_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[juno]");
      expect(spawnCalls.at(-1)?.args).toEqual(["juno.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.JUNO_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "juno-run-node-" }, async (tmp) => {
      ...
      expect(childArgs).toEqual(["juno.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.JUNO_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
  ...
  it("forwards wrapper SIGTERM to the active juno child and returns 143", async () => {
    await withTempDir({ prefix: "juno-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir({ prefix: "juno-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","juno":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","juno":{"extensions":["./stale.js"]}}\n',
        },
