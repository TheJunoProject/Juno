...
function candidateBinDirs(opts: EnsureJunoPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Juno process is on.
  try {
  ...
  // Bundled macOS app: `juno` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "juno");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.JUNO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "juno"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `juno`
...
/**
 * Best-effort PATH bootstrap so skills that require the `juno` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureJunoCliOnPath(opts: EnsureJunoPathOpts = {}) {
  if (isTruthyEnvValue(process.env.JUNO_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.JUNO_PATH_BOOTSTRAPPED = "1";
