...
import { resolveJunoPackageRoot } from "./juno-root.js";
...
function resolveAutoUpdatePolicy(cfg: JunoConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: JunoConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveJunoPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("juno update")}`,
      );
