...
import { openJunoRootFs, openJunoRootFsSync } from "./juno-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["juno"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await openJunoRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      openJunoRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = openJunoRootFsSync.realpathSync(normalized);
...
export async function resolveJunoPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveJunoPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
