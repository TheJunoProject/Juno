...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.JUNO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Juno. You can keep running the web gateway without it: \`${formatCliCommand("juno gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
