...
const resolvePreferredJunoTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".juno", "gateway.cmd");
  }),
);
...
vi.mock("./tmp-juno-dir.js", () => ({
  resolvePreferredJunoTmpDir: () => resolvePreferredJunoTmpDirMock(),
}));
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredJunoTmpDirMock.mockReset();
    resolvePreferredJunoTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ JUNO_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Juno Gateway (work)"']),
    });
    ...
    expect(script).toContain(
      'juno restart attempt source=windows-task-handoff target="Juno Gateway (work)"',
    );
    expect(script).toContain('schtasks /Run /TN "Juno Gateway (work)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers JUNO_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Juno Gateway (custom)" >>');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredJunoTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ JUNO_PROFILE: "work" });
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-state-"));
