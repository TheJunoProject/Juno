...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: JunoConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: JunoConfig) {
...
function resolveHeartbeatAgents(cfg: JunoConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: JunoConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: JunoConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: JunoConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
