...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.JUNO_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Juno";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Juno\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.JUNO_MDNS_HOSTNAME?.trim() || "juno";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
