...
export function triggerJunoRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.JUNO_SYSTEMD_UNIT,
      process.env.JUNO_PROFILE,
    );
  ...
  const label =
    process.env.JUNO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.JUNO_PROFILE);
