...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): JunoConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): JunoConfig {
  return {
