...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "juno", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["JUNO_UPDATE_PACKAGE_SPEC"]);
    process.env.JUNO_UPDATE_PACKAGE_SPEC = "file:/tmp/juno.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "juno", tag: "latest" })).toBe(
      "file:/tmp/juno.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "juno",
        tag: "beta",
        env: { JUNO_UPDATE_PACKAGE_SPEC: "juno@next" },
      }),
    ).toBe("juno@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "juno"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "juno", tag: "main" })).toBe(
      JUNO_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "juno",
        tag: "github:juno/juno#feature/my-branch",
      }),
    ).toBe("github:juno/juno#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "juno",
        tag: "https://example.com/juno-main.tgz",
      }),
    ).toBe("https://example.com/juno-main.tgz");
  });
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:juno/juno#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/juno-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/juno-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:juno/juno#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "juno-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "juno");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "juno"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "juno"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "juno"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "juno"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "juno-update-npm-prefix-" }, async (base) => {
        ...
        const pkgRoot = path.join(brewRoot, "juno");
        ...
        expect(globalInstallArgs("npm", "juno@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "juno@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "juno@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "juno@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "juno-update-npm-missing-bin-" }, async (base) => {
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    ...
    try {
      await withTempDir({ prefix: "juno-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "juno");
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "juno@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "juno@latest",
    ...
    expect(globalInstallArgs("pnpm", "juno@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "juno@latest",
    ]);
    expect(globalInstallArgs("bun", "juno@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "juno@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "juno@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "juno@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "juno@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "juno@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "juno@latest"]);
  });

  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "juno-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".juno-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".juno-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".juno-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "juno"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".juno-123", ".juno-456"],
      });
      await expect(fs.stat(path.join(root, "juno"))).resolves.toBeDefined();
      await expect(fs.stat(path.join(root, ".juno-file"))).resolves.toBeDefined();
    });
  });

  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "juno-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("falls back to legacy sidecar verification when the inventory is missing", async () => {
    await withTempDir({ prefix: "juno-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeCompatSidecars(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "juno-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "juno-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "juno-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "matrix", "@juno/matrix");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "juno-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeCompatSidecars(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "matrix", "@juno/matrix");
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "juno-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeCompatSidecars(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@juno/qa-lab");
        await writePackageDistInventory(packageRoot);
