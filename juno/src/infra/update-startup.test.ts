...
vi.mock("./juno-root.js", async () => {
  const actual = await vi.importActual<typeof import("./juno-root.js")>("./juno-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "juno-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["JUNO_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.JUNO_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveJunoPackageRoot } = await import("./juno-root.js"));
    ...
    vi.mocked(resolveJunoPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveJunoPackageRoot).mockResolvedValue("/opt/juno");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/juno/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
