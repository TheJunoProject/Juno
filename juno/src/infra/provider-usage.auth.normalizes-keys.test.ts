...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "juno-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".juno");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".juno", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      JUNO_STATE_DIR: path.join(home, ".juno"),
      ...env,
    };
