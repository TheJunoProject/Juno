...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/juno-state", "devices")).toEqual({
      dir: path.join("/tmp/juno-state", "devices"),
      pendingPath: path.join("/tmp/juno-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/juno-state", "devices", "paired.json"),
    });
  });
