...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevJunoHome = process.env.JUNO_HOME;
    const prevHome = process.env.HOME;
    process.env.JUNO_HOME = "/srv/juno-home";
    process.env.HOME = "/home/other";
    const openJunoHome = path.join(path.resolve("/srv/juno-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", openJunoHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevJunoHome === undefined) {
        delete process.env.JUNO_HOME;
      } else {
        process.env.JUNO_HOME = prevJunoHome;
      }
