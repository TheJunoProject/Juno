...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __junoDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__junoDiagnosticEventsState) {
    globalStore.__junoDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__junoDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: JunoConfig): boolean {
