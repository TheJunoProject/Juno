import { mapAllowFromEntries } from "juno/plugin-sdk/channel-config-helpers";
...
function buildWebChatDeliveryError(): Error {
  return new Error(
    `Delivering to WebChat is not supported via \`${formatCliCommand("juno agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
  );
}
