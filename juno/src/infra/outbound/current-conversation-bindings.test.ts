...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.JUNO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-current-bindings-"));
    process.env.JUNO_STATE_DIR = testStateDir;
