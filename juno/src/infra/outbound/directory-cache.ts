...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: JunoConfig | null = null;
  ...
  get(key: string, cfg: JunoConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: JunoConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: JunoConfig): void {
