...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers JUNO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for JUNO_HOME",
      env: { JUNO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when JUNO_HOME is tilde-only and no fallback home exists",
      env: { JUNO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores JUNO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          JUNO_HOME: "/srv/juno-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { JUNO_HOME: "/srv/juno-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/juno-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/juno-home " },
      expected: "/srv/juno-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/juno-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of JUNO_HOME", () => {
    expect(
