...
export type PromptDefaultModelResult = { model?: string; config?: JunoConfig };
...
function resolveConfiguredModelRaw(cfg: JunoConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: JunoConfig): string[] {
...
export function applyModelAllowlist(cfg: JunoConfig, models: string[]): JunoConfig {
