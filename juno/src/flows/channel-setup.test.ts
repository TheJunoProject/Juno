...
function makeCatalogEntry(
  ...
  return {
    ...
    install: overrides.install ?? { npmSpec: `@juno/${id}` },
  };
}
...
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, _agentId?: unknown) => "/tmp/juno-workspace"),
);
...
describe("setupChannels workspace shadow exclusion", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/juno-workspace");
