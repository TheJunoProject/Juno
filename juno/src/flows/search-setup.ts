...
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<JunoConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: JunoConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: JunoConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: JunoConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: JunoConfig, result: JunoConfig): JunoConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as JunoConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with juno configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Juno will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.juno.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Juno will store a reference instead of the API key.",
