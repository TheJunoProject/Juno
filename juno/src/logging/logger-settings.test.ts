...
let originalJunoLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.JUNO_TEST_FILE_LOG;
  originalJunoLogLevel = process.env.JUNO_LOG_LEVEL;
  delete process.env.JUNO_TEST_FILE_LOG;
  delete process.env.JUNO_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.JUNO_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalJunoLogLevel === undefined) {
    delete process.env.JUNO_LOG_LEVEL;
  } else {
    process.env.JUNO_LOG_LEVEL = originalJunoLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
