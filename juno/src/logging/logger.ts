...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredJunoTmpDir() : POSIX_JUNO_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "juno.log")
    : `${POSIX_JUNO_TMP_DIR}/juno.log`;
}
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.JUNO_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: JunoConfig["logging"] | undefined =
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.JUNO_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[juno] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
