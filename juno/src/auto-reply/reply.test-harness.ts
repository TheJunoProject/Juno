...
type HomeEnvSnapshot = {
  ...
  JUNO_STATE_DIR: string | undefined;
  JUNO_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    JUNO_AGENT_DIR: process.env.JUNO_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".juno", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.JUNO_STATE_DIR = path.join(home, ".juno");
    process.env.JUNO_AGENT_DIR = path.join(home, ".juno", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".juno", "agent");
