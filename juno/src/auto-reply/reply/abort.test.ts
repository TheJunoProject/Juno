...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "juno-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop juno!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@juno_bot", { botUsername: "juno_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@juno_bot", { botUsername: "juno_bot" })).toBe(true);
