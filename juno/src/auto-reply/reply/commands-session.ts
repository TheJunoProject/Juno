...
import { scheduleGatewaySigusr1Restart, triggerJunoRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Juno in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerJunoRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Juno via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
