...
const { buildStatusReplyMock, createJunoToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createJunoToolsMock: vi.fn(),
...
vi.mock("../../agents/juno-tools.runtime.js", () => ({
  createJunoTools: (...args: unknown[]) => createJunoToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createJunoToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
