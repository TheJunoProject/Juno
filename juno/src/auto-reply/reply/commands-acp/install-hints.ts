...
export function resolveAcpInstallCommandHint(cfg: JunoConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `juno plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^juno plugins install /u, "");
    ...
    return "juno plugins install acpx";
  }
