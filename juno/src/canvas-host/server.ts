...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.junoCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.junoCanvasA2UIAction &&
        typeof window.junoCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.junoSendUserAction === "function";
  ...
  window.addEventListener("juno:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Juno node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.junoSendUserAction === "function"
        ? window.junoSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.JUNO_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.JUNO_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Juno Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
