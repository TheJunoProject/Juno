...
describe("canvas host state dir defaults", () => {
  ...
  it("uses JUNO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("juno-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Juno Canvas");
      } finally {
        await handler.close();
      }
    });
