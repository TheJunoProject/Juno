...
const { createJunoTools } = await import("../agents/juno-tools.js");
...
type SessionSendTool = ReturnType<typeof createJunoTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createJunoTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["JUNO_GATEWAY_PORT", "JUNO_GATEWAY_TOKEN"]);
  ...
  process.env.JUNO_GATEWAY_PORT = String(gatewayPort);
  process.env.JUNO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("JUNO_CONFIG_PATH missing in gateway test environment");
      }
