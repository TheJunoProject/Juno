...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: JunoConfig) => ChannelHealthMonitor | null;
};
...
> & {
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: JunoConfig) => string | undefined;
  sharedGatewaySessionGenerationState: SharedGatewaySessionGenerationState;
  clients: Iterable<SharedGatewayAuthClient>;
};

export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: JunoConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (JUNO_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.JUNO_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.JUNO_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (JUNO_SKIP_CHANNELS=1 or JUNO_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: JunoConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
