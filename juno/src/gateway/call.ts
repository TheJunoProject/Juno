...
function loadGatewayConfig(): JunoConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: JunoConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.JUNO_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as JunoConfig) : loadGatewayConfig());
