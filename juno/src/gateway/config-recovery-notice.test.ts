...
describe("config recovery notice", () => {
  ...
  it("formats a prompt-facing warning for recovered configs", () => {
    expect(
      formatConfigRecoveryNotice({
        ...
        configPath: "/home/test/.juno/juno.json",
      }),
    ).toBe(
      "Config recovery warning: Juno restored juno.json from the last-known-good backup during startup (startup-invalid-config). The rejected config was invalid and was preserved as a timestamped .clobbered.* file. Do not write juno.json again unless you validate the full config first.",
    );
  });
  ...
  it("queues the notice for the main agent session", () => {
    ...
    expect(drainSystemEvents("agent:main:main")[0]).toContain(
      "Do not write juno.json again unless you validate the full config first.",
    );
  });
});
