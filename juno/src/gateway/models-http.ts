...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([JUNO_MODEL_ID, JUNO_DEFAULT_MODEL_ID]);
  ids.add(`juno/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`juno/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== JUNO_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
