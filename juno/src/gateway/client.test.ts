...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("juno doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with JUNO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with JUNO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.JUNO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://juno-gateway.ai:18789",
      onConnectError,
    });
