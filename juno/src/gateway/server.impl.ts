...
import { ensureJunoCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.JUNO_TEST_MINIMAL_GATEWAY === "1";
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `juno config set gateway.auth.mode token` and `juno config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: JunoConfig) =>
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: JunoConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
