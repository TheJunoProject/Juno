...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "juno-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.JUNO_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as JunoConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "juno-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.JUNO_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
