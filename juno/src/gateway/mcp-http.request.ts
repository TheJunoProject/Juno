...
function resolveScopedSessionKey(cfg: JunoConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  const senderIsOwnerRaw = normalizeOptionalLowercaseString(
    getHeader(req, "x-juno-sender-is-owner"),
  );
  return {
    sessionKey: resolveScopedSessionKey(cfg, getHeader(req, "x-session-key")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-juno-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-juno-account-id")),
    senderIsOwner:
