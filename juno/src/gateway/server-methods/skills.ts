...
import { fetchJunoHubSkillDetail } from "../../infra/junohub.js";
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.install": async ({ params, respond }) => {
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "junohub") {
      const p = params as {
        source: "junohub";
        slug: string;
        version?: string;
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "junohub") {
      ...
      if (!p.slug && !p.all) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, 'junohub skills.update requires "slug" or "all"'),
        );
        return;
      }
      if (p.slug && p.all) {
        respond(
          ...
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            'junohub skills.update accepts either "slug" or "all", not both',
          ),
        );
        return;
      }
