...
const loadConfig = vi.hoisted(() => vi.fn(() => ({}) as JunoConfig));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: JunoConfig, _agentId: string) => "/tmp/juno"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: JunoConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
describe("doctor.memory.status", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/juno");
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: JunoConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__juno_memory_core_short_term_promotion_dream__",
        },
  ...
  it("reads dreaming config from the selected memory slot plugin", async () => {
    loadConfig.mockReturnValue({
      plugins: {
        slots: {
          memory: "memos-local-juno-plugin",
        },
        entries: {
          "memos-local-juno-plugin": {
            config: {
              dreaming: {
                enabled: true,
                frequency: "0 */4 * * *",
  ...
  it("merges workspace store errors when multiple workspace stores are unreadable", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: JunoConfig, agentId: string) =>
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/juno");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/juno/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/juno/.juno-repair/dreaming/2026-04-11T22-00-00-000Z",
