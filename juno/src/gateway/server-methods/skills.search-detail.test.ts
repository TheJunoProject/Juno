...
const searchSkillsFromJunoHubMock = vi.fn();
...
vi.mock("../../agents/skills-junohub.js", () => ({
  installSkillFromJunoHub: vi.fn(),
  updateSkillsFromJunoHub: vi.fn(),
  searchSkillsFromJunoHub: (...args: unknown[]) => searchSkillsFromJunoHubMock(...args),
}));

vi.mock("../../infra/junohub.js", () => ({
  fetchJunoHubSkillDetail: (...args: unknown[]) => fetchJunoHubSkillDetailMock(...args),
  resolveJunoHubBaseUrl: vi.fn(() => "https://junohub.ai"),
  searchJunoHubSkills: vi.fn(),
  downloadJunoHubSkillArchive: vi.fn(),
}));
...
describe("skills.search handler", () => {
  ...
  it("searches JunoHub with query and limit", async () => {
  ...
  it("returns error when JunoHub is unreachable", async () => {
    searchSkillsFromJunoHubMock.mockRejectedValue(new Error("connection refused"));
  ...
  it("rejects limit below minimum", async () => {
    ...
    expect(searchSkillsFromJunoHubMock).not.toHaveBeenCalled();
  });
...
describe("skills.detail handler", () => {
  ...
  it("fetches detail for a valid slug", async () => {
    ...
    fetchJunoHubSkillDetailMock.mockResolvedValue(detail);

    const { ok, response, error } = await callHandler("skills.detail", {
      slug: "github",
    });

    expect(fetchJunoHubSkillDetailMock).toHaveBeenCalledWith({ slug: "github" });
    expect(ok).toBe(true);
    expect(error).toBeUndefined();
  ...
  it("returns error when slug is not found", async () => {
    fetchJunoHubSkillDetailMock.mockRejectedValue(new Error("not found"));
  ...
  it("rejects missing slug", async () => {
    ...
    expect(ok).toBe(false);
    expect(error).toMatchObject({ code: "INVALID_REQUEST" });
    expect(fetchJunoHubSkillDetailMock).not.toHaveBeenCalled();
  });
