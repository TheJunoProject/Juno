...
function listConfiguredAgentIds(cfg: JunoConfig): string[] {
...
export function listAgentsForGateway(cfg: JunoConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function loadCombinedSessionStoreForGateway(cfg: JunoConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: JunoConfig): GatewaySessionsDefaults {
