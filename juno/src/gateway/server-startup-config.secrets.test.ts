...
function gatewayTokenConfig(config: JunoConfig): JunoConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): JunoConfig {
  return value as JunoConfig;
}

function buildSnapshot(config: JunoConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/juno-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: JunoConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.JUNO_SKIP_CHANNELS;
  const previousSkipProviders = process.env.JUNO_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.JUNO_SKIP_CHANNELS;
    } else {
      process.env.JUNO_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.JUNO_SKIP_PROVIDERS;
    } else {
      process.env.JUNO_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
