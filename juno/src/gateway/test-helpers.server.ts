...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "JUNO_SKIP_GMAIL_WATCHER",
  "JUNO_SKIP_CANVAS_HOST",
  "JUNO_BUNDLED_PLUGINS_DIR",
  "JUNO_SKIP_CHANNELS",
  "JUNO_SKIP_PROVIDERS",
  "JUNO_SKIP_CRON",
  "JUNO_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.JUNO_CONFIG_PATH) {
    configPaths.add(process.env.JUNO_CONFIG_PATH);
  }
  if (process.env.JUNO_STATE_DIR) {
    configPaths.add(path.join(process.env.JUNO_STATE_DIR, "juno.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "juno-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.JUNO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "juno-test-no-bundled-extensions")
    : "juno-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.JUNO_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".juno-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".juno-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".juno-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>juno-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__junoTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__junoTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["JUNO_GATEWAY_TOKEN"]);
  const prev = process.env.JUNO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.JUNO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.JUNO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
