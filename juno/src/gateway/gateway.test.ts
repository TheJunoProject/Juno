...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "juno-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".juno", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["JUNO_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "juno");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.JUNO_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".juno");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "juno-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "juno-wizard-home-"));
      process.env.HOME = tempHome;
      process.env.JUNO_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "juno-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".juno", "juno.json");
      const bundledPluginsDir = path.join(tempHome, "juno-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.JUNO_STATE_DIR = path.join(tempHome, ".juno");
      process.env.JUNO_CONFIG_PATH = configPath;
      process.env.JUNO_SKIP_CHANNELS = "1";
      ...
      process.env.JUNO_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.JUNO_SKIP_PROVIDERS = "1";
      process.env.JUNO_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.JUNO_TEST_MINIMAL_GATEWAY = "1";
