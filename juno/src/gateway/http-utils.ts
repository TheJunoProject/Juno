...
export const JUNO_MODEL_ID = "juno";
export const JUNO_DEFAULT_MODEL_ID = "juno/default";
...
}): Promise<{ cfg: JunoConfig; requestAuth: AuthorizedGatewayHttpRequest } | null> {
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-juno-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-juno-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-juno-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === JUNO_MODEL_ID || lowered === JUNO_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^juno[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `juno` or `juno/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-juno-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-juno-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-juno-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-juno-message-channel")) ??
