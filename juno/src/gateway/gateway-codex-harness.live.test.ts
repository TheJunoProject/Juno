...
const CODEX_HARNESS_LIVE = isTruthyEnvValue(process.env.JUNO_LIVE_CODEX_HARNESS);
const CODEX_HARNESS_DEBUG = isTruthyEnvValue(process.env.JUNO_LIVE_CODEX_HARNESS_DEBUG);
const CODEX_HARNESS_IMAGE_PROBE = isTruthyEnvValue(
  process.env.JUNO_LIVE_CODEX_HARNESS_IMAGE_PROBE,
);
const CODEX_HARNESS_MCP_PROBE = isTruthyEnvValue(process.env.JUNO_LIVE_CODEX_HARNESS_MCP_PROBE);
const CODEX_HARNESS_AUTH_MODE =
  process.env.JUNO_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "api-key" : "codex-auth";
...
function snapshotEnv(): EnvSnapshot {
  return {
    agentRuntime: process.env.JUNO_AGENT_RUNTIME,
    configPath: process.env.JUNO_CONFIG_PATH,
    gatewayToken: process.env.JUNO_GATEWAY_TOKEN,
...
function restoreEnv(snapshot: EnvSnapshot): void {
  restoreEnvVar("JUNO_AGENT_RUNTIME", snapshot.agentRuntime);
  restoreEnvVar("JUNO_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("JUNO_GATEWAY_TOKEN", snapshot.gatewayToken);
...
describeLive("gateway live (Codex harness)", () => {
  it(
    ...
    async () => {
      const modelKey = process.env.JUNO_LIVE_CODEX_HARNESS_MODEL ?? DEFAULT_CODEX_MODEL;
      const { clearRuntimeConfigSnapshot } = await import("../config/config.js");
      const { startGatewayServer } = await import("./server.js");

      const previousEnv = snapshotEnv();
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-live-codex-harness-"));
      const stateDir = path.join(tempDir, "state");
      const workspace = await createLiveWorkspace(tempDir);
      const configPath = path.join(tempDir, "juno.json");
      const token = `test-${randomUUID()}`;
      const port = await getFreeGatewayPort();

      clearRuntimeConfigSnapshot();
      process.env.JUNO_AGENT_RUNTIME = "codex";
      process.env.JUNO_AGENT_HARNESS_FALLBACK = "none";
      ...
      process.env.JUNO_CONFIG_PATH = configPath;
