...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: JunoConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.JUNO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.JUNO_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set JUNO_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: juno doctor --fix",
