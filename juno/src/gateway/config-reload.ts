...
export function resolveGatewayReloadSettings(cfg: JunoConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: JunoConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: JunoConfig) => void | Promise<void>;
  ...
  let pendingInProcessConfig: { config: JunoConfig; persistedHash: string } | null = null;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: JunoConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: JunoConfig) => {
