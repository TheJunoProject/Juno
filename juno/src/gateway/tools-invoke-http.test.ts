...
let lastCreateJunoToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/juno-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateJunoToolsContext?.agentTo,
          agentThreadId: lastCreateJunoToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createJunoTools: (ctx: Record<string, unknown>) => {
      lastCreateJunoToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateJunoToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-juno-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-juno-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateJunoToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateJunoToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateJunoToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateJunoToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateJunoToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateJunoToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-juno-message-to": "channel:24514",
        "x-juno-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
