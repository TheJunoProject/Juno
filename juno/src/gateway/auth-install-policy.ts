...
type GatewayInstallAuthMode = NonNullable<NonNullable<JunoConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: JunoConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.JUNO_GATEWAY_PASSWORD) ||
    normalizeOptionalString(durableServiceEnv.JUNODBOT_GATEWAY_PASSWORD),
  );
}
