  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-juno-account-id": "work",
        "x-juno-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-juno-message-channel": "matrix",
          "x-juno-sender-is-owner": senderIsOwner,
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.juno?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.juno?.headers?.Authorization).toBe(
      "Bearer ${JUNO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.juno?.headers?.["x-juno-message-channel"]).toBe(
      "${JUNO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.juno?.headers?.["x-juno-sender-is-owner"]).toBe(
      "${JUNO_MCP_SENDER_IS_OWNER}",
    );
  });
});
