...
function shouldLogCliCronProbe(): boolean {
  return (
    isTruthyEnvValue(process.env.JUNO_LIVE_CLI_BACKEND_DEBUG) ||
    isTruthyEnvValue(process.env.JUNO_CLI_BACKEND_LOG_OUTPUT)
  );
}
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("JUNO_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.JUNO_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.JUNO_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "JUNO_LIVE_CLI_BACKEND_ARGS",
      process.env.JUNO_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `JUNO_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "JUNO_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.JUNO_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.JUNO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
async function callLoopbackJsonRpc(params: {
  ...
  const headers: Record<string, string> = {
    ...
    "x-juno-sender-is-owner": params.senderIsOwner ? "true" : "false",
  };
  if (params.messageProvider) {
    headers["x-juno-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-juno-account-id"] = params.accountId;
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.JUNO_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.JUNO_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.JUNO_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.JUNO_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.JUNO_SKIP_PROVIDERS = "1";
  process.env.JUNO_SKIP_GMAIL_WATCHER = "1";
  process.env.JUNO_SKIP_CRON = "1";
  process.env.JUNO_SKIP_CANVAS_HOST = "1";
  process.env.JUNO_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.JUNO_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("JUNO_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("JUNO_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("JUNO_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("JUNO_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("JUNO_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("JUNO_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("JUNO_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("JUNO_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("JUNO_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("JUNO_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("JUNO_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
