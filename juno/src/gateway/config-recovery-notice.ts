...
export function formatConfigRecoveryNotice(params: {
  ...
  const configName = path.basename(params.configPath) || "juno.json";
  return [
    `Config recovery warning: Juno restored ${configName} from the last-known-good backup during ${params.phase} (${params.reason}).`,
