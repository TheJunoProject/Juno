...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.JUNO_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.JUNO_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.JUNO_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.JUNO_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "juno.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.JUNO_GATEWAY_PORT = previous.port;
        }
