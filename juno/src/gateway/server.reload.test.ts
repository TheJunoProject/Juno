...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.JUNO_SKIP_CHANNELS;
    prevSkipGmail = process.env.JUNO_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.JUNO_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.JUNO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.JUNO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.JUNO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.JUNO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.JUNO_SKIP_PROVIDERS;
    } else {
      process.env.JUNO_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("JUNO_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ JUNO_TEST_MINIMAL_GATEWAY: undefined }, async () =>
...
`,
    ...
    const previousGatewayTokenEnv = process.env.JUNO_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.JUNO_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
