...
export function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("juno doctor --fix")}" to repair, then retry.`
    : "";
...
function pruneSkippedStartupSecretSurfaces(config: JunoConfig): JunoConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: JunoConfig): void {
