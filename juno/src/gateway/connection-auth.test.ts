...
function cfg(input: Partial<JunoConfig>): JunoConfig {
  return input as JunoConfig;
}
...
const DEFAULT_ENV = {
  JUNO_GATEWAY_TOKEN: "env-token",
  JUNO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when JUNO env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when JUNO env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when JUNO env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
