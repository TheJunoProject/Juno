...
describe("gateway auth", () => {
  ...
  it("resolves token/password from JUNO gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          JUNO_GATEWAY_TOKEN: "env-token",
          JUNO_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides JUNO_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        JUNO_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
