...
import type { loadJunoPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureJunoModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: JunoConfig;
  pluginRegistry: ReturnType<typeof loadJunoPlugins>;
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (JUNO_SKIP_CHANNELS=1 or JUNO_SKIP_PROVIDERS=1)",
    );
  }
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: JunoConfig;
    pluginRegistry: ReturnType<typeof loadJunoPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
