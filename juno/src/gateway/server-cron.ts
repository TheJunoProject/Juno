...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.JUNO_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: JunoConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: JunoConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: JunoConfig, requestedAgentId: string) => {
