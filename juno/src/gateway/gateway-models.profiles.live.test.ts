...
import { resolveJunoAgentDir } from "../agents/agent-paths.js";
...
import { ensureJunoModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, JunoConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.JUNO_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.JUNO_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.JUNO_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.JUNO_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.JUNO_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.JUNO_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["JUNO_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.JUNO_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.JUNO_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.JUNO_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.JUNO_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.JUNO_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.JUNO_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.JUNO_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.JUNO_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("JUNO_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("JUNO_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("JUNO_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.JUNO_LIVE_GATEWAY_MODELS;
    delete process.env.JUNO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.JUNO_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.JUNO_LIVE_GATEWAY_MODELS = "openai/gpt-5.4,anthropic/claude-opus-4-6";
    delete process.env.JUNO_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.JUNO_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.JUNO_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: JunoConfig;
  agentDir: string;
}): Promise<JunoConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<JunoConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.JUNO_CONFIG_PATH,
    token: process.env.JUNO_GATEWAY_TOKEN,
    skipChannels: process.env.JUNO_SKIP_CHANNELS,
    skipGmail: process.env.JUNO_SKIP_GMAIL_WATCHER,
    skipCron: process.env.JUNO_SKIP_CRON,
    skipCanvas: process.env.JUNO_SKIP_CANVAS_HOST,
    disableBonjour: process.env.JUNO_DISABLE_BONJOUR,
    logLevel: process.env.JUNO_LOG_LEVEL,
    agentDir: process.env.JUNO_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.JUNO_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.JUNO_SKIP_CHANNELS = "1";
  process.env.JUNO_SKIP_GMAIL_WATCHER = "1";
  process.env.JUNO_SKIP_CRON = "1";
  process.env.JUNO_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.JUNO_DISABLE_BONJOUR = "1";
    process.env.JUNO_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.JUNO_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveJunoAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-live-state-"));
  process.env.JUNO_STATE_DIR = tempStateDir;
  ...
  process.env.JUNO_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.juno-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveJunoAgentDir();
  const sanitizedCfg: JunoConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-live-"));
  const tempConfigPath = path.join(tempDir, "juno.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.JUNO_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "Juno live tool probe (local, safe): " +
    ...
    process.env.JUNO_CONFIG_PATH = previous.configPath;
    process.env.JUNO_GATEWAY_TOKEN = previous.token;
    process.env.JUNO_SKIP_CHANNELS = previous.skipChannels;
    process.env.JUNO_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.JUNO_SKIP_CRON = previous.skipCron;
    process.env.JUNO_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.JUNO_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.JUNO_LOG_LEVEL = previous.logLevel;
    process.env.JUNO_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.JUNO_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureJunoModelsJson(cfg);
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via JUNO_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.juno-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
