...
export function attachJunoTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__juno && typeof record.__juno === "object" && !Array.isArray(record.__juno)
      ? (record.__juno as Record<string, unknown>)
      : {};
  return {
    ...record,
    __juno: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachJunoTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "juno" && model === "delivery-mirror";
