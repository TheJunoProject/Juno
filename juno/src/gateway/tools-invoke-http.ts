...
function resolveMemoryToolDisableReasons(cfg: JunoConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-juno-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-juno-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-juno-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-juno-thread-id"));
