...
import type {
  ...
  JunoPluginApi,
  JunoPluginChannelRegistration,
  JunoPluginCliCommandDescriptor,
  JunoPluginCliRegistrar,
  JunoPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  JunoPluginGatewayRuntimeScopeSurface,
  JunoPluginHttpRouteParams,
  JunoPluginHookOptions,
  JunoPluginNodeHostCommand,
  JunoPluginReloadRegistration,
  JunoPluginSecurityAuditCollector,
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("juno.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: JunoPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: JunoPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: JunoPluginHookOptions | undefined,
    config: JunoPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: JunoPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: JunoPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as JunoPluginChannelRegistration).plugin === "object"
        ? (registration as JunoPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: JunoPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerReload = (record: PluginRecord, registration: JunoPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: JunoPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: JunoPluginNodeHostCommand,
  ) => {
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: JunoPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: JunoPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: JunoPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationMode === "full"
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<JunoPluginApi["registerCompactionProvider"]>[0],
              ) => {
