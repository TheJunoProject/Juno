...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Juno consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Juno should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Juno's bundle
   ...
   *
   * Use this when the backend should consume a canonical Juno auth profile
   ...
   * `profile-only` treats the selected Juno auth profile as the sole auth
