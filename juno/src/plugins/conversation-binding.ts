...
const APPROVALS_PATH = "~/.juno/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "juno-app-server:thread:",
  "juno-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("juno.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("juno.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Juno instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
