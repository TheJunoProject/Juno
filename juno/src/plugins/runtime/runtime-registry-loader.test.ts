...
const mocks = vi.hoisted(() => ({
  loadJunoPlugins: vi.fn<typeof import("../loader.js").loadJunoPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadJunoPlugins: (...args: Parameters<typeof mocks.loadJunoPlugins>) =>
    mocks.loadJunoPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadJunoPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadJunoPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadJunoPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
