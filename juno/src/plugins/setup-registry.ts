...
function collectConfiguredPluginEntryIds(config: JunoConfig): string[] {
...
function resolveRegister(mod: JunoPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: JunoPluginModule;
  try {
    mod = getJiti(setupSource)(setupSource) as JunoPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: JunoPluginModule }).default ?? mod);
