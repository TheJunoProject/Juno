...
function createInstalledRuntimePluginDir(
  ...
  const bundledDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `juno-runtime-contract-bundled-${pluginId}-`),
  );
  const stateDir = fs.mkdtempSync(
    path.join(os.tmpdir(), `juno-runtime-contract-state-${pluginId}-`),
  );
  ...
  fs.writeFileSync(
    path.join(pluginRoot, "juno.plugin.json"),
    JSON.stringify({
      id: pluginId,
      channels: [pluginId],
