...
import { discoverJunoPlugins } from "./discovery.js";
...
function shouldProfilePluginLoader(): boolean {
  return process.env.JUNO_PLUGIN_LOAD_PROFILE === "1";
}
...
function createGuardedPluginRegistrationApi(api: JunoPluginApi): {
  api: JunoPluginApi;
  close: () => void;
} {
  let closed = false;
...
function runPluginRegisterSync(
  register: NonNullable<JunoPluginDefinition["register"]>,
  api: Parameters<NonNullable<JunoPluginDefinition["register"]>>[0],
): void {
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadJunoPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadJunoPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: JunoPluginDefinition;
  register?: JunoPluginDefinition["register"];
} {
  const resolved = unwrapDefaultModuleExport(moduleExport);
  if (typeof resolved === "function") {
    return {
      register: resolved as JunoPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as JunoPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.JUNO_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverJunoPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverJunoPlugins>["candidates"][number];
  right: ReturnType<typeof discoverJunoPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadJunoPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadJunoPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    const discovery = discoverJunoPlugins({
      workspaceDir: options.workspaceDir,
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Juno yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: JunoPluginModule | null = null;
      try {
        ...
        mod = profilePluginLoaderSync({
          ...
          pluginId: record.id,
          source: safeSource,
          run: () => getJiti(safeSource)(safeImportSource) as JunoPluginModule,
        });
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: JunoPluginModule | null = null;
            try {
              runtimeMod = profilePluginLoaderSync({
                ...
                run: () =>
                  getJiti(safeRuntimeSource)(safeRuntimeImportSource) as JunoPluginModule,
              });
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'juno plugins list' for details.`,
        );
      }
    }
...
export async function loadJunoPluginCliRegistry(
  options: PluginLoadOptions = {},
