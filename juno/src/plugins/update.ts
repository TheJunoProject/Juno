...
import { installPluginFromJunoHub } from "./junohub.js";
import {
...
function formatJunoHubInstallFailure(params: {
  pluginId: string;
  spec: string;
  phase: "check" | "update";
  error: string;
}): string {
  return `Failed to ${params.phase} ${params.pluginId}: ${params.error} (JunoHub ${params.spec}).`;
}
...
function migratePluginConfigId(cfg: JunoConfig, fromId: string, toId: string): JunoConfig {
  if (fromId === toId) {
    return cfg;
  }
...
export async function updateNpmInstalledPlugins(params: {
  ...
  for (const pluginId of targets) {
    ...
    if (record.source !== "npm" && record.source !== "marketplace" && record.source !== "junohub") {
      outcomes.push({
        pluginId,
        status: "skipped",
    ...
    if (record.source === "junohub" && !record.junohubPackage) {
      outcomes.push({
        pluginId,
        status: "skipped",
        message: `Skipping "${pluginId}" (missing JunoHub package metadata).`,
      });
      continue;
    }
      ...
      try {
        probe =
          record.source === "npm"
            ...
            : record.source === "junohub"
              ? await installPluginFromJunoHub({
                  spec: effectiveSpec ?? `junohub:${record.junohubPackage!}`,
                  baseUrl: record.junohubUrl,
                  mode: "update",
                  dryRun: true,
                  dangerouslyForceUnsafeInstall: params.dangerouslyForceUnsafeInstall,
                  expectedPluginId: pluginId,
    ...
    } else if (record.source === "junohub") {
      const junohubResult = result as Extract<
        Awaited<ReturnType<typeof installPluginFromJunoHub>>,
        { ok: true }
      >;
      next = recordPluginInstall(next, {
        pluginId: resolvedPluginId,
        source: "junohub",
        spec: effectiveSpec ?? record.spec ?? `junohub:${record.junohubPackage!}`,
        installPath: result.targetDir,
        version: nextVersion,
