...
import { clearPluginLoaderCache, loadJunoPlugins } from "./loader.js";
import { clearPluginManifestRegistryCache } from "./manifest-registry.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadJunoPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadJunoPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "juno-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.JUNO_BUNDLED_PLUGINS_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "juno.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".juno", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ JUNO_STATE_DIR: stateDir, ...params.env }, () =>
    loadJunoPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.JUNO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
}
