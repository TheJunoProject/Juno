...
let loadJunoPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/juno.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/juno.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadJunoPluginsMock).toHaveBeenCalledTimes(count);
}
...
function expectRuntimeProviderResolution(
  ...
  expect(loadJunoPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadJunoPluginsMock = vi
      .spyOn(loaderModule, "loadJunoPlugins")
  ...
  it("retains the snapshot cache when config contents change in place", () => {
    ...
    const env = createWebSearchEnv({ JUNO_HOME: "/tmp/juno-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        JUNO_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadJunoPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      JUNO_PLUGIN_DISCOVERY_CACHE_MS: "5",
      JUNO_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadJunoPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      JUNO_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.JUNO_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
