...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "juno", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".juno", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".juno", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "juno-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".juno", "extensions"),
      },
    });
  });
});
