...
type PluginConfig = NonNullable<JunoConfig["plugins"]>;
...
function createSinglePluginWithEmptySlotsConfig(): JunoConfig {
...
function createSingleNpmInstallConfig(installPath: string): JunoConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "juno-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-juno-plugin-install/my-plugin",
      },
      extensionsDir,
    });
