...
import { clearPluginDiscoveryCache, discoverJunoPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("juno-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    JUNO_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverJunoPlugins>[0],
) {
  return discoverJunoPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverJunoPlugins>[0]) {
  return discoverJunoPlugins(params);
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      juno: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "juno.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverJunoPlugins>>,
...
describe("discoverJunoPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".juno", "extensions");
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@juno/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@juno/stray-workspace-plugin",
    });

    const result = discoverJunoPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".juno", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    ...
    const { candidates, diagnostics } = discoverJunoPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-juno");
    ...
    writePluginPackageManifest({
      ...
      packageName: "@opik/opik-juno",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-juno" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "juno"),
      packageName: "juno",
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-juno"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".juno", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "juno");
    ...
    const { candidates } = discoverJunoPlugins({
      workspaceDir,
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@juno/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@juno/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@juno/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("juno");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@juno/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const result = discoverJunoPlugins({
        env: {
          ...process.env,
          JUNO_DISABLE_BUNDLED_PLUGINS: undefined,
          JUNO_STATE_DIR: stateDir,
          JUNO_BUNDLED_PLUGINS_DIR: bundledDir,
        },
      });
  ...
  it("reuses bundled and global discovery across workspace-specific cache misses", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@juno/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@juno/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".juno", "extensions", "workspace-a-plugin"),
      packageName: "@juno/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".juno", "extensions", "workspace-b-plugin"),
      packageName: "@juno/workspace-b-plugin",
