...
const suiteTempRootTracker = createSuiteTempRootTracker("juno-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@juno/voice-call@0.0.2-beta.1",
      name: "@juno/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@juno/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@juno/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@juno/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
