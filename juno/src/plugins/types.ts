...
import type {
  JunoPluginHookOptions,
  JunoPluginToolContext,
  JunoPluginToolFactory,
  JunoPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type JunoPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<JunoConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Juno uses this to expose
 ...
 *
 * Runs after Juno resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Juno auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Juno wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Juno builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Juno no
...
/**
 * Provider-owned stream wrapper hook after Juno applies its generic
 ...
 *
 * Return `true` or `false` to override Juno's built-in provider cache TTL
 ...
 *
 * Runs only after Juno exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Juno should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Juno loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Juno falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Juno calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Juno performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Juno registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Juno wrappers.
   *
   ...
   *
   * Called after Juno resolves the raw configured credential but before the
   ...
   * hint than Juno's generic auth-store guidance.
   */
   ...
   * Juno's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Juno should surface a provider-specific hint for
   ...
   * merging. Juno deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full Juno-owned system prompt.
   */
   ...
   * rewrites after Juno has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Juno handles
   ...
   *
   * Juno uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `juno doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `juno doctor` should offer.
   *
   ...
   *
   * Juno calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Juno's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Juno falls back to that stored profile.
   */
...
export type JunoPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Juno session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Juno configuration */
  config: JunoConfig;
 ...
 */
export type JunoPluginCommandDefinition = {
...
export type JunoPluginHttpRouteAuth = "gateway" | "plugin";
export type JunoPluginHttpRouteMatch = "exact" | "prefix";
export type JunoPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type JunoPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type JunoPluginHttpRouteParams = {
  path: string;
  handler: JunoPluginHttpRouteHandler;
  auth: JunoPluginHttpRouteAuth;
  match?: JunoPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: JunoPluginGatewayRuntimeScopeSurface;
  replaceExisting?: boolean;
};

export type JunoPluginCliContext = {
  program: Command;
  config: JunoConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type JunoPluginCliRegistrar = (ctx: JunoPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Juno to keep a plugin command lazy-loaded while still
 ...
 */
export type JunoPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type JunoPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type JunoPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type JunoPluginSecurityAuditContext = {
  config: JunoConfig;
...
export type JunoPluginSecurityAuditCollector = (
  ctx: JunoPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type JunoPluginServiceContext = {
  config: JunoConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type JunoPluginService = {
  id: string;
  start: (ctx: JunoPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: JunoPluginServiceContext) => void | Promise<void>;
};

export type JunoPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type JunoPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind | PluginKind[];
  configSchema?: JunoPluginConfigSchema;
  reload?: JunoPluginReloadRegistration;
  nodeHostCommands?: JunoPluginNodeHostCommand[];
  securityAuditCollectors?: JunoPluginSecurityAuditCollector[];
  register?: (api: JunoPluginApi) => void;
  activate?: (api: JunoPluginApi) => void;
};

export type JunoPluginModule = JunoPluginDefinition | ((api: JunoPluginApi) => void);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: JunoConfig) =>
  | {
      config: JunoConfig;
      changes: string[];
    }
...
export type JunoPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | JunoPluginToolFactory,
    opts?: JunoPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: JunoPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: JunoPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: JunoPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: JunoPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Juno can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: JunoPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: JunoPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: JunoPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: JunoPluginSecurityAuditCollector) => void;
  registerService: (service: JunoPluginService) => void;
   ...
   */
  registerCommand: (command: JunoPluginCommandDefinition) => void;
