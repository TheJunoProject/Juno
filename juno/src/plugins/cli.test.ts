...
const mocks = vi.hoisted(() => ({
  ...
  loadJunoPluginCliRegistry: vi.fn(),
  loadJunoPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadJunoPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadJunoPluginCliRegistry(...args),
  loadJunoPlugins: (...args: unknown[]) => mocks.loadJunoPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: JunoConfig;
  autoEnabledConfig: JunoConfig;
  ...
  expect(mocks.loadJunoPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadJunoPluginCliRegistry.mockReset();
    mocks.loadJunoPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadJunoPlugins.mockReset();
    mocks.loadJunoPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as JunoConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as JunoConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as JunoConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadJunoPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadJunoPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadJunoPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as JunoConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as JunoConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as JunoConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadJunoPlugins).not.toHaveBeenCalled();
  });
});
