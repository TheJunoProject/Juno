...
describe("startLazyPluginServiceModule", () => {
  afterEach(() => {
    delete process.env.JUNO_LAZY_SERVICE_SKIP;
    delete process.env.JUNO_LAZY_SERVICE_OVERRIDE;
  });
  ...
  it("uses the override module when configured", async () => {
    process.env.JUNO_LAZY_SERVICE_OVERRIDE = "virtual:service";
    const start = createAsyncHookMock();
    const loadOverrideModule = vi.fn(async () => ({ startOverride: start }));

    await expectLifecycleStarted({
      overrideEnvVar: "JUNO_LAZY_SERVICE_OVERRIDE",
  ...
  it("surfaces override validation failures", async () => {
    process.env.JUNO_LAZY_SERVICE_OVERRIDE = "data:text/javascript,boom";
