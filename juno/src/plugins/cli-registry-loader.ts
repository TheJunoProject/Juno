...
export type PluginCliCommandGroupEntry = {
  pluginId: string;
  placeholders: readonly JunoPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: JunoPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadJunoPluginCliRegistry(
...
): Promise<JunoPluginCliCommandDescriptor[]> {
  try {
