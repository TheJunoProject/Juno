...
import { resolveJunoPackageRootSync } from "../infra/juno-root.js";
...
function hasTrustedJunoRootIndicator(params: {
  packageRoot: string;
  ...
  const hasJunoBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("juno")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.juno === "string");
  const hasJunoEntrypoint = fs.existsSync(path.join(params.packageRoot, "juno.mjs"));
  return hasCliEntryExport || hasJunoBin || hasJunoEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedJunoRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedJunoRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveJunoPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedJunoRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveJunoPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedJunoRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["juno/plugin-sdk", "@juno/plugin-sdk"] as const;
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.JUNO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `juno/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "juno/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.openJunoPackageRoot, "extensions"),
    path.join(params.openJunoPackageRoot, "dist", "extensions"),
    path.join(params.openJunoPackageRoot, "dist-runtime", "extensions"),
