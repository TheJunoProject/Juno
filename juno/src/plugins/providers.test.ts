...
type LoadJunoPlugins = typeof import("./loader.js").loadJunoPlugins;
...
const loadJunoPluginsMock = vi.fn<LoadJunoPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/juno.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadJunoPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: JunoConfig = {
    plugins: {},
  };
  const autoEnabledConfig: JunoConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadJunoPlugins: (...args: Parameters<LoadJunoPlugins>) =>
        loadJunoPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadJunoPluginsMock.mockReturnValue(registry);
