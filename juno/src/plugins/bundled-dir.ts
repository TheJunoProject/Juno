...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "juno-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.JUNO_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "juno.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.JUNO_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveJunoPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveJunoPackageRootSync({ argv1: process.argv[1] }),
      resolveJunoPackageRootSync({ cwd: process.cwd() }),
      resolveJunoPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
