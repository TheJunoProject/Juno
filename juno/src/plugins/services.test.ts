...
import type { JunoPluginService, JunoPluginServiceContext } from "./types.js";
...
function createRegistry(services: JunoPluginService[]) {
...
function expectServiceLogger(ctx: JunoPluginServiceContext) {
...
function expectServiceContexts(
  contexts: JunoPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: JunoPluginServiceContext[] = [];
