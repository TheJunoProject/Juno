...
import { loadJunoPluginCliRegistry, loadJunoPlugins } from "./loader.js";
import {
...
describe("plugin loader CLI metadata", () => {
  ...
  register(api) {
    ...
    const registry = await loadJunoPluginCliRegistry({
      env: { ...process.env, JUNO_STATE_DIR: stateDir },
      logger: {
        info: () => {},
  ...
  it("uses the real channel entry in cli-metadata mode for CLI metadata capture", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@juno/cli-metadata-channel",
          juno: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
        },
        null,
        2,
      ),
...
module.exports = {
  ...
  it("skips bundled channel full entries that do not provide a dedicated cli-metadata entry", async () => {
    ...
    process.env.JUNO_BUNDLED_PLUGINS_DIR = bundledRoot;

    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify(
        {
          name: "@juno/bundled-skip-channel",
          juno: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
...
module.exports = {
  ...
  it("skips bundled non-channel full entries that do not provide a dedicated cli-metadata entry", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@juno/bundled-skip-provider",
          juno: { extensions: ["./index.cjs"] },
        },
        null,
        2,
...
module.exports = {
  ...
  it("collects channel CLI metadata during full plugin loads", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@juno/full-cli-metadata-channel",
          juno: { extensions: ["./index.cjs"] },
        },
        null,
        2,
