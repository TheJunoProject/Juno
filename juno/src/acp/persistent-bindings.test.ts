...
type ConfiguredBinding = NonNullable<JunoConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: JunoConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/juno" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
