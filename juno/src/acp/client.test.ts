...
const createTempDir = () => tempDirs.make("juno-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets JUNO_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "juno",
    });

    expect(env.JUNO_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("juno");
  });

  it("overrides pre-existing JUNO_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      JUNO_SHELL: "wrong",
    });
    expect(env.JUNO_SHELL).toBe("acp-client");
  ...
  it("preserves JUNO_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        JUNO_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["JUNO_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Juno bridge", () => {
    ...
    expect(env.JUNO_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.JUNO_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("JUNO_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "juno", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "juno", "dist", "entry.js");
    const shimPath = path.join(dir, "juno.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\juno\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/juno-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/juno-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/juno-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
