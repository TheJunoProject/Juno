...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "juno-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "juno", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "juno-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "juno", version: "2.3.4" });
  ...
  it("ignores non-juno package and blank build-info versions", async () => {
    await withTempDir({ prefix: "juno-version-" }, async (root) => {
  ...
  it("prefers JUNO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        JUNO_VERSION: "9.9.9",
        JUNO_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale JUNO_VERSION for compatibility checks", () => {
    const previous = process.env.JUNO_VERSION;
    const previousService = process.env.JUNO_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.JUNO_VERSION = "2026.3.25";
      process.env.JUNO_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.JUNO_VERSION = previous;
      process.env.JUNO_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        JUNO_VERSION: "2026.3.99",
        JUNO_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        JUNO_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        JUNO_VERSION: "2026.3.99",
        JUNO_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
