...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["JUNO_CONTAINER_HINT", "JUNO_PROFILE"]);
    delete process.env.JUNO_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:juno|juno) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
