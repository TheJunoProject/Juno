...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["JUNO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["JUNO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["JUNO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.JUNO_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Juno version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __JUNO_VERSION__ === "string" ? __JUNO_VERSION__ : undefined,
  bundledVersion: process.env.JUNO_BUNDLED_VERSION,
});
