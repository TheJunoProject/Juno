...
function setConfiguredFetchProviderKey(configTarget: JunoConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: JunoConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: JunoConfig, provider: ProviderUnderTest): unknown {
