...
export function buildConfigureCandidates(config: JunoConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: JunoConfig;
  authoredJunoConfig?: JunoConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredJunoConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const junoCandidates = discoverConfigSecretTargets(params.config)
  ...
  return [...junoCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "juno secrets configure",
