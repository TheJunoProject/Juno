...
let compiledSecretTargetRegistryState: {
  ...
  openJunoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openJunoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreJunoTargetState: {
  knownTargetIds: Set<string>;
  openJunoCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openJunoTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelJunoTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const openJunoCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "juno.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    openJunoCompiledSecretTargets,
    openJunoTargetsById: buildConfigTargetIdIndex(openJunoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreJunoTargetState() {
  if (compiledCoreJunoTargetState) {
    return compiledCoreJunoTargetState;
  }
  const openJunoCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "juno.json")
    .map(compileTargetRegistryEntry);
  compiledCoreJunoTargetState = {
    knownTargetIds: new Set(openJunoCompiledSecretTargets.map((entry) => entry.id)),
    openJunoCompiledSecretTargets,
    openJunoTargetsById: buildConfigTargetIdIndex(openJunoCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(openJunoCompiledSecretTargets),
  };
  return compiledCoreJunoTargetState;
}

function getCompiledBundledChannelJunoTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelJunoTargets.has(normalizedChannelId)) {
    return compiledBundledChannelJunoTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "juno.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelJunoTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreJunoTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreJunoTargetState().openJunoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelJunoTargets(explicitBundledChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().openJunoCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreJunoTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreJunoTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.openJunoCompiledSecretTargets,
    entriesById: registryState.openJunoTargetsById,
  });
