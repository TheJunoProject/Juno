...
type SecretRegistryEntry = {
  id: string;
  configFile: "juno.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.JUNO_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_JUNO_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForJunoTarget(
  config: JunoConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every juno.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "juno.json" &&
        !PLUGIN_OWNED_JUNO_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("juno.json", batch);
      const config = {} as JunoConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `JUNO_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForJunoTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `JUNO_AUTH_SECRET_TARGET_${entry.id}`;
