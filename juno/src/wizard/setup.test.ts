...
import type { ProviderPlugin } from "juno/plugin-sdk/provider-model-shared";
...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.JUNO_GATEWAY_PORT ?? process.env.JUNO_GATEWAY_PORT;
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    const previous = process.env.JUNO_GATEWAY_PASSWORD;
    process.env.JUNO_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.JUNO_GATEWAY_PASSWORD = previous;
      }
    }
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.JUNO_GATEWAY_PORT;
    process.env.JUNO_GATEWAY_PORT = "18791";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.JUNO_GATEWAY_PORT = previousPort;
      }
    }
