...
const fixtureRoot = path.join(process.cwd(), ".tmp", `juno-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@juno/test-hooks",
      version: "0.0.1",
      juno: { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "juno.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "juno.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  ...
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.hookPackId).toBe("test-hooks");
    expect(result.npmResolution?.resolvedSpec).toBe("@juno/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@juno/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@juno/test-hooks@0.0.2-beta.1",
      name: "@juno/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@juno/test-hooks@beta"');
    }
  });
});
