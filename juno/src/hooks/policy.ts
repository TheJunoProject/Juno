import type { JunoConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "juno-bundled": {
    ...
    canOverride: ["juno-bundled"],
    canBeOverriddenBy: ["juno-managed", "juno-plugin"],
  },
  "juno-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["juno-bundled", "juno-plugin"],
    canBeOverriddenBy: ["juno-managed"],
  },
  "juno-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["juno-bundled", "juno-managed", "juno-plugin"],
    canBeOverriddenBy: ["juno-managed"],
  },
  "juno-workspace": {
    precedence: 40,
    ...
    canOverride: ["juno-workspace"],
    canBeOverriddenBy: ["juno-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "juno-plugin") {
    return { enabled: true };
  }
