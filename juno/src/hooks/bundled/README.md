...
This directory contains hooks that ship with Juno. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.juno/workspace`)
...
homepage: https://docs.juno.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.juno/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.juno/juno.json`:
...
2. Restart gateway: `pkill -9 -f 'juno.*gateway' && pnpm juno gateway`
3. Enable the hook: `juno hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.juno.ai/automation/hooks
