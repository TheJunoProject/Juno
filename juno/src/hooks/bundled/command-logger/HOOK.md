...
homepage: https://docs.juno.ai/automation/hooks#command-logger
metadata:
  {
    "juno":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.juno/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.juno/logs/commands.log ~/.juno/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/juno`:
   ```
   /home/username/.juno/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
