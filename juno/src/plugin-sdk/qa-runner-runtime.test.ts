...
const resolveJunoPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.JUNO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveJunoPackageRootSync.mockReset().mockReturnValue(null);
    delete process.env.JUNO_ENABLE_PRIVATE_QA_CLI;
  });
  ...
  afterEach(() => {
    ...
    if (originalPrivateQaCli === undefined) {
      ...
      process.env.JUNO_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
    }
  });
  ...
  it("uses the source bundled tree for qa-lab runtime loading in private qa mode", async () => {
    const sourceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-qa-runtime-root-"));
    ...
    process.env.JUNO_ENABLE_PRIVATE_QA_CLI = "1";
    resolveJunoPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
      ...
      env: expect.objectContaining({
        JUNO_ENABLE_PRIVATE_QA_CLI: "1",
        JUNO_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
      }),
    });
  });

  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-qa-test-api-root-"));
    tempDirs.push(sourceRoot);
    fs.mkdirSync(path.join(sourceRoot, "src"), { recursive: true });
    fs.mkdirSync(path.join(sourceRoot, "extensions"), { recursive: true });
    fs.writeFileSync(path.join(sourceRoot, ".git"), "gitdir: /tmp/mock\n", "utf8");
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-qa-runner-root-"));
    tempDirs.push(sourceRoot);
    fs.mkdirSync(path.join(sourceRoot, "src"), { recursive: true });
    fs.mkdirSync(path.join(sourceRoot, "extensions"), { recursive: true });
    fs.writeFileSync(path.join(sourceRoot, ".git"), "gitdir: /tmp/mock\n", "utf8");
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in juno.plugin.json',
    );
  });
});
