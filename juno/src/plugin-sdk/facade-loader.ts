...
let cachedJunoPackageRoot: string | undefined;
...
function getJunoPackageRoot() {
  if (cachedJunoPackageRoot) {
    return cachedJunoPackageRoot;
  }
  cachedJunoPackageRoot =
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getJunoPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      params.location.boundaryRoot === getJunoPackageRoot()
        ? "Juno package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getJunoPackageRoot() ? "Juno package root" : "plugin root",
    rejectHardlinks: false,
  });
  if (!opened.ok) {
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedJunoPackageRoot = undefined;
}
