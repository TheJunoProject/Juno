...
function isCurrentJunoClient(clientId: string | undefined): boolean {
  const normalized = normalizeOptionalLowercaseString(clientId) ?? "";
  return normalized.startsWith("juno-");
}
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentJunoClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
  const legacyCount = matches.filter((match) => isLegacyJunodbotClient(match.clientId)).length;
...
function scoreNodeCandidate(node: NodeMatchCandidate, matchScore: number): number {
  ...
  if (isCurrentJunoClient(node.clientId)) {
    score += 10;
  } else if (isLegacyJunodbotClient(node.clientId)) {
    score -= 10;
  }
  return score;
}
