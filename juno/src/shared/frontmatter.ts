...
export function resolveJunoManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type JunoManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveJunoManifestRequires(
  metadataObj: Record<string, unknown>,
): JunoManifestRequires | undefined {
  const requiresRaw =
...
export function resolveJunoManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveJunoManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedJunoManifestInstallBase = {
...
export function parseJunoManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedJunoManifestInstallBase | undefined {
  ...
  const spec: ParsedJunoManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyJunoManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedJunoManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
