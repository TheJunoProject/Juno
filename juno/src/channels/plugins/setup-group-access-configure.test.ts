...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: JunoConfig, policy: ChannelAccessPolicy) => JunoConfig;
  resolveAllowlist: (params: { cfg: JunoConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: JunoConfig; resolved: TResolved }) => JunoConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: JunoConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: JunoConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: JunoConfig, policy: ChannelAccessPolicy): JunoConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: JunoConfig, policy: ChannelAccessPolicy): JunoConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: JunoConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: JunoConfig; resolved: string[] }) => {
      calls.push("apply");
