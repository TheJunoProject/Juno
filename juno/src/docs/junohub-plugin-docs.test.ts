...
const pluginDocs = [
  path.join(DOCS_ROOT, "tools", "junohub.md"),
...
describe("JunoHub plugin docs", () => {
  it("keeps the canonical plugin-publish snippets contract-valid", async () => {
    ...
    const pluginManifest = JSON.parse(
      await fs.readFile(
        path.join(DOCS_ROOT, "snippets", "plugin-publish", "minimal-juno.plugin.json"),
        "utf8",
      ),
  ...
  it("does not tell plugin authors to use bare junohub publish", async () => {
    for (const docPath of pluginDocs) {
      const markdown = await fs.readFile(docPath, "utf8");
      expect(markdown).not.toMatch(/(^|[\s`])junohub publish\b/);
    }
  });
  ...
  it("keeps the canonical package snippet embedded in the primary plugin docs", async () => {
    ...
    expect(extractNamedJsonBlock(sdkSetup, "juno-junohub-package.json")).toEqual(snippet);
  });
});
