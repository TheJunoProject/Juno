...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[juno proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[juno proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`juno proxy coverage\` for details.`,
  );
}
