...
const TEMP_MEDIA_PREFIX = "juno-media-";
...
function createGroqAudioConfig(): JunoConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): JunoConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): JunoConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: JunoConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredJunoTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        JUNO_AGENT_DIR: isolatedAgentDir,
