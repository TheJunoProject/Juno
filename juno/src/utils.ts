...
export function resolveConfigDir(
  ...
  const configPath = env.JUNO_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".juno");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.JUNO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$JUNO_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via JUNO_STATE_DIR.
