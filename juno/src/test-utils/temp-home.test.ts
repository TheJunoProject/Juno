...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const tempHome = await createTempHomeEnv("juno-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.JUNO_STATE_DIR).toBe(path.join(tempHome.home, ".juno"));
    ...
    expect(process.env.JUNO_STATE_DIR).toBe(previousStateDir);
