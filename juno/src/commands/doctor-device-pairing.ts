...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["juno", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["juno", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["juno", "devices", "remove", pending.deviceId]),
    };
  }
