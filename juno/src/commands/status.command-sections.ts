...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("juno security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("juno security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.juno.ai/faq",
    "Troubleshooting: https://docs.juno.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("juno status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("juno logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("juno node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("juno status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("juno gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`juno devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("juno devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("juno devices list")}`),
  ];
}
