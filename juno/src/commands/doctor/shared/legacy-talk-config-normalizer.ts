...
export function normalizeLegacyTalkConfig(cfg: JunoConfig, changes: string[]): JunoConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as JunoConfig["talk"]) ?? {};
