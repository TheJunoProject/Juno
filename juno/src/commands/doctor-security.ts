...
import type { JunoConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: JunoConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: JunoConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Juno default (full)";
  const defaultRequestedAskSource = "Juno default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("juno approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: JunoConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: JunoConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("juno security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.juno/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("juno approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.juno.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("juno configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("juno config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("juno doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("juno config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('juno config set session.dmScope "per-channel-peer"') +
