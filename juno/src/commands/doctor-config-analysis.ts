...
import { JunoSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: JunoConfig): {
  config: JunoConfig;
  removed: string[];
} {
  const parsed = JunoSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: JunoConfig): void {
