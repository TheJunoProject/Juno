...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.juno", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.juno", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/juno-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/juno/.juno",
    });

    expect(result).toEqual({
      path: "/mnt/slow/juno/.juno",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".juno"), {
      platform: "darwin",
      mountInfo,
    });
