...
vi.mock("../agents/agent-paths.js", () => ({
  resolveJunoAgentDir: () => process.env.JUNO_AGENT_DIR ?? "/tmp/juno-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.JUNO_STATE_DIR ?? "/tmp/juno-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.JUNO_AGENT_DIR ?? "/tmp/juno-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under JUNO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("juno-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-oauth-sync-"));
    ...
    process.env.JUNO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-oauth-nosync-"));
    process.env.JUNO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside JUNO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-oauth-external-"));
    process.env.JUNO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* JUNO_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("juno-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("juno-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to JUNO_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("juno-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
