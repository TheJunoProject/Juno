...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/juno-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = JunoConfig>(
  overrides: T = {} as T,
): JunoConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): JunoConfig {
