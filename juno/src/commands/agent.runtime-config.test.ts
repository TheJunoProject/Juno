...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: JunoConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => JunoConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: JunoConfig, sourceConfig: JunoConfig) => void>(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "juno-agent-" });
}

function mockConfig(home: string, storePath: string): JunoConfig {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteMock.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as JunoConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand runtime config", () => {
  ...
  it("includes channel secret targets when delivery is requested", async () => {
    await withTempHome(async (home) => {
      ...
      } as unknown as JunoConfig["channels"];
      resolveCommandConfigWithSecretsMock.mockResolvedValueOnce({
