...
export const resolveJunoPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadJunoPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/juno-root.js", () => ({
  ...
  resolveJunoPackageRootSync: vi.fn(() => "/tmp/juno"),
}));
...
beforeEach(() => {
  ...
  resolveJunoPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.JUNO_STATE_DIR;
  originalUpdateInProgress = process.env.JUNO_UPDATE_IN_PROGRESS;
  process.env.JUNO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-doctor-state-"));
