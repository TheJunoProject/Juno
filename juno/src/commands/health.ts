...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.JUNO_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: JunoConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: JunoConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: JunoConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: JunoConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.JUNO_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<JunoConfig> {
