...
export function hasExplicitMemorySearchConfig(cfg: JunoConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: JunoConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: JunoConfig, agentId: string) => { store: { path: string } } | null;
