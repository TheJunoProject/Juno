...
beforeAll(() => {
  envSnapshot = captureEnv(["JUNO_PROFILE"]);
  process.env.JUNO_PROFILE = "isolated";
});
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/juno/pnpm-lock.yaml",
        markerPath: "/tmp/juno/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.JUNO_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.juno.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.juno.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.JUNO_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/juno-root.js", () => ({
  resolveJunoPackageRoot: vi.fn().mockResolvedValue("/tmp/juno"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("juno status --all") ||
          line.includes("juno --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("juno --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("JUNO_GATEWAY_TOKEN", "abcd1234", async () => {
