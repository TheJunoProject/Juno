...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveJunoAgentDir: vi.fn().mockReturnValue("/tmp/juno-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/juno-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/juno-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/juno-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/juno-dev/juno.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveJunoAgentDir: mocks.resolveJunoAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/juno-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveJunoAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/juno-dev/juno.json");
    expect(payload.auth.storePath).toBe("/tmp/juno-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/juno-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/juno-agent-custom");
