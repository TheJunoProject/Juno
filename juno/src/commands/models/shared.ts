...
export async function loadValidConfigOrThrow(): Promise<JunoConfig> {
...
export async function updateConfig(
  mutator: (cfg: JunoConfig) => JunoConfig,
): Promise<JunoConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: JunoConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: JunoConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("juno agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
