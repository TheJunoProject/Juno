...
const SEARCH_TOOL = "https://docs.juno.ai/mcp.SearchJuno";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.juno.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('juno docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.juno.ai/");
      runtime.log(`Search: ${formatCliCommand('juno docs "your query"')}`);
    }
    return;
  }
