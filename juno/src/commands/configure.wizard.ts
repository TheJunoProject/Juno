...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.JUNO_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.JUNO_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from juno.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<JunoConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Juno update wizard" : "Juno configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: JunoConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("juno doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.JUNO_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.JUNO_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
