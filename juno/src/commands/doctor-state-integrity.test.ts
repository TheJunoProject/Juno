...
type EnvSnapshot = {
  HOME?: string;
  JUNO_HOME?: string;
  JUNO_STATE_DIR?: string;
  JUNO_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    JUNO_HOME: process.env.JUNO_HOME,
    JUNO_STATE_DIR: process.env.JUNO_STATE_DIR,
    JUNO_OAUTH_DIR: process.env.JUNO_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: JunoConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("JUNO_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: JunoConfig) {
...
async function runStateIntegrityText(cfg: JunoConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "juno-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.JUNO_HOME = tempHome;
    process.env.JUNO_STATE_DIR = path.join(tempHome, ".juno");
    delete process.env.JUNO_OAUTH_DIR;
    fs.mkdirSync(process.env.JUNO_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when JUNO_OAUTH_DIR is explicitly configured", async () => {
    process.env.JUNO_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: JunoConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.JUNO_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints juno-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: JunoConfig = {};
    ...
    expect(text).toMatch(/juno sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/juno sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /juno sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
