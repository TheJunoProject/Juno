...
const ensureJunoModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveJunoAgentDir = vi.fn().mockReturnValue("/tmp/juno-agent");
...
beforeEach(() => {
  ...
  ensureJunoModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureJunoModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureJunoModelsJson).toHaveBeenCalled();
    expect(ensureJunoModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
