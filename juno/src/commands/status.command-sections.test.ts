...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:juno security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:juno security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:juno status --all",
      "  Need to debug live? cmd:juno logs --follow",
      "  Fix reachability first: cmd:juno gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:juno devices approve req-123)",
      "muted(Fallback: cmd:juno devices approve --latest)",
      "muted(Inspect: cmd:juno devices list)",
    ]);
  });
