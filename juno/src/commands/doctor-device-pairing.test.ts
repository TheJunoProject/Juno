...
describe("noteDevicePairingHealth", () => {
  ...
  it("warns about pending scope upgrades from local pairing state when the gateway is down", async () => {
    await withTempDir("juno-doctor-device-pairing-", async (stateDir) => {
      await withEnvAsync(
        {
        ...
        async () => {
          ...
          expect(message).toContain("juno devices approve");
    ...
    await withTempDir("juno-doctor-device-pairing-", async (stateDir) => {
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(message).toContain("juno devices rotate");
        },
      );
    });
  });
  ...
  it("quotes untrusted device pairing fields in suggested commands", async () => {
    ...
    const message = String(noteMock.mock.calls[0]?.[0] ?? "");
    expect(message).toContain("juno devices remove 'device; echo pwn'");
    expect(message).toContain(
      "juno devices rotate --device 'device; echo pwn' --role 'operator; touch /tmp/pwn'",
    );
  });
