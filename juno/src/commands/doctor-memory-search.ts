...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("juno doctor --fix")} or ${formatCliCommand("juno memory status --fix")}`
    : `Verify: ${formatCliCommand("juno memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: JunoConfig): Promise<void> {
  try {
 ...
 * Runs as part of `juno doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("juno config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("juno config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("juno memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`juno config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("juno configure --section model")}`,
        `- To disable: ${formatCliCommand("juno config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("juno memory status --deep")}`,
      ].join("\n"),
      "Memory search",
