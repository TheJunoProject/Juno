...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("juno status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("juno security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set JUNO_GATEWAY_TOKEN/JUNO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("juno security audit --deep")}".`,
    });
  }
  return findings;
}
