...
function summarizeGroupPolicy(cfg: JunoConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: JunoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: JunoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: JunoConfig): boolean {
...
function isBrowserEnabled(cfg: JunoConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: JunoConfig): SecurityAuditFinding[] {
