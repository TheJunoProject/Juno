...
import { z } from "zod";
import { createJunoChannelMcpServer, JunoChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createJunoChannelMcpServer({
...
describe("juno channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new JunoChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
