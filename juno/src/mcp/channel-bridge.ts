...
export class JunoChannelBridge {
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`juno mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `juno mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
