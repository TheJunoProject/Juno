...
vi.mock("./channel-bridge.js", () => ({
  JunoChannelBridge: class MockJunoChannelBridge {
...
describe("serveJunoChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveJunoChannelMcp } = await import("./channel-server.js");

    const servePromise = serveJunoChannelMcp({ verbose: false });
