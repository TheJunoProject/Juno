...
import type { JunoConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.juno.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: JunoConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: JunoConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: JunoConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = JunoSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as JunoConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: JunoConfig | null | undefined;
  ...
  const ensureCompatConfig = (): JunoConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Juno config schema.
