...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.JUNO_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveJunoPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    JUNO_STATE_DIR: path.join(os.tmpdir(), "juno-config-doc-baseline-state"),
    JUNO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
