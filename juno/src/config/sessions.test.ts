...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ JUNO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-juno",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-juno#general");
  });
  ...
  it("derives session transcripts dir from JUNO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { JUNO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when JUNO_STATE_DIR differs from stored paths", () => {
