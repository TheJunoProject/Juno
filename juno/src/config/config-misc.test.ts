...
import { buildWebSearchProviderConfig, withTempHome, writeJunoConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = JunoSchema.safeParse({
      $schema: "https://juno.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://juno.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = JunoSchema.safeParse({ $schema: 123 });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://juno.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@juno"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = JunoSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeJunoConfig(home, {
        gateway: { bind: "${JUNO_BIND}" },
      });

      const prev = process.env.JUNO_BIND;
      process.env.JUNO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.JUNO_BIND;
        } else {
          process.env.JUNO_BIND = prev;
        }
      }
    });
  });
