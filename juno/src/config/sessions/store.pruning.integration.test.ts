...
// Keep integration tests deterministic: never read a real juno.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "juno-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.JUNO_SESSION_CACHE_TTL_MS;
    process.env.JUNO_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.JUNO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.JUNO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
