...
import {
  compareJunoVersions,
  isSameJunoStableFamily,
  parseJunoVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseJunoVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseJunoVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseJunoVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseJunoVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseJunoVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseJunoVersion("2026.3")).toBeNull();
    expect(parseJunoVersion("latest")).toBeNull();
  });
});

describe("compareJunoVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareJunoVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareJunoVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareJunoVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareJunoVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareJunoVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareJunoVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameJunoStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameJunoStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameJunoStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameJunoStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameJunoStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
