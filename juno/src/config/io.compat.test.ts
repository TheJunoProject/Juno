...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "juno-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.juno/juno.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".juno", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.juno/juno.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".juno", "juno.json"));
    });
  });

  it("uses JUNO_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { JUNO_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".juno", "juno.json"));
    });
  });

  it("honors explicit JUNO_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".juno", 20002, "custom.json");
      const io = createIoForHome(home, { JUNO_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
