...
export type WideAreaDiscoveryConfig = {
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "juno.internal"). */
  domain?: string;
};
...
export type CanvasHostConfig = {
  enabled?: boolean;
  /** Directory to serve (default: ~/.juno/workspace/canvas). */
  root?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/juno"). */
  basePath?: string;
   ...
   * Default off; prefer hosted /__juno__/canvas or /__juno__/a2ui content.
   */
 ...
 * Used when Junodbot runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayReloadConfig = {
   ...
   * @see https://github.com/juno/juno/issues/47711
   */
