...
export function parseJunoVersion(raw: string | null | undefined): JunoVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeJunoVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseJunoVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameJunoStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseJunoVersion(a);
  const parsedB = parseJunoVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareJunoVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseJunoVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseJunoVersion(current);
  const parsedTouched = parseJunoVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameJunoStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareJunoVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: JunoVersion): number {
