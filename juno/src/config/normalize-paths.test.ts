  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.juno/logs/juno.log" },
        hooks: {
          path: "~/.juno/hooks.json5",
        ...
        channels: {
          ...
          whatsapp: {
            accounts: {
              personal: {
                authDir: "~/.juno/credentials/wa-personal",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".juno", "logs", "juno.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".juno", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".juno", "telegram.token"),
      );
      expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
        path.join(home, ".juno", "credentials", "wa-personal"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".juno", "agents", "main"));
