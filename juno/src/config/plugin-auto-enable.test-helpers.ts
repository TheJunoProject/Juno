...
export function makeTempDir(): string {
  return makeTrackedTempDir("juno-plugin-auto-enable", tempDirs);
}

export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const rootDir = makeTempDir();
  return {
    JUNO_STATE_DIR: path.join(rootDir, "state"),
    ...overrides,
  };
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "juno.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/juno.plugin.json`,
    })),
    diagnostics: [],
  };
}
