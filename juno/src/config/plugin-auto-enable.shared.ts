...
function collectModelRefs(cfg: JunoConfig): string[] {
  const refs: string[] = [];
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: JunoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: JunoConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: JunoConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: JunoConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: JunoConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: JunoConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: JunoConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: JunoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: JunoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: JunoConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: JunoConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: JunoConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: JunoConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: JunoConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithEntries(cfg: JunoConfig): boolean {
  return Array.isArray(cfg.plugins?.allow) && cfg.plugins.allow.length > 0 && hasPluginEntries(cfg);
}

function hasConfiguredProviderModelOrHarness(cfg: JunoConfig, env: NodeJS.ProcessEnv): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: JunoConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: JunoConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: JunoConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: JunoConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: JunoConfig, pluginId: string): JunoConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
