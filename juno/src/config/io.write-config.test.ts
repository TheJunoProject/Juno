...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "juno-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { JUNO_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("keeps writes inside an JUNO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".juno", "juno.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "juno.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "juno.json"), "utf-8"),
      ) as {
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".juno", "juno.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            $schema: "https://juno.ai/config.json",
            gateway: { mode: "local" },
          },
          null,
          2,
      ...
      expect(persisted.$schema).toBe("https://juno.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/juno-main" }] },
        tools: { profile: "safe" },
        commands: { ownerDisplay: "hash" },
      };
  ...
  it("does not inject include-only $schema into the root config during partial writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".juno", "juno.json");
      const includePath = path.join(home, ".juno", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://juno.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/juno-test-required-plugin",
          source: "/tmp/juno-test-required-plugin/index.ts",
          manifestPath: "/tmp/juno-test-required-plugin/juno.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
