...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.juno) || !isRecord(entry.juno.channel)) {
      continue;
    }
    const channel = entry.juno.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: JunoConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: JunoConfig, pluginId: string) => boolean;
