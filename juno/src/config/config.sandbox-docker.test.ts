...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("juno-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "juno-sandbox-browser-global" },
      agentBrowser: { network: "juno-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("juno-sandbox-browser-agent");
  });
