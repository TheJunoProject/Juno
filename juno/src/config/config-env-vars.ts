...
function collectConfigEnvVarsByTarget(cfg?: JunoConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: JunoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: JunoConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: JunoConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.JUNO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
