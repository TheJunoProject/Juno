...
import type { ConfigFileSnapshot } from "./types.juno.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/juno/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/juno/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/juno/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/juno/nickserv-password.txt");
