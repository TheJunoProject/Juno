...
import type { ConfigFileSnapshot, JunoConfig } from "./types.js";

const mockLoadConfig = vi.hoisted(() => vi.fn<() => JunoConfig>());
...
function makeSnapshot(params: { valid: boolean; config?: JunoConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/juno.json",
    exists: true,
    raw: "{}",
