...
const { makeStorePath } = createCronStoreHarness({ prefix: "juno-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses JUNO_HOME for tilde expansion", () => {
    vi.stubEnv("JUNO_HOME", "/srv/juno-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/juno-home", "cron", "jobs.json"));
  });
});
