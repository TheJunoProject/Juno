...
export function isPrivateQaCliEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.JUNO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
function resolvePrivateQaSourceModuleSpecifier(params?: {
  ...
  argv1?: string;
  moduleUrl?: string;
  resolvePackageRootSync?: typeof resolveJunoPackageRootSync;
  ...
  const resolvePackageRootSync = params?.resolvePackageRootSync ?? resolveJunoPackageRootSync;
...
export function loadPrivateQaCliModule(params?: {
  ...
  if (!specifier) {
    throw new Error("Private QA CLI is only available from an Juno source checkout.");
  }
