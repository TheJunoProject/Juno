...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno status", "Show channel health + session summary."],
          ["juno status --all", "Full diagnosis (read-only)."],
          ["juno status --json", "Machine-readable output."],
          ["juno status --usage", "Show model provider usage/quota snapshots."],
          [
            "juno status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["juno status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.juno.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.juno.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno sessions", "List all sessions."],
          ["juno sessions --agent work", "List sessions for one agent."],
          ["juno sessions --all-agents", "Aggregate sessions across agents."],
          ["juno sessions --active 120", "Only last 2 hours."],
          ["juno sessions --json", "Machine-readable output."],
          ["juno sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.juno.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "juno sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["juno sessions cleanup --enforce", "Apply maintenance now."],
          ["juno sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["juno sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "juno sessions cleanup --enforce --store ./tmp/sessions.json",
