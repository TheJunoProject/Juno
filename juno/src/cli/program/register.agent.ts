...
${formatHelpExamples([
  ['juno agent --to +15555550123 --message "status update"', "Start a new session."],
  ['juno agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'juno agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'juno agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['juno agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'juno agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.juno.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.juno.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "juno agents set-identity --identity-file ~/.juno/workspace/IDENTITY.md --agent main",
