...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: JunoConfig) => void }) => {
      ...
      return {
        path: "/tmp/juno.json",
        previousHash: "hash-1",
        snapshot: { path: "/tmp/juno.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: JunoConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: JunoConfig }>
    >(async () => ({
      path: "/tmp/juno.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: JunoConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/juno.json");
    ...
    expect(output).not.toContain("/tmp/juno.json\nforged");
