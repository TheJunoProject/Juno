...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.juno${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.JUNO_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.JUNO_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.JUNO_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.JUNO_CONFIG_PATH)) {
    env.JUNO_CONFIG_PATH = path.join(stateDir, "juno.json");
  }

  if (profile === "dev" && !env.JUNO_GATEWAY_PORT?.trim()) {
    env.JUNO_GATEWAY_PORT = "19001";
  }
}
