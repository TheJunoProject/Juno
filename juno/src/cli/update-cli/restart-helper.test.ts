  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'juno-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses JUNO_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "default",
        JUNO_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.juno.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.juno.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("captures macOS launchctl stderr to ~/.juno/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.juno/logs/gateway-restart.log' 2>&1");
    ...
    it("uses JUNO_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/juno-state/logs' 2>/dev/null && : >>'/tmp/juno-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/juno-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
    ...
    it("returns the final macOS launchctl kickstart failure after logging cleanup", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      const tmpDir = await makeTempDir("juno-restart-helper-");
...
`,
      ...
      expect(log).toContain("juno restart attempt source=update target=ai.juno.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.juno.gateway");
      expect(log).toContain("juno restart failed source=update status=42");
      expect(log).not.toContain("juno restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        JUNO_LAUNCHD_LABEL: "ai.juno.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        JUNO_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.juno.$(echo injected)");
      expect(log).not.toContain("target=ai.juno.injected");
    });

    it("uses JUNO_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "default",
        JUNO_LAUNCHD_LABEL: "com.custom.juno",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.juno'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("juno restart attempt source=update target=Juno Gateway");
      expect(content).toContain('schtasks /End /TN "Juno Gateway"');
      expect(content).toContain('schtasks /Run /TN "Juno Gateway" >>');
    ...
    it("uses JUNO_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "default",
        JUNO_WINDOWS_TASK_NAME: "Juno Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "Juno Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Juno Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "production",
      });
      expect(content).toContain("juno-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        JUNO_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.juno.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Juno Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        JUNO_LAUNCHD_LABEL: "ai.juno.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.juno.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
