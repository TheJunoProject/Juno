...
import { JUNOHUB_INSTALL_ERROR_CODE } from "../plugins/junohub.js";
...
export function applySlotSelectionForPlugin(
  config: JunoConfig,
  pluginId: string,
): { config: JunoConfig; warnings: string[] } {
...
export function buildPreferredJunoHubSpec(raw: string): string | null {
  const parsed = parseRegistryNpmSpec(raw);
  if (!parsed) {
    return null;
  }
  return `junohub:${parsed.name}${parsed.selector ? `@${parsed.selector}` : ""}`;
}

export const PREFERRED_JUNOHUB_FALLBACK_DECISION = {
  FALLBACK_TO_NPM: "fallback_to_npm",
  STOP: "stop",
} as const;

export type PreferredJunoHubFallbackDecision =
  (typeof PREFERRED_JUNOHUB_FALLBACK_DECISION)[keyof typeof PREFERRED_JUNOHUB_FALLBACK_DECISION];

export function decidePreferredJunoHubFallback(params: {
  code?: string;
}): PreferredJunoHubFallbackDecision {
  if (
    params.code === JUNOHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    params.code === JUNOHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND
  ) {
    return PREFERRED_JUNOHUB_FALLBACK_DECISION.FALLBACK_TO_NPM;
  }
  return PREFERRED_JUNOHUB_FALLBACK_DECISION.STOP;
}
