...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Juno plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.juno.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "juno"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as JunoConfig;
      const enableResult = enablePluginInConfig(cfg, id);
      let next: JunoConfig = enableResult.config;
  ...
  plugins
    .command("install")
    .description(
      "Install a plugin or hook pack (path, archive, npm spec, junohub:package, or marketplace entry)",
    )
    .argument(
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.juno.ai/plugin");
      lines.push("");
