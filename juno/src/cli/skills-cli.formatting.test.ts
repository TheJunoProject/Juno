...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-bundled-skills-test-"));
    process.env.JUNO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
