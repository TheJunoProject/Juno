...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Juno Completion";
}
 ...
 * Returns true if profile has `source <(juno completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "juno") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
