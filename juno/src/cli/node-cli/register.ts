    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "juno node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["juno node status", "Check node host service status."],
          ["juno node install", "Install the node host service."],
          ["juno node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.juno.ai/cli/node")}\n`,
    );
