...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno security audit", "Run a local security audit."],
          ["juno security audit --deep", "Include best-effort live Gateway probe checks."],
          ["juno security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "juno security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["juno security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["juno security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.juno.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Juno security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("juno security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("juno security audit --fix")}`));
        if (!fixResult) {
