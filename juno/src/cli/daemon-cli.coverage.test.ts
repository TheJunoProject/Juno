...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      JUNO_GATEWAY_PORT: String(params.port),
      ...(params.token ? { JUNO_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.JUNO_STATE_DIR = "/tmp/juno-cli-state";
    process.env.JUNO_CONFIG_PATH = "/tmp/juno-cli-state/juno.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        JUNO_PROFILE: "dev",
        JUNO_STATE_DIR: "/tmp/juno-daemon-state",
        JUNO_CONFIG_PATH: "/tmp/juno-daemon-state/juno.json",
        JUNO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.juno.gateway.plist",
    });
