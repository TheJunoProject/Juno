      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["juno gateway run", "Run the gateway in the foreground."],
            ["juno gateway status", "Show service status plus connectivity/capability."],
            ["juno gateway discover", "Find local and wide-area gateway beacons."],
            ["juno gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.juno.ai/cli/gateway")}\n`,
      ),
  );
