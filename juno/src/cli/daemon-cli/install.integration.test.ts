...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("juno-daemon-install-int-");
    configPath = path.join(tempHome, "juno.json");
    process.env.HOME = tempHome;
    process.env.JUNO_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.JUNO_GATEWAY_TOKEN).toBeUndefined();
  });
});
