...
import {
  installSkillFromJunoHub,
  readTrackedJunoHubSkillSlugs,
  searchSkillsFromJunoHub,
  updateSkillsFromJunoHub,
} from "../agents/skills-junohub.js";
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.juno.ai/cli/skills")}\n`,
    );

  skills
    .command("search")
    .description("Search JunoHub skills")
    .argument("[query...]", "Optional search query")
    .option("--limit <n>", "Max results", (value) => Number.parseInt(value, 10))
    .option("--json", "Output as JSON", false)
    .action(async (queryParts: string[], opts: { limit?: number; json?: boolean }) => {
      try {
        const results = await searchSkillsFromJunoHub({
        ...
        if (results.length === 0) {
          defaultRuntime.log("No JunoHub skills found.");
          return;
        }
  ...
  skills
    .command("install")
    .description("Install a skill from JunoHub into the active workspace")
    .argument("<slug>", "JunoHub skill slug")
  ...
  skills
    .command("update")
    .description("Update JunoHub-installed skills in the active workspace")
    .argument("[slug]", "Single skill slug")
    .option("--all", "Update all tracked JunoHub skills", false)
    .action(async (slug: string | undefined, opts: { all?: boolean }) => {
      try {
        ...
        const workspaceDir = resolveActiveWorkspaceDir();
        const tracked = await readTrackedJunoHubSkillSlugs(workspaceDir);
        if (opts.all && tracked.length === 0) {
          defaultRuntime.log("No tracked JunoHub skills to update.");
          return;
        }
