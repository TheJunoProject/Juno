    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno nodes status", "List known nodes with live status."],
          ["juno nodes pairing pending", "Show pending node pairing requests."],
          [
            'juno nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["juno nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.juno.ai/cli/nodes")}\n`,
    );
