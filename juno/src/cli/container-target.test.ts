...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "juno", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "juno", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "juno", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "juno", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "juno", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "juno", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "juno", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "juno", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "juno", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to JUNO_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "juno", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "juno", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "juno", "status"], {
        JUNO_CONTAINER: "demo",
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "juno", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "juno", "status"],
    });
  });

  it("uses JUNO_CONTAINER when the flag is absent", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "juno", "status"], {
        env: { JUNO_CONTAINER: "demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("clears inherited host routing and gateway env before execing into the child CLI", () => {
    ...
    maybeRunCliInContainer(["node", "juno", "status"], {
      env: {
        ...
        JUNO_GATEWAY_PASSWORD: "password",
      } as NodeJS.ProcessEnv,
      spawnSync,
    });
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "juno", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "juno", "--container", "demo", "health"], {
        env: { USER: "juno" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("checks docker after podman and before failing", () => {
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "somalley", JUNO_CONTAINER: "" },
      },
    );
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "juno", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over JUNO_CONTAINER", () => {
    const spawnSync = vi
      .fn()
      .mockReturnValueOnce({
        status: 0,
    ...
    expect(
      maybeRunCliInContainer(["node", "juno", "--container", "flag-demo", "health"], {
        env: { JUNO_CONTAINER: "env-demo" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      maybeRunCliInContainer(["node", "juno", "--container", "demo", "status"], {
        env: { JUNO_CLI_CONTAINER_BYPASS: "1" } as NodeJS.ProcessEnv,
      }),
    ).toEqual({
      handled: false,
      argv: ["node", "juno", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "juno", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
      "juno update is not supported with --container; rebuild or restart the container image instead.",
    );
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "juno", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
