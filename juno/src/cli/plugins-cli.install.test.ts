...
const CLI_STATE_ROOT = "/tmp/juno-state";

function cliInstallPath(pluginId: string): string {
  return installedPluginRoot(CLI_STATE_ROOT, pluginId);
}

function createEnabledPluginConfig(pluginId: string): JunoConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): JunoConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createJunoHubInstalledConfig(params: {
  pluginId: string;
...
function createJunoHubInstallResult(params: {
  pluginId: string;
  packageName: string;
  version: string;
  channel: string;
}): Awaited<ReturnType<typeof installPluginFromJunoHub>> {
  return {
    ok: true,
    ...
    junohub: {
      ...
      junohubPackage: params.packageName,
      junohubFamily: "code-plugin",
      junohubChannel: params.channel,
      version: params.version,
...
function mockJunoHubPackageNotFound(packageName: string) {
  installPluginFromJunoHub.mockResolvedValue({
    ok: false,
    error: `JunoHub /api/v1/packages/${packageName} failed (404): Package not found`,
...
function primeNpmPluginFallback(pluginId = "demo") {
  ...
  mockJunoHubPackageNotFound(pluginId);
...
function createPathHookPackInstalledConfig(tmpRoot: string): JunoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): JunoConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  loadConfig.mockReturnValue(cfg);
  mockJunoHubPackageNotFound("@acme/demo-hooks");
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing juno.plugin.json",
  });
...
function primeHookPackPathFallback(params: {
  ...
  loadConfig.mockReturnValue({} as JunoConfig);
...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `juno doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("installs JunoHub plugins and persists source metadata", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
    ...
    const installedCfg = createJunoHubInstalledConfig({
      pluginId: "demo",
      install: {
        source: "junohub",
        spec: "junohub:demo@1.2.3",
    ...
    loadConfig.mockReturnValue(cfg);
    parseJunoHubPluginSpec.mockReturnValue({ name: "demo" });
    installPluginFromJunoHub.mockResolvedValue(
      createJunoHubInstallResult({
        pluginId: "demo",
        packageName: "demo",
        version: "1.2.3",
        channel: "official",
    ...
    await runPluginsCommand(["plugins", "install", "junohub:demo"]);

    expect(installPluginFromJunoHub).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "junohub:demo",
      }),
    );
  ...
  it("passes force through as overwrite mode for JunoHub installs", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
    ...
    await runPluginsCommand(["plugins", "install", "junohub:demo", "--force"]);
  ...
  it("prefers JunoHub before npm for bare plugin specs", async () => {
    const cfg = {
      plugins: {
        entries: {},
      },
  ...
  it("falls back to npm when JunoHub does not have the package", async () => {
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-hook-link-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the install logger to the --link dry-run probe", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-link-plugin-"));
    const cfg = {
      plugins: {
        entries: {},
        load: {
  ...
  it("does not fall back to hook pack for local path when dangerous force unsafe install is set", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-local-plugin-"));
    const cfg = {} as JunoConfig;
  ...
  it("does not fall back to hook pack for npm installs when dangerous force unsafe install is set", async () => {
    ...
    installPluginFromJunoHub.mockResolvedValue({
      ok: false,
      error: "JunoHub /api/v1/packages/demo failed (404): Package not found",
      code: "package_not_found",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-local-hook-pack-"));
    const cfg = {} as JunoConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("still falls back to npm hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    ...
    installPluginFromJunoHub.mockResolvedValue({
      ok: false,
      error: "JunoHub /api/v1/packages/@acme/demo-hooks failed (404): Package not found",
      code: "package_not_found",
    });
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("does not fall back to npm when JunoHub rejects a real package", async () => {
    installPluginFromJunoHub.mockResolvedValue({
      ok: false,
      error: 'Use "juno skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "juno skills install demo" instead.');
  });
