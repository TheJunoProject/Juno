...
const serveJunoChannelMcp = mocks.serveJunoChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveJunoChannelMcp: mocks.serveJunoChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("juno-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("juno-cli-mcp-home-", async () => {
      ...
      expect(serveJunoChannelMcp).toHaveBeenCalledWith({
