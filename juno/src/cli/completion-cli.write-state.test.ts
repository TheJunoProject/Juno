...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["juno.bash", "juno.fish", "juno.ps1", "juno.zsh"]),
    );
