...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    searchSkillsFromJunoHubMock: vi.fn(),
    installSkillFromJunoHubMock: vi.fn(),
    updateSkillsFromJunoHubMock: vi.fn(),
    readTrackedJunoHubSkillSlugsMock: vi.fn(),
...
const {
  ...
  searchSkillsFromJunoHubMock,
  installSkillFromJunoHubMock,
  updateSkillsFromJunoHubMock,
  readTrackedJunoHubSkillSlugsMock,
  buildWorkspaceSkillStatusMock,
  skillStatusReportFixture,
  defaultRuntime,
  runtimeLogs,
...
vi.mock("../agents/skills-junohub.js", () => ({
  searchSkillsFromJunoHub: (...args: unknown[]) => mocks.searchSkillsFromJunoHubMock(...args),
  installSkillFromJunoHub: (...args: unknown[]) => mocks.installSkillFromJunoHubMock(...args),
  updateSkillsFromJunoHub: (...args: unknown[]) => mocks.updateSkillsFromJunoHubMock(...args),
  readTrackedJunoHubSkillSlugs: (...args: unknown[]) =>
    mocks.readTrackedJunoHubSkillSlugsMock(...args),
}));
...
describe("skills cli commands", () => {
  ...
  it("searches JunoHub skills from the native CLI", async () => {
    searchSkillsFromJunoHubMock.mockResolvedValue([
      {
        slug: "calendar",
    ...
    expect(searchSkillsFromJunoHubMock).toHaveBeenCalledWith({
      query: "calendar",
      limit: undefined,
    });
  ...
  it("installs a skill from JunoHub into the active workspace", async () => {
    installSkillFromJunoHubMock.mockResolvedValue({
      ok: true,
      slug: "calendar",
      version: "1.2.3",
  ...
  it("updates all tracked JunoHub skills", async () => {
    readTrackedJunoHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromJunoHubMock.mockResolvedValue([
      {
        ok: true,
        slug: "calendar",
    ...
    expect(readTrackedJunoHubSkillSlugsMock).toHaveBeenCalledWith("/tmp/workspace");
  ...
  it("keeps non-JSON skills list output on stdout with human-readable formatting", async () => {
    ...
    expect(runtimeStdout.at(-1)).toContain("juno skills search");
  });
});
