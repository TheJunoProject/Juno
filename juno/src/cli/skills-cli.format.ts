...
function appendJunoHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("juno skills list")}\` to see all skills.`
      : "No skills found.";
    return appendJunoHubHint(message, opts.json);
  }
  ...
  return appendJunoHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendJunoHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("juno skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`juno config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$JUNO_CONFIG_PATH")} ${theme.muted("(default: ~/.juno/juno.json)")}`,
    );
  }
