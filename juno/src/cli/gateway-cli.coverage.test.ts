...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints json output", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (Juno)",
        displayName: "Studio",
        domain: "juno.internal.",
        host: "studio.juno.internal",
        port: 18789,
    ...
    await withEnvOverride(
      {
        ...
        JUNO_LAUNCHD_LABEL: undefined,
        JUNO_SYSTEMD_UNIT: undefined,
        INVOCATION_ID: undefined,
        SYSTEMD_EXEC_PID: undefined,
        JOURNAL_STREAM: undefined,
        JUNO_WINDOWS_TASK_NAME: undefined,
        JUNO_SERVICE_MARKER: undefined,
        JUNO_SERVICE_KIND: undefined,
      },
      async () => {
  ...
  it("keeps exit 1 for gateway lock acquisition failures", async () => {
    ...
    serviceIsLoaded.mockResolvedValue(true);
    startGatewayServer.mockRejectedValueOnce(
      new GatewayLockError("failed to acquire gateway lock at /tmp/juno/gateway.lock"),
    );
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ JUNO_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
