  ...
  plugins: [
    {
      ...
      source: "bundled",
      manifestPath: "/tmp/memory-wiki/juno.plugin.json",
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "juno", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "juno", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "juno", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "juno", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "juno", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "juno", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "juno", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "juno", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "juno", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "juno", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("juno memory");
  });
