...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Juno and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["juno update", "Update a source checkout (git)"],
        ["juno update --channel beta", "Switch to beta channel (git + npm)"],
        ["juno update --channel dev", "Switch to dev channel (git + npm)"],
        ["juno update --tag beta", "One-off update to a dist-tag or version"],
        ["juno update --tag main", "One-off package install from GitHub main"],
        ["juno update --dry-run", "Preview actions without changing anything"],
        ["juno update --no-restart", "Update without restarting the service"],
        ["juno update --json", "Output result as JSON"],
        ["juno update --yes", "Non-interactive (accept downgrade prompts)"],
        ["juno update wizard", "Interactive update wizard"],
        ["juno --update", "Shorthand for juno update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run juno update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.juno.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno update status", "Show channel + version status."],
          ["juno update status --json", "JSON output."],
          ["juno update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.juno.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
