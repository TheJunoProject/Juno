...
describe("resolveInitialTuiAgentId", () => {
  const cfg: JunoConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/juno" },
        { id: "ops", workspace: "/tmp/juno/projects/ops" },
      ],
    },
  };
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run juno devices list");
    expect(state.pairingHint).toContain("juno devices list");
  });
