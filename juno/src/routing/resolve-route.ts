...
function listAgents(cfg: JunoConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: JunoConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<JunoConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: JunoConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: JunoConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: JunoConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<JunoConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  JunoConfig,
  {
    bindingsRef: JunoConfig["bindings"];
    agentsRef: JunoConfig["agents"];
    sessionRef: JunoConfig["session"];
...
function resolveRouteCacheForConfig(cfg: JunoConfig): Map<string, ResolvedAgentRoute> {
