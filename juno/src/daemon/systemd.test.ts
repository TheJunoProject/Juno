...
const TEST_MANAGED_HOME = "/tmp/juno-test-home";
const GATEWAY_SERVICE = "juno-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/juno gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/juno", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/juno-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "juno-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "juno-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/juno-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled juno-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled juno-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when JUNO_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/juno-gateway.service",
    },
    {
      name: "uses profile-specific service name when JUNO_PROFILE is set to a custom value",
      env: { HOME: "/home/test", JUNO_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/juno-gateway-jbphoenix.service",
    },
    {
      name: "prefers JUNO_SYSTEMD_UNIT over JUNO_PROFILE",
      env: {
        HOME: "/home/test",
        JUNO_PROFILE: "jbphoenix",
        JUNO_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles JUNO_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        JUNO_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from JUNO_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        JUNO_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/juno gateway start --name "My Bot"')).toEqual([
      "/usr/bin/juno",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('juno --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["juno", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('juno --path "C:\\\\Program Files\\\\Juno"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["juno", "--path", "C:\\\\Program Files\\\\Juno"]);

    expect(
      splitArgsPreservingQuotes('juno --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["juno", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/juno gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads JUNO_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/juno gateway run", "EnvironmentFile=%h/.juno/.env"],
      { [`${TEST_SERVICE_HOME}/.juno/.env`]: "JUNO_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.JUNO_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/juno gateway run",
        "EnvironmentFile=%h/.juno/.env",
        'Environment="JUNO_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.juno/.env`]: "JUNO_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.JUNO_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.JUNO_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.juno/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/juno-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/juno gateway run",
          'EnvironmentFile=%h/.juno/first.env "%h/.juno/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.juno/first.env") {
        return "JUNO_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.juno/second env.env") {
        return 'JUNO_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      JUNO_GATEWAY_TOKEN: "first-token",
      JUNO_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "JUNO_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "JUNO_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "JUNO_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      JUNO_GATEWAY_TOKEN: "override-token",
      JUNO_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/juno-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/juno gateway run",
          "EnvironmentFile=%h/.juno/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.juno/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'JUNO_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "JUNO_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      JUNO_GATEWAY_TOKEN: "quoted token",
      JUNO_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      JUNO_GATEWAY_TOKEN: "file",
      JUNO_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".juno");
    const env = {
      HOME: home,
      JUNO_STATE_DIR: stateDir,
      JUNO_SYSTEMD_UNIT: "juno-gateway-stage-test",
    };
    ...
    await fs.writeFile(
      ...
      ["JUNO_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/juno", "gateway", "run"],
        workingDirectory: "/tmp",
        environment: {
          JUNO_GATEWAY_TOKEN: "dotenv-token",
      ...
      expect(unit).toContain("Environment=JUNO_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=JUNO_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("JUNO_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    ...
    await fs.writeFile(
      path.join(stateDir, ".env"),
      ["JUNO_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
      "utf8",
    );
    ...
    try {
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          JUNO_GATEWAY_TOKEN: "fresh-token",
          LLM_API_KEY: "dotenv-key",
        },
      });
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=JUNO_GATEWAY_TOKEN=fresh-token");
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "juno-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ JUNO_PROFILE: "work" });
  });
