...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when JUNO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".juno", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when JUNO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", JUNO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".juno-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers JUNO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        JUNO_PROFILE: "rescue",
        JUNO_STATE_DIR: "C:\\State\\juno",
      },
      expected: path.join("C:\\State\\juno", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", JUNO_PROFILE: "default" },
      expected: path.join("/home/test", ".juno", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\juno",
          environment: {
            NODE_ENV: "production",
            JUNO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\juno\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\juno\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Juno Share\\node.exe" "\\\\fileserver\\Juno Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Juno Share\\node.exe",
            "\\\\fileserver\\Juno Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from JUNO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ JUNO_STATE_DIR: path.join(tmpDir, "custom-state") }),
