...
// Real content from the juno-gateway.service unit file (the canonical gateway unit).
const GATEWAY_SERVICE_CONTENTS = `\
[Unit]
Description=Juno Gateway (v2026.3.8)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/bin/node /home/juno/.npm-global/lib/node_modules/juno/dist/entry.js gateway --port 18789
Restart=always
Environment=JUNO_SERVICE_MARKER=juno
Environment=JUNO_SERVICE_KIND=gateway
Environment=JUNO_SERVICE_VERSION=2026.3.8

[Install]
WantedBy=default.target
`;

// Real content from the juno-test.service unit file (a non-gateway juno service).
const TEST_SERVICE_CONTENTS = `\
[Unit]
Description=Juno test service
...
const JUNODBOT_GATEWAY_CONTENTS = `\
[Unit]
...
describe("detectMarkerLineWithGateway", () => {
  it("returns null for juno-test.service (juno only in description, no gateway on same line)", () => {
    expect(detectMarkerLineWithGateway(TEST_SERVICE_CONTENTS)).toBeNull();
  });

  it("returns juno for the canonical gateway unit (ExecStart has both juno and gateway)", () => {
  ...
  it("handles line continuations — marker and gateway split across physical lines", () => {
    const contents = `[Service]\nExecStart=/usr/bin/node /opt/juno/dist/entry.js \\\n  gateway --port 18789\n`;
    expect(detectMarkerLineWithGateway(contents)).toBe("juno");
  });
});
...
describe("findExtraGatewayServices (linux / scanSystemdDir) — real filesystem", () => {
  ...
  it.skipIf(!isLinux)("does not report juno-test.service as a gateway service", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "juno-test-"));
    const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
    try {
      await fs.mkdir(systemdDir, { recursive: true });
      await fs.writeFile(path.join(systemdDir, "juno-test.service"), TEST_SERVICE_CONTENTS);
  ...
  it.skipIf(!isLinux)(
    "does not report the canonical juno-gateway.service as an extra service",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "juno-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "juno-gateway.service"),
...
describe("findExtraGatewayServices (win32)", () => {
  ...
  it("collects only non-juno marker tasks from schtasks output", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      code: 0,
      stdout: [
        "TaskName: Juno Gateway",
