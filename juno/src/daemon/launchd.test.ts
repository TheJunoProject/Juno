...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.juno.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", JUNO_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("stops LaunchAgent by disabling relaunch before stopping the process", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.juno.gateway`;
    expect(state.launchctlCalls).toContainEqual(["disable", serviceId]);
    expect(state.launchctlCalls).toContainEqual(["stop", "ai.juno.gateway"]);
  ...
  it("treats already-unloaded services as successfully stopped without bootout fallback", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual([
      "disable",
      `${typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501"}/ai.juno.gateway`,
    ]);
    expect(state.launchctlCalls.some((call) => call[0] === "bootout")).toBe(false);
    expect(output).toContain("Stopped LaunchAgent");
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.juno.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when JUNO_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.juno.gateway.plist",
    },
    {
      name: "uses profile-specific label when JUNO_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", JUNO_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.juno.jbphoenix.plist",
    },
    {
      name: "prefers JUNO_LAUNCHD_LABEL over JUNO_PROFILE",
      env: {
        HOME: "/Users/test",
        JUNO_PROFILE: "jbphoenix",
        JUNO_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from JUNO_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        JUNO_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty JUNO_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        JUNO_PROFILE: "myprofile",
        JUNO_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.juno.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
