...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `juno-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      JUNO_LAUNCHD_LABEL: `ai.juno.launchd-int-${testId}`,
      JUNO_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
