...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.JUNO_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    JUNO_PROFILE: profile,
    JUNO_GATEWAY_PORT: String(port),
    JUNO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    JUNO_SYSTEMD_UNIT: systemdUnit,
    JUNO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    JUNO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    JUNO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    JUNO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.JUNO_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    JUNO_GATEWAY_TOKEN: gatewayToken,
    JUNO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    JUNO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    JUNO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    JUNO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    JUNO_LOG_PREFIX: "node",
    JUNO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    JUNO_SERVICE_KIND: NODE_SERVICE_KIND,
    JUNO_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    JUNO_STATE_DIR: sharedEnv.stateDir,
    JUNO_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.JUNO_STATE_DIR;
  const configPath = env.JUNO_CONFIG_PATH;
