...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.JUNO_GATEWAY_PORT).toBe("18789");
    expect(env.JUNO_GATEWAY_TOKEN).toBeUndefined();
    expect(env.JUNO_SERVICE_MARKER).toBe("juno");
    expect(env.JUNO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.JUNO_SERVICE_VERSION).toBe("string");
    expect(env.JUNO_SYSTEMD_UNIT).toBe("juno-gateway.service");
    expect(env.JUNO_WINDOWS_TASK_NAME).toBe("Juno Gateway");
    if (process.platform === "darwin") {
      expect(env.JUNO_LAUNCHD_LABEL).toBe("ai.juno.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", JUNO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.JUNO_SYSTEMD_UNIT).toBe("juno-gateway-work.service");
    expect(env.JUNO_WINDOWS_TASK_NAME).toBe("Juno Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.JUNO_LAUNCHD_LABEL).toBe("ai.juno.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through JUNO_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", JUNO_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.JUNO_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits JUNO_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        JUNO_GATEWAY_TOKEN: "   ",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".juno"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", JUNO_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".juno-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", JUNO_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".juno"));
  });

  it("uses JUNO_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", JUNO_STATE_DIR: "/var/lib/juno" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/juno"));
  });

  it("expands ~ in JUNO_STATE_DIR", () => {
    const env = { HOME: "/Users/test", JUNO_STATE_DIR: "~/juno-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/juno-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { JUNO_STATE_DIR: "C:\\State\\juno" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\juno");
  });
});
