...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds JUNO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `juno gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service JUNO_GATEWAY_TOKEN does not match gateway.auth.token in juno.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.JUNO_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.JUNO_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `juno gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
