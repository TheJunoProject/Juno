...
const planJunoModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planJunoModelsJson: (...args: unknown[]) => planJunoModelsJsonMock(...args),
  }));
  ({ ensureJunoModelsJson } = await import("./models-config.js"));
});

beforeEach(() => {
  planJunoModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureJunoModelsJson(first), ensureJunoModelsJson(second)]);
      } finally {
