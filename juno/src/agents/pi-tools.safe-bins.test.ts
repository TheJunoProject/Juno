...
let createJunoCodingTools: typeof import("./pi-tools.js").createJunoCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      JUNO_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "juno-test-no-bundled-extensions"),
    },
    async () => {
      ({ createJunoCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
describe("createJunoCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "juno-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "juno-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "juno-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "juno-safe-bins-redirect-",
        safeBins: ["head"],
