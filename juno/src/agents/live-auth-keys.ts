...
const GOOGLE_LIVE_SINGLE_KEY = "JUNO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "JUNO_LIVE_ANTHROPIC_KEY",
    listVar: "JUNO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "JUNO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `JUNO_LIVE_${base}_KEY`;
