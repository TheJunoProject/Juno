...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const openJunoTmpDir = path.resolve(resolvePreferredJunoTmpDir());
  if (!isPathInside(openJunoTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: openJunoTmpDir });
  return resolved;
}
