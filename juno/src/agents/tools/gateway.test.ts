...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    juno: process.env.JUNO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.juno === undefined) {
      delete process.env.JUNO_GATEWAY_TOKEN;
    } else {
      process.env.JUNO_GATEWAY_TOKEN = envSnapshot.juno;
    }
  });
  ...
  it("uses JUNO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.JUNO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
