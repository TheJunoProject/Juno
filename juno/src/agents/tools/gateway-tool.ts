...
import { isJunoOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `juno security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as JunoConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as JunoConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isJunoOwnerOnlyCoreToolName("gateway"),
    description:
