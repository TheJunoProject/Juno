...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.juno/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "juno doctor" and "juno approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
