...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "juno-sandbox-ssh-"));
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredJunoTmpDir() ?? os.tmpdir());
}
