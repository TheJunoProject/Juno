...
import { markJunoExecEnv } from "../../infra/juno-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "juno.sandbox=1");
  args.push("--label", `juno.sessionKey=${params.scopeKey}`);
  args.push("--label", `juno.createdAtMs=${createdAtMs}`);
  args.push("--label", `juno.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `juno.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markJunoExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "juno.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`juno sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`juno sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("juno sandbox recreate --all");
}
