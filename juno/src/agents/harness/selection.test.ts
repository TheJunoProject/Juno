...
const originalRuntime = process.env.JUNO_AGENT_RUNTIME;
const originalHarnessFallback = process.env.JUNO_AGENT_HARNESS_FALLBACK;

afterEach(() => {
  clearAgentHarnesses();
  piRunAttempt.mockClear();
  if (originalRuntime == null) {
    delete process.env.JUNO_AGENT_RUNTIME;
  } else {
    process.env.JUNO_AGENT_RUNTIME = originalRuntime;
  }
  if (originalHarnessFallback == null) {
    delete process.env.JUNO_AGENT_HARNESS_FALLBACK;
  } else {
    process.env.JUNO_AGENT_HARNESS_FALLBACK = originalHarnessFallback;
  }
});

function createAttemptParams(config?: JunoConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
describe("runAgentHarnessAttemptWithFallback", () => {
  ...
  it("falls back to the PI harness in auto mode when the selected plugin harness fails", async () => {
    process.env.JUNO_AGENT_RUNTIME = "auto";
