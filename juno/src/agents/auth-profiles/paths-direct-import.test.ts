...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  const envSnapshot = captureEnv(["JUNO_STATE_DIR"]);
  let stateDir = "";

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveJunoAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveJunoAgentDir()`
    // nullish branch. With JUNO_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-juno-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-path-ensure-"));
    process.env.JUNO_STATE_DIR = stateDir;
  });
