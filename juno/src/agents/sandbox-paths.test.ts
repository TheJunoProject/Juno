...
async function withOutsideHardlinkInJunoTmp<T>(
  params: {
    openJunoTmpDir: string;
  ...
  const hardlinkPath = path.join(params.openJunoTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.openJunoTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.openJunoTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.openJunoTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const openJunoTmpDir = resolvePreferredJunoTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Juno tmp root",
      media: path.join(openJunoTmpDir, "image.png"),
      expected: path.join(openJunoTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Juno tmp root",
      media: pathToFileURL(path.join(openJunoTmpDir, "photo.png")).href,
      expected: path.join(openJunoTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Juno tmp root",
      media: path.join(openJunoTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(openJunoTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(openJunoTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside juno tmp root",
      media: path.join(os.tmpdir(), "outside-juno", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Juno tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(openJunoTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(openJunoTmpDir, { recursive: true });
      const symlinkPath = path.join(openJunoTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Juno tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJunoTmp(
      {
        openJunoTmpDir,
  ...
  it("rejects symlinked Juno tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJunoTmp(
