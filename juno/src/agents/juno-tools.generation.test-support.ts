...
export function describeJunoGenerationToolRegistration(params: {
  suiteName: string;
  toolName: string;
  toolLabel: string;
}) {
  describe(params.suiteName, () => {
    it(`registers ${params.toolName} when ${params.toolLabel} is present`, () => {
      ...
      expect(collectPresentJunoTools([tool])).toEqual([tool]);
    });

    it(`omits ${params.toolName} when ${params.toolLabel} is absent`, () => {
      expect(collectPresentJunoTools([null]).map((tool) => tool.name)).not.toContain(
        params.toolName,
      );
    });
  });
