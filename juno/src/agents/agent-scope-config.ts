...
type AgentEntry = NonNullable<NonNullable<JunoConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: JunoConfig): AgentEntry[] {
...
export function listAgentIds(cfg: JunoConfig): string[] {
...
export function resolveDefaultAgentId(cfg: JunoConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: JunoConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: JunoConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
