...
import { createJunoTools } from "./juno-tools.js";
...
function resolveExecConfig(params: { cfg?: JunoConfig; agentId?: string }) {
...
export function createJunoCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createJunoReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
