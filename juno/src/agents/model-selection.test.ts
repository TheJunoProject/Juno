...
function resolveAnthropicOpusThinking(cfg: JunoConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: JunoConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<JunoConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<JunoConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as JunoConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as JunoConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as JunoConfig)).toBe(false);
    });
  });
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", () => {
      const warnLogs = createWarnLogCapture("juno-model-selection-test");
      try {
        const cfg: Partial<JunoConfig> = {
          agents: {
            defaults: {
