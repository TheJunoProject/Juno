...
import { KeyedAsyncQueue } from "juno/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredJunoTmpDir(), "juno-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".juno-cli-images");
  }
  return path.join(resolvePreferredJunoTmpDir(), "juno-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredJunoTmpDir(), "juno-cli-system-prompt-"),
  );
