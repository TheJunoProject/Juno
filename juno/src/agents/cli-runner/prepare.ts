...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveJunoDocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveJunoDocsPath>[0],
  ) => (await import("../docs-path.js")).resolveJunoDocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          JUNO_MCP_TOKEN: mcpLoopbackRuntime.token,
          JUNO_MCP_AGENT_ID: sessionAgentId ?? "",
          JUNO_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          JUNO_MCP_SESSION_KEY: params.sessionKey ?? "",
          JUNO_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
          JUNO_MCP_SENDER_IS_OWNER: params.senderIsOwner === true ? "true" : "false",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const docsPath = await prepareDeps.resolveJunoDocsPath({
    workspaceDir,
