...
beforeAll(async () => {
  envSnapshot = captureEnv(["JUNO_BUNDLED_PLUGINS_DIR"]);
  bundleProbeHomeDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-home-");
  bundleProbeWorkspaceDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-workspace-");
  const emptyBundledDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-bundled-");
  process.env.JUNO_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): JunoConfig {
  return {
    plugins: {
      entries: {
...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-empty-");
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-workspace-root-");
    const pluginRoot = path.join(workspaceDir, ".juno", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "juno"]);
    expect(raw.mcpServers?.juno?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.juno?.headers?.Authorization).toBe("Bearer ${JUNO_MCP_TOKEN}");

    await prepared.cleanup?.();
  });

  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await tempHarness.createTempDir("juno-cli-bundle-mcp-env-");
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        JUNO_MCP_TOKEN: "loopback-token-123",
        JUNO_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
        JUNO_MCP_SENDER_IS_OWNER: "false",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/juno-bundle-mcp-disabled",
    });
  ...
  it("injects codex MCP config overrides with env-backed loopback headers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/juno-bundle-mcp-codex",
      config: { plugins: { enabled: false } },
      additionalConfig: {
        mcpServers: {
          juno: {
    ...
    expect(prepared.backend.args).toEqual([
      ...
      "--json",
      "-c",
      'mcp_servers={ juno = { url = "http://127.0.0.1:23119/mcp", bearer_token_env_var = "JUNO_MCP_TOKEN", env_http_headers = { x-session-key = "JUNO_MCP_SESSION_KEY", x-juno-sender-is-owner = "JUNO_MCP_SENDER_IS_OWNER" } } }',
    ]);
  ...
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/juno-bundle-mcp-gemini",
      config: { plugins: { enabled: false } },
      additionalConfig: {
        mcpServers: {
          juno: {
    ...
    expect(prepared.env?.JUNO_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["juno"]);
    expect(raw.mcpServers?.juno?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.juno?.headers?.Authorization).toBe("Bearer loopback-token-123");
