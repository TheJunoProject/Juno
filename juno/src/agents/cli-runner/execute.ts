...
const CLI_BACKEND_PRESERVE_ENV = "JUNO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.JUNO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.JUNO_MCP_SESSION_KEY || "<empty>"}`,
    `agentId=${childEnv.JUNO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.JUNO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.JUNO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.JUNO_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.JUNO_MCP_TOKEN ||
            env.JUNO_MCP_SESSION_KEY ||
            env.JUNO_MCP_SENDER_IS_OWNER
          ) {
