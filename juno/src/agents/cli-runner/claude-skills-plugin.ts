...
const JUNO_CLAUDE_PLUGIN_NAME = "juno-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredJunoTmpDir(), "juno-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, JUNO_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: JUNO_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Juno skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
