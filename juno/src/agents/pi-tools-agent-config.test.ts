...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: JunoConfig) {
    return createJunoCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<JunoConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<JunoConfig["agents"]>["list"]>[number]["tools"];
  }): JunoConfig {
    return {
      tools: params.tools,
      agents: {
