...
const TEST_ENV_VAR = "JUNO_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/juno-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): JunoConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: JunoConfig) {
