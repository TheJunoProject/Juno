...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("juno-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("juno-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("juno-tilde-test-mkdir-");
  ...
  it("ignores JUNO_HOME for write operations", async () => {
    const junoHome = await createTempDir("juno-home-override-", os.tmpdir());
    const dir = await createTempDir("juno-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("JUNO_HOME", junoHome);

    createHostWorkspaceWriteTool(junoHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(junoHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores JUNO_HOME for mkdir operations", async () => {
    const junoHome = await createTempDir("juno-home-override-", os.tmpdir());
    const dir = await createTempDir("juno-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(junoHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores JUNO_HOME for readFile operations", async () => {
    const junoHome = await createTempDir("juno-home-override-", os.tmpdir());
    const dir = await createTempDir("juno-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(junoHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores JUNO_HOME for access operations", async () => {
    const junoHome = await createTempDir("juno-home-override-", os.tmpdir());
    const dir = await createTempDir("juno-tilde-test-edit-");
