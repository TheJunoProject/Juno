...
describe("provider attribution", () => {
  it("resolves the canonical Juno product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Juno runtime wrapper.",
      product: "Juno",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { JUNO_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "juno",
        version: "2026.3.22",
        "User-Agent": "juno/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { JUNO_VERSION: "2026.3.22" })).toEqual({
      originator: "juno",
      version: "2026.3.22",
      "User-Agent": "juno/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { JUNO_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ JUNO_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
