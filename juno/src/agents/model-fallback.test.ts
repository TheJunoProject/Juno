...
function makeFallbacksOnlyCfg(): JunoConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): JunoConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "juno-auth-suite-mock");
...
// https://github.com/juno/juno/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("juno-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
