...
function resolveAttachmentLimits(config: JunoConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".juno", "attachments");
  const relDir = path.posix.join(".juno", "attachments", attachmentId);
