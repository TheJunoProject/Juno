...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: JunoConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: JunoConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: JunoConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: JunoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: JunoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: JunoConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/juno");
  });

  it("uses JUNO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "juno-home");
    vi.stubEnv("JUNO_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as JunoConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".juno", "workspace"));
  });

  it("uses JUNO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "juno-home");
    vi.stubEnv("JUNO_HOME", home);
    // Clear state dir so it falls back to JUNO_HOME
    vi.stubEnv("JUNO_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as JunoConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".juno", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/juno-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/juno-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-agent-scope-"));
