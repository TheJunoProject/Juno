...
function setConfigOverride(next: JunoConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/juno/juno/issues/31298
    previousFastTestEnv = process.env.JUNO_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.JUNO_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // JUNO_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Juno runtime context (internal):");
