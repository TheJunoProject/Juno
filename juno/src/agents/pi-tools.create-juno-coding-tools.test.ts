...
import "./test-helpers/fast-juno-tools.js";
import { createJunoCodingTools } from "./pi-tools.js";
...
describe("createJunoCodingTools", () => {
  const testConfig: JunoConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createJunoCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createJunoCodingTools({
      config: testConfig,
    ...
    const codexTools = createJunoCodingTools({
      config: testConfig,
    ...
    const disabledConfig: JunoConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createJunoCodingTools({
    ...
    const anthropicTools = createJunoCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: JunoConfig = {
      tools: {
        exec: {
    ...
    const allowed = createJunoCodingTools({
    ...
    const denied = createJunoCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createJunoCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createJunoCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createJunoCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createJunoCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-depth-policy-"));
    try {
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createJunoCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createJunoCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createJunoCodingTools();
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-read-"));
    try {
      ...
      const contents = "Hello from juno read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "juno-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "juno-workspace"),
    ...
    const tools = createJunoCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-canonical-"));
    try {
      const tools = createJunoCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-legacy-alias-"));
    try {
      const tools = createJunoCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-structured-write-"));
    try {
      const tools = createJunoCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-structured-edit-"));
    try {
