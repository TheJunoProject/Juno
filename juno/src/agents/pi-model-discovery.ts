...
function createJunoModelRegistry(
  authStorage: PiAuthStorage,
  modelsJsonPath: string,
  agentDir: string,
): PiModelRegistry {
...
export function scrubLegacyStaticAuthJsonEntriesForDiscovery(pathname: string): void {
  if (process.env.JUNO_AUTH_STORE_READONLY === "1") {
    return;
  }
...
export function discoverModels(authStorage: PiAuthStorage, agentDir: string): PiModelRegistry {
  return createJunoModelRegistry(authStorage, path.join(agentDir, "models.json"), agentDir);
}
