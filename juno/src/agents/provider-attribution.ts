...
const JUNO_ATTRIBUTION_PRODUCT = "Juno";
const JUNO_ATTRIBUTION_ORIGINATOR = "juno";
...
function formatJunoUserAgent(version: string): string {
  return `${JUNO_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: JUNO_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: JUNO_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatJunoUserAgent(identity.version),
    },
  };
}
