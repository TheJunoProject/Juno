...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousJunoAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousJunoAgentDir = process.env.JUNO_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousJunoAgentDir === undefined) {
      delete process.env.JUNO_AGENT_DIR;
    } else {
      process.env.JUNO_AGENT_DIR = previousJunoAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "JUNO_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.JUNO_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.JUNO_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.JUNO_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: JunoConfig = {
  models: {
    providers: {
      "custom-proxy": {
