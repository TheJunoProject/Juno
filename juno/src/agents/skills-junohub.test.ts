...
const fetchJunoHubSkillDetailMock = vi.fn();
const downloadJunoHubSkillArchiveMock = vi.fn();
const listJunoHubSkillsMock = vi.fn();
const resolveJunoHubBaseUrlMock = vi.fn(() => "https://junohub.ai");
const searchJunoHubSkillsMock = vi.fn();
...
vi.mock("../infra/junohub.js", () => ({
  fetchJunoHubSkillDetail: fetchJunoHubSkillDetailMock,
  downloadJunoHubSkillArchive: downloadJunoHubSkillArchiveMock,
  listJunoHubSkills: listJunoHubSkillsMock,
  resolveJunoHubBaseUrl: resolveJunoHubBaseUrlMock,
  searchJunoHubSkills: searchJunoHubSkillsMock,
}));
...
const { installSkillFromJunoHub, searchSkillsFromJunoHub, updateSkillsFromJunoHub } =
  await import("./skills-junohub.js");

describe("skills-junohub", () => {
  beforeEach(() => {
    fetchJunoHubSkillDetailMock.mockReset();
    downloadJunoHubSkillArchiveMock.mockReset();
    listJunoHubSkillsMock.mockReset();
    resolveJunoHubBaseUrlMock.mockReset();
    searchJunoHubSkillsMock.mockReset();
    archiveCleanupMock.mockReset();
    withExtractedArchiveRootMock.mockReset();
    installPackageDirMock.mockReset();
    fileExistsMock.mockReset();

    resolveJunoHubBaseUrlMock.mockReturnValue("https://junohub.ai");
    fileExistsMock.mockImplementation(async (input: string) => input.endsWith("SKILL.md"));
    fetchJunoHubSkillDetailMock.mockResolvedValue({
      skill: {
        slug: "agentreceipt",
    ...
    downloadJunoHubSkillArchiveMock.mockResolvedValue({
      archivePath: "/tmp/agentreceipt.zip",
      integrity: "sha256-test",
      cleanup: archiveCleanupMock,
    });
    archiveCleanupMock.mockResolvedValue(undefined);
    searchJunoHubSkillsMock.mockResolvedValue([]);
  ...
  it("installs JunoHub skills from flat-root archives", async () => {
    const result = await installSkillFromJunoHub({
      workspaceDir: "/tmp/workspace",
      slug: "agentreceipt",
    });

    expect(downloadJunoHubSkillArchiveMock).toHaveBeenCalledWith({
      slug: "agentreceipt",
      version: "1.0.0",
      baseUrl: undefined,
    });
  ...
  describe("legacy tracked slugs remain updatable", () => {
    async function createLegacyTrackedSkillFixture(slug: string) {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-skills-junohub-"));
      const skillDir = path.join(workspaceDir, "skills", slug);
      await fs.mkdir(path.join(skillDir, ".junohub"), { recursive: true });
      await fs.mkdir(path.join(workspaceDir, ".junohub"), { recursive: true });
      await fs.writeFile(
        path.join(skillDir, ".junohub", "origin.json"),
        `${JSON.stringify(
          {
            version: 1,
            registry: "https://legacy.junohub.ai",
            slug,
      ...
      await fs.writeFile(
        path.join(workspaceDir, ".junohub", "lock.json"),
        `${JSON.stringify(
          {
            version: 1,
            skills: {
    ...
    it("updates all tracked legacy Unicode slugs in place", async () => {
      ...
      try {
        const results = await updateSkillsFromJunoHub({
          workspaceDir,
        });

        expect(fetchJunoHubSkillDetailMock).toHaveBeenCalledWith({
          slug,
          baseUrl: "https://legacy.junohub.ai",
        });
        expect(downloadJunoHubSkillArchiveMock).toHaveBeenCalledWith({
          slug,
          version: "1.0.0",
  ...
  it("uses search for browse-all skill discovery", async () => {
    searchJunoHubSkillsMock.mockResolvedValueOnce([
      {
        score: 1,
        slug: "calendar",
    ...
    await expect(searchSkillsFromJunoHub({ limit: 20 })).resolves.toEqual([
      {
        score: 1,
        slug: "calendar",
        displayName: "Calendar",
    ...
    expect(searchJunoHubSkillsMock).toHaveBeenCalledWith({
      query: "*",
      limit: 20,
      baseUrl: undefined,
    });
    expect(listJunoHubSkillsMock).not.toHaveBeenCalled();
  });
});
