...
describe("ensureAuthProfileStore", () => {
  ...
  function restoreAgentDirEnv(params: {
    ...
    restoreEnvValue("JUNO_AGENT_DIR", params.previousAgentDir);
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "juno-auth-merge-"));
    const previousAgentDir = process.env.JUNO_AGENT_DIR;
    ...
    try {
      ...
      process.env.JUNO_AGENT_DIR = mainDir;
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("juno-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("juno-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "juno-oauth-migrate-"));
    ...
    try {
      ...
      process.env.JUNO_STATE_DIR = root;
      process.env.JUNO_AGENT_DIR = agentDir;
      ...
      clearRuntimeAuthProfileStoreSnapshots();
      restoreEnvValue("JUNO_STATE_DIR", previousStateDir);
      restoreAgentDirEnv({ previousAgentDir, previousPiAgentDir });
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("exposes provider-managed runtime auth without persisting copied tokens", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "juno-external-auth-"));
    const previousAgentDir = process.env.JUNO_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const agentDir = path.join(root, "agent");
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "juno-secrets-runtime-"));
    const previousStateDir = process.env.JUNO_STATE_DIR;
    try {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("juno-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "juno-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
