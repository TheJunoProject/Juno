...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "juno-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-sandbox-skills-"));
  });
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    ...
    process.env.JUNO_BUNDLED_SKILLS_DIR = bundledDir;
