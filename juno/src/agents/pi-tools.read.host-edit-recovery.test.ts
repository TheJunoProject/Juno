...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when JUNO_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const junoHome = path.join(tmpDir, "juno-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(junoHome, { recursive: true });

    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousOpenjunoHome = process.env.JUNO_HOME;
    ...
    try {
      ...
      await expect(fs.access(path.join(junoHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
      ...
      if (previousOpenjunoHome === undefined) {
        delete process.env.JUNO_HOME;
      } else {
        process.env.JUNO_HOME = previousOpenjunoHome;
      }
    }
  });
