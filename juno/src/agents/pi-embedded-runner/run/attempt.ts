...
import { resolveJunoDocsPath } from "../../docs-path.js";
...
import { createJunoCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveJunoAgentDir();
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createJunoCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveJunoDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/juno/juno/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("juno:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/juno/juno/issues/8643
