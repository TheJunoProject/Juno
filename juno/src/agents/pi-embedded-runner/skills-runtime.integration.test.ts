...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("juno-bundled-");
  const workspaceDir = await createTempDir("juno-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: JunoConfig) {
