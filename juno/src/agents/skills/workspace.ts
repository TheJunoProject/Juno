...
function resolveSkillsLimits(config?: JunoConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "juno-bundled" && candidateIsSymlink) {
    return {
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values())
    ...
    .map((record) => {
      ...
      return {
        skill,
        frontmatter,
        metadata: resolveJunoMetadata(frontmatter),
        invocation,
        exposure: {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`juno skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`juno skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
