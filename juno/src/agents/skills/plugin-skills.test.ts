    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "juno.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "juno.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "juno.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("juno-");
  const acpxRoot = await tempDirs.make("juno-acpx-plugin-");
  const helperRoot = await tempDirs.make("juno-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("juno-");
  const pluginRoot = await tempDirs.make("juno-plugin-");
  const outsideDir = await tempDirs.make("juno-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("juno-");
    const pluginRoot = await tempDirs.make("juno-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("juno-");
    const pluginRoot = await tempDirs.make("juno-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
