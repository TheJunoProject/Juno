...
function isLikelyJunoCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "juno" ||
    entryBasename === "juno.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Juno CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureJunoModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyJunoCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): JunoConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureJunoModelsJson(cfg);
    } catch {
