...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureJunoModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.js", () => ({
  ensureJunoModelsJson: contextTestState.ensureJunoModelsJson,
}));

vi.mock("./agent-paths.js", () => ({
  resolveJunoAgentDir: () => "/tmp/juno-agent",
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureJunoModelsJson.mockClear();
  return { ensureJunoModelsJson: contextTestState.ensureJunoModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real juno startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        ...
        {
          argv: ["node", "juno", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "juno", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureJunoModelsJson } = mockContextModuleDeps(loadConfigMock);
        ...
        expect(ensureJunoModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
