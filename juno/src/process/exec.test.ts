...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.JUNO_BASE_ENV).toBe("base");
    expect(resolved.JUNO_TEST_ENV).toBe("ok");
    expect(resolved.JUNO_TO_REMOVE).toBeUndefined();
    expect(resolved.JUNO_CLI).toBe(JUNO_CLI_ENV_VALUE);
  });
