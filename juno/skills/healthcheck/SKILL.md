---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Juno deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Juno cron scheduling for periodic checks, or version status checks on a machine running Juno (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Juno, then align it to a user-defined risk tolerance without breaking access. Use Juno security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Juno changes the host firewall, SSH, or OS updates; it does not.
...
5. Juno gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Juno can access sensitive data.
...
### 2) Run Juno security audits (read-only)

As part of the default read-only checks, run `juno security audit --deep`. Only offer alternatives if the user requests them:

1. `juno security audit` (faster, non-probing)
2. `juno security audit --json` (structured output)

Offer to apply Juno safe defaults (numbered):

1. `juno security audit --fix`

Be explicit that `--fix` only tightens Juno defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Juno version/update status (read-only)

As part of the default read-only checks, run `juno update status`.
...
- Credential hygiene notes (location of Juno creds, prefer disk encryption)
...
After Juno install or first hardening pass, run at least one baseline audit and version check:

- `juno security audit`
- `juno security audit --deep`
- `juno update status`

Ongoing monitoring is recommended. Use the Juno cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `juno cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `juno update status`
...
Before creating, `juno cron list` and match on exact `name`. If found, `juno cron edit <id> ...`.
If not found, `juno cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `juno update status` (preferred for source checkouts and channels)
2. `npm view juno version` (published npm version)

## Juno command accuracy

Use only supported commands and flags:

- `juno security audit [--deep] [--fix] [--json]`
- `juno status` / `juno status --deep`
- `juno health --json`
- `juno update status`
- `juno cron add|list|runs|run`

Do not invent CLI flags or imply Juno enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Juno compaction:
