...
1. Download the runtime for your OS (extracts into `$JUNO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.juno/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$JUNO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.juno/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Juno config file (`$JUNO_CONFIG_PATH`, default `~/.juno/juno.json`):
