---
name: node-connect
description: Diagnose Juno node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Juno is advertising that route, then fix pairing/auth.
...
- whether `juno devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `juno qr --json`. It uses the same setup-code payload Android scans.

```bash
juno config get gateway.mode
juno config get gateway.bind
juno config get gateway.tailscale.mode
juno config get gateway.remote.url
juno config get gateway.auth.mode
juno config get gateway.auth.allowTailscale
juno config get plugins.entries.device-pair.config.publicUrl
juno qr --json
juno devices list
juno nodes status
```

If this Juno instance is pointed at a remote gateway, also run:
...
If `juno qr --json` says `Gateway is only bound to loopback`:
...
- `juno devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run juno qr again, rescan, then approve the pending device pairing.`
