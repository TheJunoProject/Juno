---
summary: "Move (migrate) an Juno install from one machine to another"
read_when:
  - You are moving Juno to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Juno to a New Machine

This guide moves an Juno gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.juno/` by default) and your **workspace**, you preserve:

- **Config** -- `juno.json` and all gateway settings
...
<Tip>
Run `juno status` on the old machine to confirm your state directory path.
Custom profiles use `~/.juno-<profile>/` or a path set via `JUNO_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    juno gateway stop
    cd ~
    tar -czf juno-state.tgz .juno
    ```

    If you use multiple profiles (e.g. `~/.juno-work`), archive each separately.

  </Step>

  <Step title="Install Juno on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.juno/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `JUNO_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `juno doctor`.
  </Accordion>

  <Accordion title="Copying only juno.json">
...
- [ ] `juno status` shows the gateway running
