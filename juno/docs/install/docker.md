---
summary: "Optional Docker-based setup and onboarding for Juno"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Juno on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export JUNO_IMAGE="ghcr.io/juno/juno:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/juno/juno/pkgs/container/juno).
  ...
  <Step title="Complete onboarding">
    ...
    `juno-gateway` directly. `juno-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm juno-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm juno-cli channels login

    # Telegram
    docker compose run --rm juno-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm juno-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t juno:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node juno-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node juno-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d juno-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `JUNO_EXTRA_MOUNTS`
or `JUNO_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `juno-cli` shares `juno-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d juno-gateway`, run onboarding
and setup-time config writes through `juno-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `JUNO_IMAGE`               | Use a remote image instead of building locally                   |
| `JUNO_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `JUNO_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `JUNO_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `JUNO_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `JUNO_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `JUNO_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec juno-gateway node dist/index.js health --token "$JUNO_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `JUNO_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `JUNO_CONFIG_DIR` to `/home/node/.juno` and
`JUNO_WORKSPACE_DIR` to `/home/node/.juno/workspace`, so those paths
survive container replacement.

That mounted config directory is where Juno keeps:

- `juno.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `JUNO_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/juno/`.

### Shell helpers (optional)

For easier day-to-day Docker management, install `JunoDock`:
...
If you installed JunoDock from the older `scripts/shell-helpers/junodock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `junodock-start`, `junodock-stop`, `junodock-dashboard`, etc. Run
`junodock-help` for all commands.
See [JunoDock](/install/junodock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export JUNO_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export JUNO_SANDBOX=1
    export JUNO_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm juno-cli gateway probe
    docker compose run -T --rm juno-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `juno-cli` uses `network_mode: "service:juno-gateway"` so CLI
    ...
    `no-new-privileges` on `juno-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.juno`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/juno-config /path/to/juno-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export JUNO_HOME_VOLUME="juno_home"`
    2. **Bake system deps**: `export JUNO_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `JUNO_HOME_VOLUME` or `JUNO_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/juno/juno/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Juno runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm juno-cli dashboard --no-open
    docker compose run --rm juno-cli devices list
    docker compose run --rm juno-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm juno-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm juno-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [JunoDock](/install/junodock) — Docker Compose community setup
- [Updating](/install/updating) — keeping Juno up to date
