---
summary: "Run Juno Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Juno running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Juno on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Juno Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Juno 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Juno Gateway in Docker
- Persist `~/.juno` + `~/.juno/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.juno` state includes `juno.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-juno-project --name="Juno Gateway"
    gcloud config set project my-juno-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create juno-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh juno-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Juno repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    JUNO_IMAGE=juno:latest
    JUNO_GATEWAY_TOKEN=
    JUNO_GATEWAY_BIND=lan
    JUNO_GATEWAY_PORT=18789

    JUNO_CONFIG_DIR=/home/$USER/.juno
    JUNO_WORKSPACE_DIR=/home/$USER/.juno/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.juno
    ```

    Leave `JUNO_GATEWAY_TOKEN` blank unless you explicitly want to
    manage it through `.env`; Juno writes a random gateway token to
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `JUNO_GATEWAY_TOKEN`.
    ...
    services:
      juno-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - JUNO_GATEWAY_BIND=${JUNO_GATEWAY_BIND}
          - JUNO_GATEWAY_PORT=${JUNO_GATEWAY_PORT}
          - JUNO_GATEWAY_TOKEN=${JUNO_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${JUNO_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`JUNO_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm juno-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh juno-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop juno-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type juno-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start juno-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create juno-deploy \
     --display-name="Juno Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-juno-project \
     --member="serviceAccount:juno-deploy@my-juno-project.iam.gserviceaccount.com" \
