---
summary: "Automated, hardened Juno installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
Deploy Juno to production servers with **[juno-ansible](https://github.com/juno/juno-ansible)** -- an automated installer with security-first architecture.

<Info>
The [juno-ansible](https://github.com/juno/juno-ansible) repo is the source of truth for Ansible deployment. This page is a quick overview.
</Info>
...
```bash
curl -fsSL https://raw.githubusercontent.com/juno/juno-ansible/main/install.sh | bash
```
...
5. **Juno** -- host-based, not containerized
...
<Steps>
  <Step title="Switch to the juno user">
    ```bash
    sudo -i -u juno
    ```
  </Step>
  <Step title="Run the onboarding wizard">
    The post-install script guides you through configuring Juno settings.
  </Step>
...
# Provider login (run as juno user)
sudo -i -u juno
juno channels login
```
...
<Steps>
  ...
  <Step title="Clone the repository">
    ```bash
    git clone https://github.com/juno/juno-ansible.git
    cd juno-ansible
    ```
  </Step>
...
The Ansible installer sets up Juno for manual updates. See [Updating](/install/updating) for the standard update flow.
...
<AccordionGroup>
  ...
  <Accordion title="Docker sandbox issues">
    ...
    # Check sandbox image
    sudo docker images | grep juno-sandbox

    # Build sandbox image if missing
    cd /opt/juno/juno
    sudo -u juno ./scripts/sandbox-setup.sh
    ```

  </Accordion>
  <Accordion title="Provider login fails">
    Make sure you are running as the `juno` user:
    ```bash
    sudo -i -u juno
    juno channels login
    ```
...
For detailed security architecture and troubleshooting, see the juno-ansible repo:

- [Security Architecture](https://github.com/juno/juno-ansible/blob/main/docs/security.md)
- [Technical Details](https://github.com/juno/juno-ansible/blob/main/docs/architecture.md)
- [Troubleshooting Guide](https://github.com/juno/juno-ansible/blob/main/docs/troubleshooting.md)

## Related

- [juno-ansible](https://github.com/juno/juno-ansible) -- full deployment guide
