---
summary: "Deploy Juno Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Juno on a Kubernetes cluster
  - You want to test Juno in a Kubernetes environment
title: "Kubernetes"
---

# Juno on Kubernetes

A minimal starting point for running Juno on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Juno is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/juno 18789:18789 -n juno
...
```bash
kubectl get secret juno-secrets -n juno -o jsonpath='{.data.JUNO_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: juno (configurable via JUNO_NAMESPACE)
├── Deployment/juno        # Single pod, init container + gateway
├── Service/juno           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/juno-config  # juno.json + AGENTS.md
└── Secret/juno-secrets    # Gateway token + API keys
```
...
Edit `juno.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret juno-secrets -n juno \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/juno -n juno
```

### Custom namespace

```bash
JUNO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/juno/juno:latest # or pin to a specific version from https://github.com/juno/juno/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # juno.json + AGENTS.md
