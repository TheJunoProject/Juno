---
summary: "Host Juno on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Juno on Oracle Cloud
  - Looking for free VPS hosting for Juno
...
Run a persistent Juno Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@juno`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=juno --reset` to re-authenticate.

**Gateway will not start** -- Run `juno doctor --non-interactive` and check logs with `journalctl --user -u juno-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Juno up to date
