---
summary: "Deploy Juno on Render with Infrastructure-as-Code"
read_when:
  - Deploying Juno to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Juno on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/juno/juno)
...
repository configures everything needed to run Juno:
...
services:
  - type: web
    ...
    envVars:
      - key: JUNO_GATEWAY_PORT
        value: "8080"
      - key: JUNO_STATE_DIR
        value: /data/.juno
      - key: JUNO_WORKSPACE_DIR
        value: /data/workspace
      - key: JUNO_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Juno state
...
`JUNO_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Juno repository, Render will not auto-deploy your Juno. To update it, run a manual Blueprint sync from the dashboard.
...
For Juno, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Juno state plus any configured
...
- Missing `JUNO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `JUNO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `juno backup create` in the Render shell.
