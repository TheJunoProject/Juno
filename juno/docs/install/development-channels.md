...
Juno ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/juno`, override with
  `JUNO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
juno update --tag 2026.4.1-beta.1
...
juno update --tag juno@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `juno update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Juno prompts for confirmation (skip with `--yes`).
...
Preview what `juno update` would do without making changes:

```bash
juno update --dry-run
juno update --channel beta --dry-run
juno update --tag 2026.4.1-beta.1 --dry-run
juno update --dry-run --json
```
...
When you switch channels with `juno update`, Juno also syncs plugin
sources:
