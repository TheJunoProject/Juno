---
summary: "Frequently asked questions about Juno setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/juno/juno-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   juno health --json
   juno health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://juno.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Juno **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/juno/juno/issues](https://github.com/juno/juno/issues)
    [https://github.com/juno/juno/pulls](https://github.com/juno/juno/pulls)
    ...
    - `juno status`: quick snapshot of gateway/agent health + basic config.
    - `juno models status`: checks provider auth + model availability.
    - `juno doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `juno status --all`, `juno logs --follow`,
    `juno gateway status`, `juno health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Juno">
    ...
    ```bash
    git clone https://github.com/juno/juno.git
    cd juno
    pnpm install
    pnpm build
    pnpm ui:build
    ...
    If you don't have a global install yet, run it via `pnpm juno onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `JUNO_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `JUNO_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `juno doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `juno gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `juno gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Juno now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Juno on the new machine.
    2. Copy `$JUNO_STATE_DIR` (default: `~/.juno`) from the old machine.
    3. Copy your workspace (default: `~/.juno/workspace`).
    4. Run `juno doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.juno/` (for example `~/.juno/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/juno/juno/blob/main/CHANGELOG.md](https://github.com/juno/juno/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.juno.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.juno.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.juno.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/juno/juno/tree/main/docs](https://github.com/juno/juno/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://juno.ai/install.ps1](https://juno.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://juno.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://juno.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://juno.ai/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://juno.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or juno not recognized">
    ...
    **2) juno is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Juno, track/report it in:

    - [Issue #30640](https://github.com/juno/juno/issues/30640)
  ...
  <Accordion title="How do I install Juno on Linux?">
  ...
  <Accordion title="How do I install Juno on a VPS?">
  ...
  <Accordion title="Can I ask Juno to update itself?">
    ...
    juno update
    juno update status
    juno update --channel stable|beta|dev
    juno update --tag <dist-tag|version>
    juno update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    juno update --yes --no-restart
    juno gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `juno onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Juno with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Juno, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Juno**: Anthropic staff
      told us this usage is allowed again, and Juno is treating `claude -p`
    ...
    Juno also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Juno-style Claude CLI usage is allowed again, so
    Juno treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Juno treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Juno token path, but Juno now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Juno, see [OpenAI](/providers/openai), [Qwen / Model
...
for usage/billing and raise limits as needed.
    ...
    Juno Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Juno can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. Juno has a bundled **Amazon Bedrock (Converse)** provider. With AWS env markers present, Juno can auto-discover the streaming/text Bedrock catalog and merge it as an implicit `amazon-bedrock` provider; otherwise you can explicitly enable `plugins.entries.amazon-bedrock.config.discovery.enabled` or add a manual provider entry. See [Amazon Bedrock](/providers/bedrock) and [Model providers](/providers/models). If you prefer a managed key flow, an OpenAI-compatible proxy in front of Bedrock is still a valid option.
  </Accordion>

  <Accordion title="How does Codex auth work?">
    Juno supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Why does ChatGPT GPT-5.4 not unlock openai/gpt-5.4 in Juno?">
    Juno treats the two routes separately:

    - `openai-codex/gpt-5.4` = ChatGPT/Codex OAuth
    - `openai/gpt-5.4` = direct OpenAI Platform API

    In Juno, ChatGPT/Codex sign-in is wired to the `openai-codex/*` route,
    not the direct `openai/*` route. If you want the direct API path in
    Juno, set `OPENAI_API_KEY` (or the equivalent OpenAI provider config).
    If you want ChatGPT/Codex sign-in in Juno, use `openai-codex/*`.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Juno can show the currently visible provider usage/quota windows in
    `juno models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Juno fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Juno. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `juno.json`.
    ...
    2. Enable the plugin: `juno plugins enable google`
    3. Login: `juno models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Juno needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** model build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Juno runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Juno, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `juno nodes status` / `juno nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Setup asks for numeric user IDs only. If you already have legacy `@username` entries in config, `juno doctor --fix` can try to resolve them.

    Safer (no third-party bot):

    - DM your bot, then run `juno logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Juno instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Juno via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Juno code install. Your state
    (`~/.juno`) and workspace (`~/.juno/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Juno-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Juno on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Juno is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Juno in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Juno, in one paragraph?">
  ...
  <Accordion title="Value proposition">
    Juno is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for Juno?">
  ...
  <Accordion title="Can Juno help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Juno draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Juno is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Juno when you
    ...
    Showcase: [https://juno.ai/showcase](https://juno.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.juno/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.juno/juno.json`). Precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.juno/skills` → bundled → `skills.load.extraDirs`, so managed overrides still win over bundled skills without touching git. If you need the skill installed globally but only visible to some agents, keep the shared copy in `~/.juno/skills` and control visibility with `agents.defaults.skills` and `agents.list[].skills`. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.juno/juno.json` (lowest precedence). Default precedence is `<workspace>/skills` → `<workspace>/.agents/skills` → `~/.agents/skills` → `~/.juno/skills` → bundled → `skills.load.extraDirs`. `junohub` installs into `./skills` by default, which Juno treats as `<workspace>/skills` on the next session. If the skill should only be visible to certain agents, pair that with `agents.defaults.skills` or `agents.list[].skills`.
  </Accordion>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Juno falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Juno intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    juno tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `JUNO_SKIP_CRON` is not set.
    ...
    ```bash
    juno cron run <jobId>
    juno cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `juno skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://junohub.ai](https://junohub.ai).

    ```bash
    juno skills search "calendar"
    juno skills search --limit 20
    juno skills install <skill-slug>
    juno skills install <skill-slug> --version <version>
    juno skills install <skill-slug> --force
    juno skills update --all
    juno skills list --eligible
    juno skills check
    ```

    Native `juno skills install` writes into the active workspace `skills/`
    directory. Install the separate `junohub` CLI only if you want to publish or
    sync your own skills. For shared installs across agents, put the skill under
    `~/.juno/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Juno run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.juno.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Juno can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.juno/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "juno": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.juno/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [JunoHub](/tools/junohub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Juno?">
    Use the built-in `user` browser profile, which attaches through Chrome DevTools MCP:

    ```bash
    juno browser --browser-profile user tabs
    juno browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `JUNO_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `JUNO_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Juno validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Juno memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Juno also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Juno auto-selects a provider when it
...
<AccordionGroup>
  <Accordion title="Is all data used with Juno saved locally?">
    No - **Juno's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.juno` + your workspace directory).
  ...
  <Accordion title="Where does Juno store its data?">
    Everything lives under `$JUNO_STATE_DIR` (default: `~/.juno`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$JUNO_STATE_DIR/juno.json`                             | Main config (JSON5)                                                |
    | `$JUNO_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$JUNO_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$JUNO_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$JUNO_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$JUNO_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$JUNO_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$JUNO_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$JUNO_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.juno/agent/*` (migrated by `juno doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.juno/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.juno`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.juno`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.juno/skills`).

    Default workspace is `~/.juno/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.juno/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.juno` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Juno?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Juno repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Juno reads an optional **JSON5** config from `$JUNO_CONFIG_PATH` (default: `~/.juno/juno.json`):

    ```
    $JUNO_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.juno/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `JUNO_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Juno enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `juno doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Juno.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `JUNO_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `juno configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Juno auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.juno/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Juno protects many accidental clobbers:

    - Juno-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Juno-owned writes are rejected and saved as `juno.json.rejected.*`.
    - If a direct edit breaks startup or hot reload, the Gateway restores the last-known-good config and saves the rejected file as `juno.json.clobbered.*`.
    - The main agent receives a boot warning after recovery so it does not blindly write the bad config again.

    Recover:

    - Check `juno logs --follow` for `Config auto-restored from last-known-good`, `Config write rejected:`, or `config reload restored last-known-good config`.
    - Inspect the newest `juno.json.clobbered.*` or `juno.json.rejected.*` beside the active config.
    - Keep the active restored config if it works, then copy only the intended keys back with `juno config set` or `config.patch`.
    - Run `juno config validate` and `juno doctor`.
    - If you have no last-known-good or rejected payload, restore from backup, or re-run `juno doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `juno config set` for small changes.
    - Use `juno configure` for interactive edits.
  ...
  <Accordion title="Can the Juno browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       juno devices list
       juno devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `juno gateway status`
    - Gateway health: `juno status`
    - Channel health: `juno channels status`
  ...
  <Accordion title="Can two Juno instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `juno agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    juno agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
...
<AccordionGroup>
  <Accordion title="How does Juno load environment variables?">
    Juno reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.juno/.env` (aka `$JUNO_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.juno/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `JUNO_LOAD_SHELL_ENV=1`, `JUNO_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `juno models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Juno won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.juno/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Juno instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Juno but keep it installed?">
    ...
    - If you used profiles (`--profile` / `JUNO_PROFILE`), reset each state dir (defaults are `~/.juno-<profile>`).
    - Dev reset: `juno gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Juno skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Juno runs on **your own account**, so if you're in the group, Juno can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    juno directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Juno not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.juno/agents/<agentId>/sessions/`.
    ...
    - Use `juno doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Juno's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.4`). If you omit the provider, Juno first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Juno falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `juno models set ...` (updates just model config)
    - `juno configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.juno/juno.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `juno doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `juno onboard` and choose `Ollama`
    ...
    - for manual switching, use `juno models list` and `juno models set ollama/<model>`
  ...
  <Accordion title="What do Juno, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `juno models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Juno release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Juno ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.juno/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `juno agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Juno can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Juno
    ...
    explicit billing text on `401` or `403`, Juno can still keep that in
    ...
    `organization spending limit exceeded`), Juno treats it as
    ...
    unknown/error in it". Juno does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.juno/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.juno/agent/*` (migrated by `juno doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.juno/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `juno models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `juno models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.juno/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        juno models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Juno will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Juno now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Juno uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Juno may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `juno models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    juno models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    juno models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    juno models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    juno models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    juno models status --probe
    ```
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > JUNO_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does juno gateway status say "Runtime: running" but "Connectivity probe: failed"?'>
    ...
    Use `juno gateway status` and trust these lines:
  ...
  <Accordion title='Why does juno gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `JUNO_STATE_DIR` mismatch).

    Fix:

    ```bash
    juno gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Juno enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `juno gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Juno in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `juno gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `juno dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `juno doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `JUNO_GATEWAY_TOKEN` or `gateway.auth.password` / `JUNO_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `juno devices list`
      - `juno devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `juno status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `JUNO_CONFIG_PATH` (per-instance config)
    - `JUNO_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `juno --profile <name> ...` per instance (auto-creates `~/.juno-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `juno --profile <name> gateway install`.

    Profiles also suffix service names (`ai.juno.<profile>`; legacy `com.juno.*`, `juno-gateway-<profile>.service`, `Juno Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    juno tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$JUNO_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.juno/logs/...`; profiles use `~/.juno-<profile>/logs/...`)
    - Linux: `journalctl --user -u juno-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Juno Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `juno gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Juno?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `juno gateway status`
    2. Is the Gateway healthy? `juno status`
    3. Does the UI have the right token? `juno dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    juno channels status
    juno channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Juno already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: juno gateway restart vs juno gateway">
    - `juno gateway restart`: restarts the **background service** (launchd/systemd).
    - `juno gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `juno gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Juno assistant setup](/start/juno) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    juno message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Juno to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `juno pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `juno pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `juno doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Juno only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    juno pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    juno pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Juno blocks **cross-provider** messaging by default. If a tool call is bound
