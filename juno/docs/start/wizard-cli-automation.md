---
summary: "Scripted onboarding and agent setup for the Juno CLI"
read_when:
...
Use `--non-interactive` to automate `juno onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Juno now prefers Claude CLI reuse when available.
