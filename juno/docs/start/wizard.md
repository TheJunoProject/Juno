...
CLI onboarding is the **recommended** way to set up Juno on macOS,
...
`juno dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`juno configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.juno/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `juno doctor` first.
</Note>
...
Use `juno agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.juno/workspace-<agentId>`.
...
- CLI command reference: [`juno onboard`](/cli/onboard)
