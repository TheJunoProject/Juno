---
summary: "Get Juno installed and run your first chat in minutes."
read_when:
...
Install Juno, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.juno/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.juno/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Juno as a service account or want custom paths:

- `JUNO_HOME` — home directory for internal path resolution
- `JUNO_STATE_DIR` — override the state directory
- `JUNO_CONFIG_PATH` — override the config file path
