...
read_when:
  ...
  - Reviewing Juno security posture
title: "Pairing"
---

# Pairing

“Pairing” is Juno’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `juno-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.juno/credentials/`:
...
3. On your phone, open the Juno iOS app → Settings → Gateway.
...
reconnects asking for more scopes or a broader role, Juno keeps the
existing approval as-is and creates a fresh pending upgrade request. Use
`juno devices list` to compare the currently approved access with the newly
requested access before you approve.

### Node pairing state storage

Stored under `~/.juno/devices/`:
...
- The legacy `node.pair.*` API (CLI: `juno nodes pending|approve|reject|rename`) is a
