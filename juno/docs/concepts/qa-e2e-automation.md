...
The private QA stack is meant to exercise Juno in a more realistic,
...
```bash
pnpm juno qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`JUNO_RUN_NODE_OUTPUT_LOG=<path>` to a repo-local log file.
...
disposable server. It requires `JUNO_QA_TELEGRAM_GROUP_ID`,
`JUNO_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`JUNO_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm juno qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Juno
...
- `mock-openai` is the scenario-aware Juno mock. It remains the default
