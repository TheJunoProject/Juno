...
summary: "How Juno remembers things across sessions"
read_when:
...
Juno remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.juno/workspace`).
...
<Info>
Juno auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Juno
...
```bash
juno memory rem-backfill --rollback
juno memory rem-backfill --rollback-short-term
```

## CLI

```bash
juno memory status          # Check index status and provider
juno memory search "query"  # Search from the command line
juno memory index --force   # Rebuild the index
```
