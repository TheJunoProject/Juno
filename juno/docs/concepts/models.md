...
Juno selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Juno can use (plus aliases).
...
- If you omit the provider, Juno resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Juno
...
juno models list
juno models status
juno models set <provider/model>
juno models set-image <provider/model>

juno models aliases list
juno models aliases add <alias> <provider/model>
juno models aliases remove <alias>

juno models fallbacks list
juno models fallbacks add <provider/model>
juno models fallbacks remove <provider/model>
juno models fallbacks clear

juno models image-fallbacks list
juno models image-fallbacks add <provider/model>
juno models image-fallbacks remove <provider/model>
juno models image-fallbacks clear
```

`juno models` (no subcommand) is a shortcut for `models status`.
...
`juno models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.juno/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Juno writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Juno regenerates `models.json`, including command-driven paths like `juno agent`.
