---
summary: "How Juno manages conversation sessions"
read_when:
...
Juno organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `juno security audit`.
...
Juno automatically bounds session storage over time. By default, it runs
...
Preview with `juno sessions cleanup --dry-run`.

## Inspecting sessions

- `juno status` -- session store path and recent activity.
- `juno sessions --json` -- all sessions (filter with `--active <minutes>`).
