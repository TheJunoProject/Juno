...
read_when:
  - You want to understand how Juno assembles model context
...
A **context engine** controls how Juno builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Juno ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.juno/juno.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Juno plugin. Install
...
juno plugins install -l ./my-context-engine
```
...
// juno.json
{
  plugins: {
    slots: {
      contextEngine: "lossless-juno", // must match the plugin's registered engine id
    },
    entries: {
      "lossless-juno": {
        enabled: true,
...
Every time Juno runs a model prompt, the context engine participates at
...
Juno currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Juno
...
The built-in `legacy` engine preserves Juno's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Juno uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Juno disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Juno automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `juno/plugin-sdk/core` to use
  Juno's built-in compaction behavior.
...
Juno resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Juno's built-in summarization. Plugin engines can implement
...
- Use `juno doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Juno
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `juno plugins install -l ./my-engine` to link a
