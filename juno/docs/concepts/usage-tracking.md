...
- `/usage cost` in chats: local cost summary aggregated from Juno session logs.
- CLI: `juno status --usage` prints a full per-provider breakdown.
- CLI: `juno channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Juno treats
  ...
  quota, so Juno inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Juno prefers the
...
can supply plugin-specific usage auth logic; otherwise Juno falls back to
