---
summary: "What the Juno system prompt contains and how it is assembled"
read_when:
...
Juno builds a custom system prompt for every agent run. The prompt is **Juno-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Juno and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Juno-owned prompt. The provider runtime can:
...
- **Juno Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Juno docs (repo or npm package) and when to read them.
...
Juno can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Juno
...
occurs, Juno can inject a warning block in Project Context; control this with
...
When eligible skills exist, Juno injects a compact **available skills list**
...
local Juno docs directory (either `docs/` in the repo workspace or the bundled npm
package docs) and also notes the public mirror, source repo, community Discord, and
JunoHub ([https://junohub.ai](https://junohub.ai)) for skills discovery. The prompt instructs the model to consult local docs first
for Juno behavior, commands, configuration, or architecture, and to run
`juno status` itself when possible (asking the user only when it lacks access).
