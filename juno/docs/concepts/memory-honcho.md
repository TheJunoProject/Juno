...
[Honcho](https://honcho.dev) adds AI-native memory to Juno. It persists
...
```bash
juno plugins install @honcho-ai/juno-honcho
juno honcho setup
juno gateway --force
```
...
Settings live under `plugins.entries["juno-honcho"].config`:
    ...
    entries: {
      "juno-honcho": {
        config: {
          ...
          workspaceId: "juno", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `juno honcho setup` detects and
...
```bash
juno honcho setup                        # Configure API key and migrate files
juno honcho status                       # Check connection status
juno honcho ask <question>               # Query Honcho about the user
juno honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/juno-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Juno integration guide](https://docs.honcho.dev/v3/guides/integrations/juno)
- [Memory](/concepts/memory) -- Juno memory overview
