...
Juno gives agents tools to work across sessions, inspect status, and
...
After the target responds, Juno can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Juno
...
the completion origin only identifies a channel Juno can still reuse the
