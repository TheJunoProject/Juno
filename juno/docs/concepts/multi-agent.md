...
- **Session store** (chat history + routing state) under `~/.juno/agents/<agentId>/sessions`.
...
`~/.juno/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.juno` (or `JUNO_STATE_DIR`)
- Workspace: `~/.juno/workspace` (or `~/.juno/workspace-<agentId>`)
- Agent dir: `~/.juno/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.juno/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Juno runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.juno/workspace` (or `~/.juno/workspace-<profile>` when `JUNO_PROFILE` is set).
- State defaults to `~/.juno/agents/main/agent`.
...
```bash
juno agents add coding
juno agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.juno/agents/<agentId>`.
...
```bash
juno channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.juno/workspace-alex" },
      { id: "mia", workspace: "~/.juno/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Juno upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Juno falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.juno/workspace-main" },
      { id: "coding", workspace: "~/.juno/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.juno/workspace-main" },
      { id: "alerts", workspace: "~/.juno/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
juno channels login --channel whatsapp --account personal
juno channels login --channel whatsapp --account biz
```

`~/.juno/juno.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.juno/workspace-home",
        agentDir: "~/.juno/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.juno/workspace-work",
        agentDir: "~/.juno/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.juno/credentials/whatsapp/personal
          // authDir: "~/.juno/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.juno/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.juno/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.juno/workspace-opus",
