...
With those enabled, Juno can show:
...
`config.promptOverride` replaces the default Active Memory prompt. Juno
...
5. Verify memory search itself works with `juno memory status --deep`.
...
If `memorySearch.provider` is unset, Juno auto-detects the first available
embedding provider.
...
- if an embedding provider is resolved and then fails at runtime, Juno does
...
- run `juno memory status --deep` to inspect the current memory-search
