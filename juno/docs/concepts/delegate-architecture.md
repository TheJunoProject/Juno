---
summary: "Delegate architecture: running Juno as a named agent on behalf of an organization"
...
Goal: run Juno as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Juno agent that:
...
Juno's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Juno's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.juno/workspace-delegate",
  tools: {
...
- Cron run history: `~/.juno/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.juno/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Juno's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
juno agents add delegate
```

This creates:

- Workspace: `~/.juno/workspace-delegate`
- State: `~/.juno/agents/delegate/agent`
- Sessions: `~/.juno/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.juno/workspace" },
      {
        id: "delegate",
        workspace: "~/.juno/workspace-delegate",
        tools: {
...
~/.juno/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.juno/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.juno/workspace-org",
        agentDir: "~/.juno/agents/org-assistant/agent",
...
recall view. Juno redacts credential/token-like text, truncates long
