---
summary: "CLI reference for `juno agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
juno agent --to +15555550123 --message "status update" --deliver
juno agent --agent ops --message "Summarize logs"
...
juno agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Juno persists markers from the active source config snapshot, not from resolved runtime secret values.
