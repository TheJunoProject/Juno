---
summary: "CLI reference for `juno logs` (tail gateway logs via RPC)"
read_when:
...
`juno logs` also accepts the standard Gateway client flags:
...
juno logs
juno logs --follow
juno logs --follow --interval 2000
juno logs --limit 500 --max-bytes 500000
juno logs --json
juno logs --plain
juno logs --no-color
juno logs --limit 500
juno logs --local-time
juno logs --follow --local-time
juno logs --url ws://127.0.0.1:18789 --token "$JUNO_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `juno logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
