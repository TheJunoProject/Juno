---
summary: "CLI reference for `juno cron` (schedule and run background jobs)"
read_when:
...
Tip: run `juno cron --help` for the full command surface.
...
Note: `juno cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `juno cron runs --id <job-id>` to follow the eventual outcome.

Note: `juno cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.juno/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`juno doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
juno cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
juno cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
juno cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
juno cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
juno cron run <job-id>
juno cron run <job-id> --due
juno cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
juno cron edit <job-id> --agent ops
juno cron edit <job-id> --clear-agent
juno cron edit <job-id> --session current
juno cron edit <job-id> --session "session:daily-brief"
```
...
```bash
juno cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
juno cron edit <job-id> --best-effort-deliver
juno cron edit <job-id> --no-best-effort-deliver
juno cron edit <job-id> --no-deliver
```
