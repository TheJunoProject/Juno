...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Juno Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`juno acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Juno channel
conversations instead of hosting an ACP harness session, use
[`juno mcp serve`](/cli/mcp) instead.
...
- Juno acts as an ACP server
- an IDE or ACP client connects to Juno
- Juno forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Juno runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Juno: use `juno acp`
- Juno should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Juno gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
juno acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
juno acp --url wss://gateway-host:18789 --token-file ~/.juno/gateway.token

# Attach to an existing session key
juno acp --session agent:main:main

# Attach by label (must already exist)
juno acp --session-label "support inbox"

# Reset the session key before the first prompt
juno acp --session agent:main:main --reset-session
```
...
juno acp client --server-args --url wss://gateway-host:18789 --token-file ~/.juno/gateway.token

# Override the server command (default: juno)
juno acp client --server "node" --server-args juno.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Juno through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Juno Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `juno acp` over stdio.

Example config (persisted):

```bash
juno config set gateway.remote.url wss://gateway-host:18789
juno config set gateway.remote.token <token>
```
...
```bash
juno acp --session agent:main:main
juno acp --session agent:design:main
juno acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Juno plugin tools, enable the
...
Juno bot over ACP, use `acpx` with its built-in `juno` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx juno` at `juno acp`.
3. Target the Juno session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Juno ACP session
acpx juno exec "Summarize the active Juno session state."

# Persistent named session for follow-up turns
acpx juno sessions ensure --name codex-bridge
acpx juno -s codex-bridge --cwd /path/to/repo \
  "Ask my Juno work agent for recent context relevant to this repo."
```

If you want `acpx juno` to target a specific Gateway and session key every
time, override the `juno` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "juno": {
      "command": "env JUNO_HIDE_BANNER=1 JUNO_SUPPRESS_NOTES=1 juno acp --url ws://127.0.0.1:18789 --token-file ~/.juno/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Juno checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env JUNO_HIDE_BANNER=1 JUNO_SUPPRESS_NOTES=1 node juno.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Juno agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Juno ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`JUNO_GATEWAY_TOKEN`, `JUNO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`JUNO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `JUNO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `juno acp client` sets `JUNO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `juno`).
