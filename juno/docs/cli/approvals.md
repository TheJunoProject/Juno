---
summary: "CLI reference for `juno approvals` and `juno exec-policy`"
read_when:
...
`juno exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.juno/exec-approvals.json`

Examples:

```bash
juno exec-policy show
juno exec-policy show --json

juno exec-policy preset yolo
juno exec-policy preset cautious --json

juno exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `juno exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `juno approvals set --gateway`
or `juno approvals set --node <id|name|ip>`.

## Common commands

```bash
juno approvals get
juno approvals get --node <id|name|ip>
juno approvals get --gateway
```

`juno approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
juno approvals set --file ./exec-approvals.json
juno approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
juno approvals set --node <id|name|ip> --file ./exec-approvals.json
juno approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Juno policy aligned, also set:
...
```bash
juno approvals allowlist add "~/Projects/**/bin/rg"
juno approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
juno approvals allowlist add --agent "*" "/usr/bin/uname"

juno approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `juno nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.juno/exec-approvals.json`.
