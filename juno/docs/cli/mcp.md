---
summary: "Expose Juno channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Juno-backed channels
  - Running `juno mcp serve`
  - Managing Juno-saved MCP server definitions
title: "mcp"
---

# mcp

`juno mcp` has two jobs:

- run Juno as an MCP server with `juno mcp serve`
- manage Juno-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Juno acting as an MCP server
- `list` / `show` / `set` / `unset` is Juno acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`juno acp`](/cli/acp) when Juno should host a coding harness
session itself and route that runtime through ACP.

## Juno as an MCP server

This is the `juno mcp serve` path.

## When to use `serve`

Use `juno mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Juno-backed channel conversations
- you already have a local or remote Juno Gateway with routed sessions
- you want one MCP server that works across Juno's channel backends instead
  of running separate per-channel bridges

Use [`juno acp`](/cli/acp) instead when Juno should host the coding
runtime itself and keep the agent session inside Juno.

## How it works

`juno mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Juno Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `juno mcp serve`
...
conversations. A conversation appears when Juno already has session state
...
# Remote Gateway
juno mcp serve --url wss://gateway-host:18789 --token-file ~/.juno/gateway.token

# Remote Gateway with password auth
juno mcp serve --url wss://gateway-host:18789 --password-file ~/.juno/gateway.password

# Enable verbose bridge logs
juno mcp serve --verbose

# Disable Claude-specific push notifications
juno mcp serve --claude-channel-mode off
```
...
Juno equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Juno channel configuration
...
Juno ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `juno mcp serve`
...
## Juno as an MCP client registry

This is the `juno mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Juno over MCP. They manage Juno-owned MCP
server definitions under `mcp.servers` in Juno config.

Those saved definitions are for runtimes that Juno launches or configures
later, such as embedded Pi and other runtime adapters. Juno stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Juno config
...
Juno also stores a lightweight MCP server registry in config for surfaces
that want Juno-managed MCP definitions.

Commands:

- `juno mcp list`
- `juno mcp show [name]`
- `juno mcp set <name> <json>`
- `juno mcp unset <name>`
...
```bash
juno mcp list
juno mcp show context7 --json
juno mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
juno mcp set docs '{"url":"https://mcp.example.com"}'
juno mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Juno uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
