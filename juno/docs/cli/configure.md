---
summary: "CLI reference for `juno configure` (interactive configuration prompts)"
read_when:
...
Tip: `juno config` without a subcommand opens the same wizard. Use
`juno config get|set|unset` for non-interactive edits.

For web search, `juno configure --section web` lets you choose a provider
...
juno configure --section model --section channels
juno configure --section gateway --section daemon
```
