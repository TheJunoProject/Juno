---
summary: "CLI reference for `juno dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: juno.internal)
title: "dns"
---
...
```bash
juno dns setup
juno dns setup --domain juno.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `juno.internal`)
...
- zone file path
- current tailnet IPs
- recommended `juno.json` discovery config
...
- If `--domain` is omitted, Juno uses `discovery.wideArea.domain` from config.
