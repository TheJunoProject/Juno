---
summary: "Juno CLI reference for `juno` commands, subcommands, and options"
read_when:
...
- [`junobot`](/cli/junobot) (legacy alias namespace)
- [`voicecall`](/cli/voicecall) (plugin; if installed)

## Global flags

- `--dev`: isolate state under `~/.juno-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.juno-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `juno update` (source installs only).
...
Juno uses a lobster palette for CLI output.
...
Note: plugins can add additional top-level commands (for example `juno voicecall`).

## Security

- `juno security audit` — audit config + local state for common security foot-guns.
- `juno security audit --deep` — best-effort live Gateway probe.
- `juno security audit --fix` — tighten safe defaults and state/config permissions.
...
- `juno plugins list` — discover plugins (use `--json` for machine output).
- `juno plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `juno plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `juno plugins marketplace list <marketplace>` — list marketplace entries before install.
- `juno plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `juno plugins doctor` — report plugin load errors.
...
- `juno memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `juno memory index` — reindex memory files.
- `juno memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `juno memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `Juno Completion` block into your shell profile and points it at the cached script under the Juno state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.juno/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `juno config` with no
...
- `config set --dry-run`: validate assignments without writing `juno.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `juno.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `juno --update` rewrites to `juno update`.

### `backup`

Create and verify local backup archives for Juno state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `juno health` or `juno status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `juno doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Juno promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
juno channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
juno channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
juno channels remove --channel discord --account work --delete
juno channels status --probe
juno status --deep
```

### `directory`

Look up self, peer, and group IDs for channels that expose a directory surface. See [`juno directory`](/cli/directory).
...
- `skills search [query...]`: search JunoHub skills.
- `skills search --limit <n> --json`: cap search results or emit machine-readable output.
- `skills install <slug>`: install a skill from JunoHub into the active workspace.
- `skills install <slug> --version <version>`: install a specific JunoHub version.
- `skills install <slug> --force`: overwrite an existing workspace skill folder.
- `skills update <slug|--all>`: update tracked JunoHub skills.
...
Tip: use `juno skills search`, `juno skills install`, and `juno skills update` for JunoHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`juno qr`](/cli/qr).
...
- After scanning, approve the request with `juno devices list` / `juno devices approve <requestId>`.

### `junobot`

Legacy alias namespace. Currently supports `juno junobot qr`, which maps to [`juno qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `juno plugins install`)
- `hooks update [id]` (deprecated alias for `juno plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `juno hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the Juno-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended Juno + Tailscale DNS config.
...
- `juno message send --target +15555550123 --message "Hi"`
- `juno message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Running `juno agents` with no subcommand is equivalent to `juno agents list`.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Juno may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose Juno channels over MCP stdio.

#### `mcp serve`

Expose routed Juno channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `juno nodes`
...
Juno can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `juno status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so Juno inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise Juno falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `juno tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Juno services are treated as first-class and aren't flagged as "extra".
...
Anthropic note: Anthropic staff told us Juno-style Claude CLI usage is
allowed again, so Juno treats Claude CLI reuse and `claude -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but Juno now prefers Claude CLI reuse and `claude -p` when available.

### `models` (root)

`juno models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported Juno token path, but Juno now prefers Claude CLI reuse and `claude -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `JUNO_GATEWAY_TOKEN` / `JUNO_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `JUNO_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`juno browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`juno voicecall`](/cli/voicecall).
...
Search the live Juno docs index.
