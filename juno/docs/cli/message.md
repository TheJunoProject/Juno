---
summary: "CLI reference for `juno message` (send + channel actions)"
read_when:
...
- On cache miss, Juno will attempt a live directory lookup when the provider supports it.
...
- `juno message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Juno does not force a `default` account SecretRef scope
...
```
juno message send --channel discord \
...
```bash
juno message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
juno message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
juno message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
juno message send --channel msteams \
...
```
juno message poll --channel msteams \
...
```
juno message react --channel slack \
...
```
juno message react --channel signal \
...
```
juno message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
juno message send --channel telegram --target @mychat \
