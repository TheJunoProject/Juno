---
summary: "CLI reference for `juno config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `juno.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `juno configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `juno config` without a subcommand
...
```bash
juno config file
juno config --section model
juno config --section gateway --section daemon
juno config schema
juno config get browser.executablePath
...
juno config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
juno config set secrets.providers.vaultfile --provider-source file --provider-path /etc/juno/secrets.json --provider-mode json
juno config unset plugins.entries.brave.config.webSearch.apiKey
juno config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
juno config validate
juno config validate --json
```

### `config schema`

Print the generated JSON schema for `juno.json` to stdout as JSON.
...
```bash
juno config schema > juno.schema.json
```
...
juno config get agents.list[0].id
```
...
juno config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
juno config set agents.defaults.heartbeat.every "0m"
juno config set gateway.port 19001 --strict-json
juno config set channels.whatsapp.groups '["*"]' --strict-json
```
...
`juno config set` supports four assignment styles:

1. Value mode: `juno config set <path> <value>`
2. SecretRef builder mode:

```bash
juno config set channels.discord.token \
...
```bash
juno config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/juno-vault \
  --provider-arg read \
...
```bash
juno config set --batch-file ./config-set.batch.json --dry-run
```
...
juno config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/juno/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `juno.json`.
...
`juno config set` and other Juno-owned config writers validate the full
...
and the rejected payload is saved beside it as `juno.json.rejected.*`.

Prefer CLI writes for small edits:

```bash
juno config set gateway.reload.mode hybrid --dry-run
juno config set gateway.reload.mode hybrid
juno config validate
```
...
- `config file`: Print the active config file path (resolved from `JUNO_CONFIG_PATH` or default location).
