---
summary: "CLI reference for `juno wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `juno wiki`
title: "wiki"
---
...
Use `juno wiki` when you want a compiled knowledge vault with:
...
- `.juno-wiki/cache/agent-digest.json`
- `.juno-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `juno memory search` when the
...
```bash
juno wiki get entity.alpha
juno wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`juno wiki` behavior is shaped by:
