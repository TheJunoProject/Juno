---
summary: "CLI reference for `juno qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
juno qr
juno qr --setup-code-only
juno qr --json
juno qr --remote
juno qr --url wss://gateway.example/ws
```
...
- With `--remote`, Juno requires either `gateway.remote.url` or
