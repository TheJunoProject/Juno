---
summary: "CLI reference for `juno update` (safe-ish source update + gateway auto-restart)"
read_when:
...
Safely update Juno and switch between stable/beta/dev channels.
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:juno/juno#main`.
...
When you switch channels explicitly (`--channel ...`), Juno also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/juno`, override with `JUNO_GIT_DIR`),
...
8. Runs `juno doctor` as the final “safe update” check.
...
`juno --update` rewrites to `juno update` (useful for shells and launcher scripts).

## See also

- `juno doctor` (offers to run update first on git checkouts)
