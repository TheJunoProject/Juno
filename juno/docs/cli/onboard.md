---
summary: "CLI reference for `juno onboard` (interactive onboarding)"
read_when:
...
```bash
juno onboard
juno onboard --flow quickstart
juno onboard --flow manual
juno onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`JUNO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `juno gateway run`.
...
- Fastest first chat: `juno dashboard` (Control UI, no channel setup).
