---
summary: "CLI reference for `juno plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
juno plugins list
juno plugins list --enabled
juno plugins list --verbose
juno plugins list --json
juno plugins install <path-or-spec>
juno plugins inspect <id>
juno plugins inspect <id> --json
juno plugins inspect --all
juno plugins info <id>
juno plugins enable <id>
juno plugins disable <id>
juno plugins uninstall <id>
juno plugins doctor
juno plugins update <id>
juno plugins update --all
juno plugins marketplace list <marketplace>
juno plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Juno. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Juno plugins must ship `juno.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: juno` or `Format: bundle`. Verbose list/info
...
```bash
juno plugins install <package>                      # JunoHub first, then npm
juno plugins install junohub:<package>              # JunoHub only
juno plugins install <package> --force              # overwrite existing install
juno plugins install <package> --pin                # pin version
juno plugins install <package> --dangerously-force-unsafe-install
juno plugins install <path>                         # local path
juno plugins install <plugin>@<marketplace>         # marketplace
juno plugins install <plugin> --marketplace <name>  # marketplace (explicit)
juno plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```

Bare package names are checked against JunoHub first, then npm. Security note:
...
run `juno doctor --fix` first. The only documented exception is a narrow
...
the same id from a new local path, archive, JunoHub package, or npm artifact.
...
override, while `juno skills install` remains a separate JunoHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`juno.hooks` in `package.json`. Use `juno hooks` for filtered hook
...
those to a prerelease, Juno stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), Juno
...
JunoHub installs use an explicit `junohub:<package>` locator:

```bash
juno plugins install junohub:juno-codex-app-server
juno plugins install junohub:juno-codex-app-server@1.2.3
```

Juno now also prefers JunoHub for bare npm-safe plugin specs. It only falls
back to npm if JunoHub does not have that package or version:

```bash
juno plugins install juno-codex-app-server
```

Juno downloads the package archive from JunoHub, checks the advertised
plugin API / minimum gateway compatibility, then installs it through the normal
archive path. Recorded installs keep their JunoHub source metadata for later
updates.
...
```bash
juno plugins marketplace list <marketplace-name>
juno plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
juno plugins install <plugin-name> --marketplace <marketplace-name>
juno plugins install <plugin-name> --marketplace <owner/repo>
juno plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
juno plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Juno accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Juno auto-detects:

- native Juno plugins (`juno.plugin.json`)
...
```bash
juno plugins install -l ./my-plugin
```
...
```bash
juno plugins uninstall <id>
juno plugins uninstall <id> --dry-run
...
```bash
juno plugins update <id-or-npm-spec>
juno plugins update --all
juno plugins update <id-or-npm-spec> --dry-run
juno plugins update @juno/voice-call@beta
juno plugins update juno-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Juno reuses the recorded install spec for that
...
or exact version. Juno resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
Juno prints a warning and asks for confirmation before proceeding. Use
