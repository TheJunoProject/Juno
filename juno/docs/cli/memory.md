---
summary: "CLI reference for `juno memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
juno memory status
juno memory status --deep
juno memory status --fix
juno memory index --force
juno memory search "meeting notes"
juno memory search --query "deployment" --max-results 20
juno memory promote --limit 10 --min-score 0.75
juno memory promote --apply
juno memory promote --json --min-recall-count 0 --min-unique-queries 0
juno memory promote-explain "router vlan"
juno memory promote-explain "router vlan" --json
juno memory rem-harness
juno memory rem-harness --json
juno memory status --json
juno memory status --deep --index
juno memory status --deep --index --verbose
juno memory status --agent main
juno memory index --agent main --verbose
```
...
```bash
juno memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `juno cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
juno memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
juno memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
