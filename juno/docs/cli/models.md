---
summary: "CLI reference for `juno models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
juno models status
juno models list
juno models set <model-or-alias>
juno models scan
```

`juno models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Juno falls back to matching OAuth/API-key
...
the command uses `JUNO_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, Juno resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `JUNO_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Juno could not resolve a probeable
...
```bash
juno models auth add
juno models auth login --provider <id>
juno models auth setup-token --provider <id>
juno models auth paste-token
```
...
`juno plugins list` to see which providers are installed.

Examples:

```bash
juno models auth login --provider openai-codex --set-default
```
