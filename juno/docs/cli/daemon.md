---
summary: "CLI reference for `juno daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `juno daemon ...` in scripts
...
`juno daemon ...` maps to the same service control surface as `juno gateway ...` service commands.
...
Use [`juno gateway`](/cli/gateway) for current docs and examples.
