---
summary: "CLI reference for `juno secrets` (reload, audit, configure, apply)"
read_when:
...
Use `juno secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
juno secrets reload
juno secrets reload --json
juno secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `juno.json` refs)
...
juno secrets audit
juno secrets audit --check
juno secrets audit --json
juno secrets audit --allow-exec
```
...
```bash
juno secrets configure
juno secrets configure --plan-out /tmp/juno-secrets-plan.json
juno secrets configure --apply --yes
juno secrets configure --providers-only
juno secrets configure --skip-provider-setup
juno secrets configure --agent ops
juno secrets configure --json
```
...
- `configure` targets secret-bearing fields in `juno.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Juno fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
juno secrets apply --from /tmp/juno-secrets-plan.json --json
```
...
- `juno.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.juno/.env` known secret keys whose values were migrated
