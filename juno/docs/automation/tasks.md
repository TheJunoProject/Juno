...
- `juno tasks list` shows all tasks; `juno tasks audit` surfaces issues.
...
juno tasks list --runtime acp
juno tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
juno tasks show <lookup>

# Cancel a running task (kills the child session)
juno tasks cancel <lookup>

# Change notification policy for a task
juno tasks notify <lookup> state_changes

# Run a health audit
juno tasks audit

# Preview or apply maintenance
juno tasks maintenance
juno tasks maintenance --apply

# Inspect TaskFlow state
juno tasks flow list
...
| CLI operations         | `cli`        | `juno agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `juno tasks cancel`                        |
...
When a task reaches a terminal state, Juno notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Juno also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
juno tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
juno tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
juno tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `juno status` when issues are detected.
...
```bash
juno tasks maintenance [--json]
juno tasks maintenance --apply [--json]
```
...
```bash
juno tasks flow list [--status <status>] [--json]
juno tasks flow show <lookup> [--json]
juno tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `juno tasks list`.

## Status integration (task pressure)

`juno status` includes an at-a-glance task summary:
...
```
$JUNO_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `juno tasks` to inspect individual task records and `juno tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.juno/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
