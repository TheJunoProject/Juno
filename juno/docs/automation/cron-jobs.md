...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into Juno
...
# See run history
juno cron runs --id <job-id>
```

## How cron works

- Cron runs **inside the Gateway** process (not inside the model).
- Jobs persist at `~/.juno/cron/jobs.json` so restarts do not lose schedules.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, Juno re-prompts once for the actual
...
This fires ~5–6 times per month instead of 0–1 times per month. Juno uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
running, Juno suppresses that partial parent update instead of announcing it.
...
Wire Gmail inbox triggers to Juno via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), Juno hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
juno webhooks gmail setup --account juno@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `JUNO_SKIP_GMAIL_WATCHER=1` to opt out.
...
# Edit a job
juno cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
juno cron run <jobId>

# Run only if due
juno cron run <jobId> --due

# View run history
juno cron runs --id <jobId> --limit 50

# Delete a job
juno cron remove <jobId>

# Agent selection (multi-agent setups)
juno cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
juno cron edit <jobId> --clear-agent
```

Model override note:

- `juno cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `JUNO_SKIP_CRON=1`.
...
- Check `cron.enabled` and `JUNO_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `juno cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  Juno suppresses direct outbound delivery and also suppresses the fallback
