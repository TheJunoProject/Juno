...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `juno hooks`.

There are two kinds of hooks in Juno:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in Juno. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `juno hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "juno": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
1. **Bundled hooks**: shipped with Juno
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.juno/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `juno.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.juno/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
juno hooks enable <hook-name>
```
...
Logs every slash command to `~/.juno/logs/commands.log`.
...
1. Verify the hook is enabled: `juno hooks list`
2. Restart your gateway process so hooks reload.
3. Check gateway logs: `./scripts/junolog.sh | grep hook`
