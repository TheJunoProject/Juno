...
summary: "Architecture of Juno's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Juno
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Juno integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Juno uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Juno directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Juno's local TUI mode)                                             |
...
├── pi-tools.ts                    # createJunoCodingTools()
...
├── juno-tools.ts              # Juno-specific tools
...
Image injection is prompt-local: Juno loads image refs from the current prompt and
...
2. **Custom Replacements**: Juno replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Juno Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Juno's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Juno CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Juno wraps this with `guardSessionManager()` for tool result safety.
...
Juno maintains an auth profile store with multiple API keys per provider:
...
Juno loads custom pi extensions for specialized behavior:
...
Juno also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Juno Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Juno tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.juno/agents/<agentId>/sessions/` (or `$JUNO_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `JUNO_LIVE_TEST=1`)
