...
An **agent harness** is the low level executor for one prepared Juno agent
...
runtime and the normal Juno provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Juno
  session transcript
...
Before a harness is selected, Juno has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Juno transcript/session file
...
**Import:** `juno/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "juno/plugin-sdk/agent-harness";
...
Juno chooses a harness after provider/model resolution:

1. `JUNO_AGENT_RUNTIME=<id>` forces a registered harness with that id.
2. `JUNO_AGENT_RUNTIME=pi` forces the built-in PI harness.
3. `JUNO_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
4. If no registered harness matches, Juno uses PI unless PI fallback is
   disabled.

Forced plugin harness failures surface as run failures. In `auto` mode,
Juno may fall back to PI when the selected plugin harness fails before a
turn has produced side effects. Set `JUNO_AGENT_HARNESS_FALLBACK=none` or
...
Juno. The harness then claims that provider in `supports(...)`.
...
- app-server request: Juno sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs remain OpenAI provider
refs and continue to use the normal Juno provider path. Select `codex/gpt-*`
...
Juno requires Codex app-server `0.118.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Juno only runs against the protocol surface it has been tested with.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded Juno
agent turns. Enable the bundled `codex` plugin first, and include `codex` in
`plugins.allow` if your config uses a restrictive allowlist. It is different
from `openai-codex/*`:

- `openai-codex/*` uses ChatGPT/Codex OAuth through the normal Juno provider
  path.
- `codex/*` uses the bundled Codex provider and routes the turn through Codex
  app-server.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Juno still owns the chat channel,
...
By default, Juno runs embedded agents with `agents.defaults.embeddedHarness`
...
plugin harness fails before producing output, Juno falls back to PI.
...
an explicit `runtime: "pi"` or `JUNO_AGENT_RUNTIME=pi`.
...
want Juno to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`JUNO_AGENT_RUNTIME` still overrides the configured runtime. Use
`JUNO_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
JUNO_AGENT_RUNTIME=codex \
JUNO_AGENT_HARNESS_FALLBACK=none \
juno gateway run
```
...
Keep that binding explicitly associated with the Juno session, and keep
mirroring user-visible assistant/tool output into the Juno transcript.

The Juno transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Juno can
clear it when the owning Juno session is reset.

## Tool and media results

Core constructs the Juno tool list and passes it into the prepared attempt.
