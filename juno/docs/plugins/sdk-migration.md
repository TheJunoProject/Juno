...
read_when:
  - You see the JUNO_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the JUNO_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Juno plugin
---

# Plugin SDK Migration

Juno has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`juno/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`juno/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`juno/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `juno/plugin-sdk/slack`, `juno/plugin-sdk/discord`,
`juno/plugin-sdk/signal`, `juno/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`juno/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `juno/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `juno/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "juno/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "juno/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "juno/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "juno/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "juno/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "juno/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `JunoSchema` |
...
```bash
JUNO_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 juno gateway run
JUNO_SUPPRESS_EXTENSION_API_WARNING=1 juno gateway run
```
