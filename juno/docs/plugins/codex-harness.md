---
title: "Codex Harness"
summary: "Run Juno embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets Juno run embedded agent turns through the
...
Juno still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `JUNO_AGENT_RUNTIME=codex`.
...
Juno has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through Juno/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- Juno with the bundled `codex` plugin available.
...
Juno on the protocol surface it has been tested against.
...
With fallback disabled, Juno fails early if the Codex plugin is disabled,
...
Juno session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Juno session binding for that thread.
...
- `JUNO_CODEX_APP_SERVER_BIN`
- `JUNO_CODEX_APP_SERVER_ARGS`
- `JUNO_CODEX_APP_SERVER_APPROVAL_POLICY`
- `JUNO_CODEX_APP_SERVER_SANDBOX`
- `JUNO_CODEX_APP_SERVER_GUARDIAN=1`
...
Model switching stays Juno-controlled. When an Juno session is attached
...
generic and works on any channel that supports Juno text commands.
...
- `/codex resume <thread-id>` attaches the current Juno session to an existing Codex thread.
...
normal turns. On the next message, Juno resumes that Codex thread, passes the
currently selected Juno `codex/*` model into app-server, and keeps extended
history enabled.
...
Juno still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Juno delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. Juno keeps a transcript mirror for channel
...
**Juno falls back to PI:** set `embeddedHarness.fallback: "none"` or
`JUNO_AGENT_HARNESS_FALLBACK=none` while testing.
