...
read_when:
  ...
  - You are defining plugin config schemas or package.json juno metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`juno.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `juno` field that tells the plugin system what
...
**Provider plugin / JunoHub publish baseline:**

```json juno-junohub-package.json
{
...
If you publish the plugin externally on JunoHub, those `compat` and `build`
...
`juno.channel` is cheap package metadata for channel discovery and setup
...
`juno.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Juno version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `juno doctor --fix`.
...
When enabled, Juno loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `juno.plugin.json` in the package root.
Juno uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Juno",
  "configSchema": {
    "type": "object",
...
For plugin packages, use the package-specific JunoHub command:
...
always use `junohub package publish`.

## Setup entry

The `setup-entry.ts` file is a lightweight alternative to `index.ts` that
Juno loads when it only needs setup surfaces (onboarding, config repair,
...
`juno/plugin-sdk/channel-entry-contract` instead of
...
**When Juno uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `juno/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Juno validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "juno/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `juno onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "juno/plugin-sdk/channel-setup";
...
helpers from `juno/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`juno/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `juno/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "juno/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/juno-my-channel",
  docsPath: "/channels/my-channel",
});
...
**External plugins:** publish to [JunoHub](/tools/junohub) or npm, then install:

```bash
juno plugins install @myorg/juno-my-plugin
```

Juno tries JunoHub first and falls back to npm automatically. You can also
force JunoHub explicitly:

```bash
juno plugins install junohub:@myorg/juno-my-plugin   # JunoHub only
```

There is no matching `npm:` override. Use the normal npm package spec when you
want the npm path after JunoHub fallback:
...
<Info>
  For npm-sourced installs, `juno plugins install` runs
