...
read_when:
  - Building or debugging native Juno plugins
...
This page covers the internal architecture of the Juno plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Juno. Every
native Juno plugin registers against one or more capability types:
...
Juno classifies every loaded plugin into a shape based on its actual
...
Use `juno plugins inspect <id>` to see a plugin's shape and capability
...
When you run `juno doctor` or `juno plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `juno status --all` and `juno plugins doctor`.

## Architecture overview

Juno's plugin system has four layers:

1. **Manifest + discovery**
   Juno finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `juno.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Juno plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Juno reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Juno
...
That split lets Juno validate config, explain missing/disabled plugins, and
...
normal chat actions. Juno keeps one shared `message` tool in core, and
...
Juno treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Juno-facing
  surfaces
...
So if Juno adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Juno has shared
...
```ts
import type { JunoPluginDefinition } from "juno/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "juno/plugin-sdk/media-understanding";

const plugin: JunoPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Juno already treats image/audio/video understanding as one shared
...
`JunoPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Juno can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Juno knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `JunoPluginApi` or
  `api.runtime`
...
Native Juno plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Juno currently treats them
...
name: `@juno/<id>` by default, or an approved typed suffix such as
...
Juno exports capabilities, not implementation convenience.
...
At startup, Juno does roughly this:
...
The manifest is the control-plane source of truth. Juno uses it to:
...
Juno keeps short in-process caches for:
...
- Set `JUNO_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `JUNO_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `JUNO_PLUGIN_DISCOVERY_CACHE_MS` and
  `JUNO_PLUGIN_MANIFEST_CACHE_MS`.
...
Juno still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Juno calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Juno tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Juno's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Juno's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Juno adds those
...
- Juno only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Juno plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-juno-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-juno-scopes` only when the header is explicitly present
  - if `x-juno-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-juno-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `juno/plugin-sdk` import when
authoring plugins:

- `juno/plugin-sdk/plugin-entry` for plugin registration primitives.
- `juno/plugin-sdk/core` for the generic shared plugin-facing contract.
- `juno/plugin-sdk/config-schema` for the root `juno.json` Zod schema
  export (`JunoSchema`).
- Stable channel primitives such as `juno/plugin-sdk/channel-setup`,
  `juno/plugin-sdk/setup-runtime`,
  `juno/plugin-sdk/setup-adapter-runtime`,
  `juno/plugin-sdk/setup-tools`,
  `juno/plugin-sdk/channel-pairing`,
  `juno/plugin-sdk/channel-contract`,
  `juno/plugin-sdk/channel-feedback`,
  `juno/plugin-sdk/channel-inbound`,
  `juno/plugin-sdk/channel-lifecycle`,
  `juno/plugin-sdk/channel-reply-pipeline`,
  `juno/plugin-sdk/command-auth`,
  `juno/plugin-sdk/secret-input`, and
  `juno/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `juno/plugin-sdk/channel-config-helpers`,
  `juno/plugin-sdk/allow-from`,
  `juno/plugin-sdk/channel-config-schema`,
  `juno/plugin-sdk/telegram-command-config`,
  `juno/plugin-sdk/channel-policy`,
  `juno/plugin-sdk/approval-gateway-runtime`,
  `juno/plugin-sdk/approval-handler-adapter-runtime`,
  `juno/plugin-sdk/approval-handler-runtime`,
  `juno/plugin-sdk/approval-runtime`,
  `juno/plugin-sdk/config-runtime`,
  `juno/plugin-sdk/infra-runtime`,
  `juno/plugin-sdk/agent-runtime`,
  `juno/plugin-sdk/lazy-runtime`,
  `juno/plugin-sdk/reply-history`,
  `juno/plugin-sdk/routing`,
  `juno/plugin-sdk/status-helpers`,
  `juno/plugin-sdk/text-runtime`,
  `juno/plugin-sdk/runtime-store`, and
  `juno/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `juno/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `juno/plugin-sdk/*` subpaths. Juno core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Juno is not yet serving a runtime snapshot.
...
- Avoid the root `juno/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `juno/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `juno/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `juno/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `juno/plugin-sdk/channel-targets`; channel-specific
...
`juno/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Juno writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Juno's
...
- if both `catalog` and `discovery` are registered, Juno uses `catalog`
...
- Read-only command paths such as `juno status`, `juno status --all`,
  `juno channels status`, `juno channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `juno.extensions`:
...
Security guardrail: every `juno.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `juno plugins install` installs plugin dependencies with
...
Optional: `juno.setupEntry` can point at a lightweight setup-only module.
When Juno needs setup surfaces for a disabled channel plugin, or
...
Optional: `juno.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Juno load the
...
Channel plugins can advertise setup/discovery metadata via `juno.channel` and
install hints via `juno.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@juno/nextcloud-talk",
  "juno": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `juno.channel` fields beyond the minimal example:
...
Juno can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.juno/mpm/plugins.json`
- `~/.juno/mpm/catalog.json`
- `~/.juno/plugins/catalog.json`

Or point `JUNO_PLUGIN_CATALOG_PATHS` (or `JUNO_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "juno": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "juno/plugin-sdk/core";

export default function (api) {
  api.registerContextEngine("lossless-juno", () => ({
    info: { id: "lossless-juno", name: "Lossless Juno", ownsCompaction: true },
    async ingest() {
...
2. add typed plugin registration/runtime surfaces
   Extend `JunoPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Juno stays opinionated without becoming hardcoded to one
