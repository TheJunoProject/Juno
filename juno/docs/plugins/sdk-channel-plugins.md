...
summary: "Step-by-step guide to building a messaging channel plugin for Juno"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Juno to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Juno to a
...
<Info>
  If you have not built any Juno plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Juno keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `juno/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `juno/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `juno/plugin-sdk/approval-auth-runtime`
- `juno/plugin-sdk/approval-client-runtime`
- `juno/plugin-sdk/approval-delivery-runtime`
...
- `juno/plugin-sdk/approval-native-runtime`
- `juno/plugin-sdk/approval-reply-runtime`
- `juno/plugin-sdk/channel-runtime-context`

Likewise, prefer `juno/plugin-sdk/setup-runtime`,
`juno/plugin-sdk/setup-adapter-runtime`,
`juno/plugin-sdk/reply-runtime`,
`juno/plugin-sdk/reply-dispatch-runtime`,
`juno/plugin-sdk/reply-reference`, and
`juno/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `juno/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `juno/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `juno/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `juno/plugin-sdk/setup` seam only when you also need the
...
- `juno/plugin-sdk/account-core`,
  `juno/plugin-sdk/account-id`,
  `juno/plugin-sdk/account-resolution`, and
  `juno/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `juno/plugin-sdk/inbound-envelope` and
  `juno/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `juno/plugin-sdk/messaging-targets` for target parsing/matching
- `juno/plugin-sdk/outbound-media` and
  `juno/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `juno/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `juno/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `juno/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `juno/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `juno/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
} from "juno/plugin-sdk/channel-inbound";
...
`juno/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`juno/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#juno-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/juno-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "juno": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Juno to Acme Chat."
        }
      }
    }
    ```

    ```json juno.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    } from "juno/plugin-sdk/channel-core";
    import type { JunoConfig } from "juno/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "juno/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Juno
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "juno/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Juno loads this instead of the full entry when the channel is disabled
    ...
    `juno/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Juno. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Juno.
...
├── package.json              # juno.channel metadata
├── juno.plugin.json      # Manifest with config schema
