...
**Import:** `juno/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `JunoPluginConfigSchema \| () => JunoPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: JunoPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `juno.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Juno resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `juno/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: JunoPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: JunoPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Juno
...
Juno loads this instead of the full entry when a channel is disabled,
...
- `juno/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `juno/plugin-sdk/channel-setup` for optional-install setup surfaces
- `juno/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`juno/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "juno/plugin-sdk/channel-entry-contract";
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Juno to lazy-load the real CLI module on first invocation
...
Juno classifies loaded plugins by their registration behavior:
...
Use `juno plugins inspect <id>` to see a plugin's shape.
