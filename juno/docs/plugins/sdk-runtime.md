...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal Juno
...
Bind a Task Flow runtime to an existing Juno session key or trusted tool
...
trusted Juno session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Juno plugin SDK", count: 5 },
});
```
