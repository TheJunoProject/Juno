...
read_when:
  - You are building an Juno plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (juno.plugin.json)

This page is for the **native Juno plugin manifest** only.
...
Juno auto-detects those bundle layouts too, but they are not validated
against the `juno.plugin.json` schema described here.

For compatible bundles, Juno currently reads bundle metadata plus declared
...
Juno runtime expectations.

Every native Juno plugin **must** ship a `juno.plugin.json` file in the
**plugin root**. Juno uses this manifest to validate configuration
...
`juno.plugin.json` is the metadata Juno reads before it loads your
plugin code.
...
- static capability ownership snapshots used for bundled compat wiring and
  contract coverage
- cheap QA runner metadata that the shared `juno qa` host can inspect
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Juno can inspect without loading plugin code.                                                                                                                      |
| `providerAuthAliases`               | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                   |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that Juno can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
| `qaRunners`                         | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `juno qa` host before plugin runtime loads.                                                                                                               |
...
Juno reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Juno falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Juno
...
the shared `juno qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `juno qa`, for example `matrix`.    |
...
Use `contracts` only for static capability ownership metadata that Juno can
...
Use `modelSupport` when Juno should infer your provider plugin from
...
Legacy top-level capability keys are deprecated. Use `juno doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `juno.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `juno` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Juno must know it before loading plugin code, put it in `juno.plugin.json`
...
`juno` block instead of `juno.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `juno.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `juno.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `juno.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `juno.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `juno.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `juno.install.npmSpec` / `juno.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `juno.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `juno.install.minHostVersion`                                 | Minimum supported Juno host version, using a semver floor like `>=2026.3.22`.                                                            |
| `juno.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `juno.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |

`juno.install.minHostVersion` is enforced during install and manifest
registry loading. Invalid values are rejected; newer-but-valid values skip the
plugin on older hosts.

`juno.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`juno.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`juno.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
- The manifest is **required for native Juno plugins**, including local filesystem loads.
