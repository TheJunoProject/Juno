...
summary: "Step-by-step guide to building a model provider plugin for Juno"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Juno
...
(LLM) to Juno. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to Juno's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Juno can detect
    ...
    is optional and lets Juno auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on JunoHub, those `juno.compat` and `juno.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "juno/plugin-sdk/provider-auth";
    ...
    `juno onboard --acme-ai-api-key <key>` and select
    `acme-ai/acme-large` as their model.

    If the upstream provider uses different control tokens than Juno, add a
    ...
    Juno parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "juno/plugin-sdk/provider-entry";
    ...
    `juno/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `juno/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "juno/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "juno/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "juno/plugin-sdk/provider-tools";
    ...
    `juno/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `juno/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@juno/anthropic-provider` exports
    ...
    `juno/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@juno/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@juno/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      Juno calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Juno classifies this as a **hybrid-capability** plugin. This is the
...
`junohub package publish`.

## File structure

```
<bundled-plugin-root>/acme-ai/
├── package.json              # juno.providers metadata
├── juno.plugin.json      # Manifest with provider auth metadata
