...
read_when:
  ...
  - You want a reference for all registration methods on JunoPluginApi
...
prefer `juno/plugin-sdk/channel-core`; keep `juno/plugin-sdk/core` for
...
`juno/plugin-sdk/slack`, `juno/plugin-sdk/discord`,
`juno/plugin-sdk/signal`, `juno/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `JunoSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `juno.json` Zod schema export (`JunoSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Juno tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `JunoPluginApi` object with these
methods:
...
- User config still wins. Juno merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `juno/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `JunoConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `juno/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Juno is already running. If no runtime
...
- `@juno/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `juno/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `juno/plugin-sdk/speech`, `.../provider-model-shared`, or another
