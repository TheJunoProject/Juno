...
read_when:
  - You want to place an outbound voice call from Juno
...
Voice calls for Juno via a plugin. Supports outbound notifications and
...
- Use `juno voicecall ...` or the `voice_call` tool
...
juno plugins install "$PLUGIN_SRC"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `juno doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `juno doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `juno doctor --fix`:
...
```bash
juno voicecall call --to "+15555550123" --message "Hello from Juno"
juno voicecall start --to "+15555550123"   # alias for call
juno voicecall continue --call-id <id> --message "Any questions?"
juno voicecall speak --call-id <id> --message "One moment"
juno voicecall end --call-id <id>
juno voicecall status --call-id <id>
juno voicecall tail
juno voicecall latency                     # summarize turn latency from logs
