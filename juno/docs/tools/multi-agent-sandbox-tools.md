...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `juno sandbox explain`.
...
Legacy `agent.*` configs are migrated by `juno doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=juno-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${JUNO_STATE_DIR:-$HOME/.juno}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
