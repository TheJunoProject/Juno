...
- `commands.config` (default `false`) enables `/config` (reads/writes `juno.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes Juno-managed MCP config under `mcp.servers`).
...
- `/config show|get|set|unset` reads or writes `juno.json`. Owner-only. Requires `commands.config: true`.
- `/mcp show|get|set|unset` reads or writes Juno-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
...
- `/restart` restarts Juno when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
- For full provider usage breakdown, use `juno status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Juno session logs.
- `/restart` is enabled by default; set `commands.restart: false` to disable it.
- `/plugins install <spec>` accepts the same plugin specs as `juno plugins install`: local path/archive, npm package, or `junohub:<pkg>`.
...
- If a run is already active, Juno marks a live switch as pending and only restarts into the new model at a clean retry point.
...
- **Provider usage/quota** (example: “Claude 80% left”) shows up in `/status` for the current model provider when usage tracking is enabled. Juno normalizes provider windows to `% left`; for MiniMax, remaining-only percent fields are inverted before display, and `model_remains` responses prefer the chat-model entry plus a model-tagged plan label.
...
```
/debug show
/debug set messages.responsePrefix="[juno]"
...
- Overrides apply immediately to new config reads, but do **not** write to `juno.json`.
...
`/config` writes to your on-disk config (`juno.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[juno]"
...
`/mcp` writes Juno-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in Juno config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/juno`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
