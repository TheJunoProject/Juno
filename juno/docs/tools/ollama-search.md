...
- an Ollama host that is reachable from Juno
- `ollama signin`
...
If no explicit Ollama base URL is set, Juno uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Juno reuses
...
- If the Ollama host is auth-protected, Juno reuses the normal Ollama
  provider API key when present.
- Juno warns during setup if Ollama is unreachable or not signed in, but
