...
read_when:
  - Running Juno Gateway in WSL2 while Chrome lives on Windows
...
- Juno Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/juno/juno/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Juno and Chrome are on the same machine
...
- Juno points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Juno is not the problem yet.
...
Fix that before touching Juno config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Juno at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Juno to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Juno to succeed
...
```bash
juno browser open https://example.com --browser-profile remote
juno browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `juno browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `juno browser stop --browser-profile remote`
...
3. Juno config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Juno config or Control UI auth
