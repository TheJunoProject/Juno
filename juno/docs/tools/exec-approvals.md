...
`ask: "always"` in `~/.juno/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `juno approvals get`, `juno approvals get --gateway`, or
`juno approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `juno exec-policy show` exposes the same merged view and
`juno exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`juno exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `juno` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Juno also tries to bind
...
- requested exec policy in Juno config (`tools.exec.*`)
- host-local approvals policy in `~/.juno/exec-approvals.json`
...
- In YOLO mode, Juno does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
juno config set tools.exec.host gateway
juno config set tools.exec.security full
juno config set tools.exec.ask off
...
- `juno exec-policy` does not synchronize node approvals
- `juno exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `juno approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, Juno treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.juno/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `juno approvals allowlist ...`).
- `juno security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `juno doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Juno still rejects the `env` builtin in safe-bin
...
`~/.juno/exec-approvals.json` directly.

CLI: `juno approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Juno sends a followup `agent` turn to the same session.
...
topics, Juno preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Juno prevents the agent from reusing
