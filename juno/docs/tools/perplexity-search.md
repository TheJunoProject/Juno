...
Juno supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Juno also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `juno configure --section web`. It stores the key in
`~/.juno/juno.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.juno/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
