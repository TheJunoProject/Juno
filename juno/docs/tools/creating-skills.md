...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.juno/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Juno picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    juno agent --message "give me a greeting"
    ```
...
| `metadata.juno.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.juno.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.juno.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `juno agent --message "..."` to test before sharing
- **Use JunoHub** — browse and contribute skills at [JunoHub](https://junohub.ai)
...
| `~/.juno/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Juno) | Low        | Global                |
...
- [JunoHub](/tools/junohub) — public skill registry
