...
read_when:
  ...
  - Debugging why juno is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Juno-managed)"
---

# Browser (juno-managed)

Juno can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `juno` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **juno** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`juno`, `work`, `remote`, ...).
...
```bash
juno browser --browser-profile juno status
...
If `juno browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`juno browser`),
...
If `juno browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "juno"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.juno/juno.json`.
...
- If you override the Gateway port (`gateway.port` or `JUNO_GATEWAY_PORT`),
...
- Default profile is `juno` (Juno-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `juno` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Juno uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
juno config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Juno will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `juno browser stop` stops the browser process that
  Juno launched
- attach-only and remote CDP profiles: `juno browser stop` closes the active
  ...
  though no browser process was launched by Juno
...
Juno preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Juno can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Juno treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Juno can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Juno
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Juno accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** — `http://host[:port]` or `https://host[:port]`.
  Juno calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** — `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Juno connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** — `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Juno tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Juno
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-juno-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Juno
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Juno does **not** auto-generate that token when `gateway.auth.mode` is
...
Juno supports multiple named profiles (routing configs). Profiles can be:

- **juno-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `juno` profile is auto-created if missing.
...
Juno can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Juno attaches.
...
```bash
juno browser --browser-profile user start
juno browser --browser-profile user status
juno browser --browser-profile user tabs
juno browser --browser-profile user snapshot --format ai
```
...
- `juno doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `juno` profile because it can
  act inside your signed-in browser session.
- Juno does not launch the browser for this driver; it attaches to an
  existing session only.
- Juno uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Juno passes it through to target that explicit
...
When launching locally, Juno picks the first available:
...
- `x-juno-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `juno` browser when a per-tab CDP
...
```bash
docker compose run --rm juno-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`JUNO_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `juno browser snapshot --interactive --compact --depth 6`
- `juno browser snapshot --efficient`
- `juno browser snapshot --labels`
- `juno browser snapshot --selector "#main" --interactive`
- `juno browser snapshot --frame "iframe#main" --interactive`
- `juno browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `juno browser stop` is still the
...
- `juno browser requests --filter api --clear`
- `juno browser pdf`
- `juno browser responsebody "**/api" --max-chars 5000`
...
- `juno browser storage local set theme dark`
- `juno browser storage session clear`
- `juno browser set offline on`
- `juno browser set headers --headers-json '{"X-Debug":"1"}'`
- `juno browser set credentials user pass`
- `juno browser set credentials --clear`
- `juno browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `juno browser set geo --clear`
- `juno browser set media dark`
- `juno browser set timezone America/New_York`
- `juno browser set locale en-US`
...
- Download and trace output paths are constrained to Juno temp roots:
  - traces: `/tmp/juno` (fallback: `${os.tmpdir()}/juno`)
  - downloads: `/tmp/juno/downloads` (fallback: `${os.tmpdir()}/juno/downloads`)
- Upload paths are constrained to an Juno temp uploads root:
  - uploads: `/tmp/juno/uploads` (fallback: `${os.tmpdir()}/juno/uploads`)
...
Juno supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `juno browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `juno browser click 12`, `juno browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `juno browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `juno browser click e12`, `juno browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `juno browser wait --url "**/dash"`
- Wait for load state:
  - `juno browser wait --load networkidle`
- Wait for a JS predicate:
  - `juno browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `juno browser wait "#main"`
...
1. `juno browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `juno browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `juno browser trace start`
   - reproduce the issue
   - `juno browser trace stop` (prints `TRACE:<path>`)
...
```bash
juno browser status --json
juno browser snapshot --interactive --json
juno browser requests --filter api --json
juno browser cookies --json
```
...
- The juno browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `juno browser evaluate` and `wait --fn`
...
- **CDP startup or readiness failure** means Juno cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "juno" is not reachable after start`
...
- For the local loopback `juno` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Juno's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (juno, chrome, or remote CDP).
