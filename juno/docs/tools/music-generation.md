...
For shared provider-backed agent sessions, Juno starts music generation as a
...
Not all providers support all parameters. Juno still validates hard limits
such as input counts before submission. When a provider supports duration but
uses a shorter maximum than the requested value, Juno automatically clamps
...
Tool results report the applied settings. When Juno clamps duration during provider fallback, the returned `durationSeconds` reflects the submitted value and `details.normalization.durationSeconds` shows the requested-to-applied mapping.
...
- Task tracking: use `juno tasks list` or `juno tasks show <taskId>` to inspect queued, running, and terminal status for the generation.
- Completion wake: Juno injects an internal completion event back into the same session so the model can write the user-facing follow-up itself.
...
When generating music, Juno tries providers in this order:
...
```bash
JUNO_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
