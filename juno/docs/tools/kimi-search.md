...
Juno supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Juno reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.juno/.env`.

If you omit `baseUrl`, Juno defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Juno defaults to `kimi-k2.5`.
