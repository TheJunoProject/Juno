...
- Docs are hosted on Mintlify (`https://docs.juno.ai`).
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `juno/docs` repo (often cloned locally as `../juno-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `juno/docs`.
