...
Juno supports **camera capture** for agent workflows:
...
```bash
juno nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
juno nodes camera snap --node <id> --facing front
juno nodes camera clip --node <id> --duration 3000
juno nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`juno.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `juno` CLI to invoke camera commands on the macOS node.

Examples:

```bash
juno nodes camera list --node <id>            # list camera ids
juno nodes camera snap --node <id>            # prints MEDIA:<path>
juno nodes camera snap --node <id> --max-width 1280
juno nodes camera snap --node <id> --delay-ms 2000
juno nodes camera snap --node <id> --device-id <id>
juno nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
juno nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
juno nodes camera clip --node <id> --device-id <id>
juno nodes camera clip --node <id> --no-audio
```

Notes:

- `juno nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
juno nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
