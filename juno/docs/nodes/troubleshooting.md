...
```bash
juno nodes describe --node <idOrNameOrIp>
juno nodes canvas snapshot --node <idOrNameOrIp>
juno logs --follow
```
...
juno nodes status
juno approvals get --node <idOrNameOrIp>
juno approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`juno approvals get --node ...`), not in the gateway pairing record.
