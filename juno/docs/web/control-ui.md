...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/juno`)
...
If the page fails to load, start the Gateway first: `juno gateway`.
...
created. Re-run `juno devices list` before approval.
...
reconnect. Juno keeps the old approval active, blocks the broader reconnect,
and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `juno devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.juno/juno.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop juno`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Juno
...
```bash
juno gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Juno **blocks** Control UI connections without device identity.
...
```bash
JUNO_CONTROL_UI_BASE_PATH=/juno/ pnpm ui:build
```
