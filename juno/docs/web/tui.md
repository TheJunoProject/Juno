...
```bash
juno tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `juno tui --deliver`
...
- Local shell commands receive `JUNO_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `JUNO_THEME=light` before launching `juno tui`.
- To force the original dark palette instead, set `JUNO_THEME=dark`.
...
- Check the Gateway logs: `juno logs --follow`.
- Confirm the agent can run: `juno status` and `juno models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `juno agents list` and your routing config.
