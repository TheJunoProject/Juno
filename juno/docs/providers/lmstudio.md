---
summary: "Run Juno with LM Studio"
read_when:
  - You want to run Juno with open source models via LM Studio
...
3. Juno requires an LM Studio token value. Set `LM_API_TOKEN`:
...
```bash
juno models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Juno
