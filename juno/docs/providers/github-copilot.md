---
summary: "Sign in to GitHub Copilot from Juno using the device flow"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `juno models auth login-github-copilot` flow
...
models for your GitHub account and plan. Juno can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Juno

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Juno runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        juno models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        juno models set github-copilot/claude-opus-4.6
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Juno talks to
...
```bash
# Skip confirmation
juno models auth login-github-copilot --yes

# Login and set the default model in one step
juno models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Juno
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Environment variable resolution order">
    Juno resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Juno uses the highest-priority one.
    The device-login flow (`juno models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Juno runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Juno can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Juno discovers available
...
1. Juno resolves your GitHub token (from env vars or auth profile).
...
available, Juno skips Copilot and tries the next provider.
