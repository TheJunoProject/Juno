---
summary: "Use Z.AI (GLM models) with Juno"
read_when:
  - You want Z.AI / GLM models in Juno
...
for authentication. Create your API key in the Z.AI console. Juno uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Juno detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        juno onboard --auth-choice zai-coding-cn

        # General API
        juno onboard --auth-choice zai-global

        # General API CN (China region)
        juno onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Juno currently seeds the bundled `zai` provider with:
