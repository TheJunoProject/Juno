---
summary: "Run Juno with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Juno against a local SGLang server
...
Juno can connect to SGLang using the `openai-completions` API.

Juno can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Juno will query:
