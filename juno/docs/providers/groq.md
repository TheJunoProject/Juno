...
read_when:
  - You want to use Groq with Juno
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Juno connects
...
Groq's model catalog changes frequently. Run `juno models list | grep groq`
...
<Tip>
Use `juno models list --provider groq` for the most up-to-date list of
...
media-understanding provider, Juno uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.juno/.env` or `env.shellEnv` config for
