...
read_when:
  - You want to use Volcano Engine or Doubao models with Juno
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    juno onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    juno models list --provider volcengine
    juno models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `juno onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Juno falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.juno/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Juno as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Juno setup.
  </Card>
</CardGroup>
