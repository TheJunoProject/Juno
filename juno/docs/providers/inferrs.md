---
summary: "Run Juno through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Juno against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Juno compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. Juno works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated Juno provider plugin.
...
<Steps>
  ...
  <Step title="Add an Juno provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If Juno runs fail with an error like:
    ...
    Juno will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full Juno agent-runtime
    turns.
    ...
    That disables Juno's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal Juno agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than Juno's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but juno infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If Juno no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Juno against local model servers.
  </Card>
