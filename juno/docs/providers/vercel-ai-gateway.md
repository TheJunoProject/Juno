...
read_when:
  - You want to use Vercel AI Gateway with Juno
...
<Tip>
Juno auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    juno onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    juno models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Juno accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Juno resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Juno Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.juno/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
