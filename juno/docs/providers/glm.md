---
summary: "GLM model family overview + how to use it in Juno"
read_when:
  - You want GLM models in Juno
...
GLM is a **model family** (not a company) available through the Z.AI platform. In Juno, GLM
...
<Steps>
  <Step title="Choose an auth route and run onboarding">
    ...
    juno onboard --auth-choice zai-api-key

    # Example: Coding Plan global
    juno onboard --auth-choice zai-coding-global
    ```

  </Step>
  <Step title="Set GLM as the default model">
    ```bash
    juno config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
  <Step title="Verify models are available">
    ```bash
    juno models list --provider zai
    ```
  </Step>
</Steps>
...
<Tip>
`zai-api-key` lets Juno detect the matching Z.AI endpoint from the key and
...
Juno currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Juno inspects the key format
