---
summary: "Use Synthetic's Anthropic-compatible API in Juno"
read_when:
...
Juno registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    juno onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Juno's Anthropic client appends `/v1` to the base URL automatically, so use
...
`juno models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Juno appends `/v1` automatically.
