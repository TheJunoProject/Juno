...
read_when:
  - You want to use Google Gemini models with Juno
...
<Tabs>
    ...
    **Best for:** standard Gemini API access through Google AI Studio.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        juno onboard --auth-choice gemini-api-key
        ```
      ...
      <Step title="Verify the model is available">
        ```bash
        juno models list --provider google
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Best for:** reusing an existing Gemini CLI login via PKCE OAuth instead of a separate API key.
    ...
    <Steps>
      <Step title="Install the Gemini CLI">
        ...
        Juno supports both Homebrew installs and global npm installs, including
        common Windows/npm layouts.
      </Step>
      <Step title="Log in via OAuth">
        ```bash
        juno models auth login --provider google-gemini-cli --set-default
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        juno models list --provider google-gemini-cli
        ```
      </Step>
    </Steps>
    ...
    **Environment variables:**
    ...
    - `JUNO_GEMINI_OAUTH_CLIENT_ID`
    - `JUNO_GEMINI_OAUTH_CLIENT_SECRET`
...
<Tip>
Gemini 3 models use `thinkingLevel` rather than `thinkingBudget`. Juno maps
...
Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Juno
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Juno
    ...
    - Gemini cache-hit usage is normalized into Juno `cacheRead` from
  ...
  <Accordion title="Gemini CLI JSON usage notes">
    When using the `google-gemini-cli` OAuth provider, Juno normalizes
  ...
  <Accordion title="Environment and daemon setup">
    ...
    is available to that process (for example, in `~/.juno/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
