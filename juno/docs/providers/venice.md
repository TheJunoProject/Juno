---
summary: "Use Venice AI privacy-focused models in Juno"
read_when:
  - You want privacy-focused inference in Juno
...
<Steps>
  ...
  <Step title="Configure Juno">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        juno onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    juno agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Juno shows all available Venice models. Pick based on your needs:
...
```bash
juno models set venice/kimi-k2-5
juno models set venice/claude-opus-4-6
```

List all available models:

```bash
juno models list | grep venice
```

You can also run `juno configure`, select **Model/auth**, and choose **Venice AI**.
...
Juno automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
juno agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
juno agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
juno agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
juno agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
juno agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `juno models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
