---
summary: "Use Mistral models and Voxtral transcription with Juno"
read_when:
  - You want to use Mistral models in Juno
...
Juno supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    juno onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    juno onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    juno models list --provider mistral
    ```
  </Step>
</Steps>
...
Juno currently ships this bundled Mistral catalog:
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Juno maps the session **thinking** level to Mistral's API:

    | Juno thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
