---
summary: "Use Vydra image, video, and speech in Juno"
read_when:
  - You want Vydra media generation in Juno
...
Juno uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    juno onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  <Accordion title="Image generation">
    ...
    Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and Juno does not add a Vydra-specific upload bridge in the bundled plugin yet.
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export JUNO_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
