...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Juno
...
Running Juno via Node with `tsx` fails at startup with:

```
[juno] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `juno status` and `gateway:watch` worked.
