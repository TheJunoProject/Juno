...
Juno can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Juno requires a shared password.
...
- `token` (default when `JUNO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `JUNO_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Juno verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Juno only treats a request as Serve when it arrives from loopback with
...
Prefer `JUNO_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
juno gateway --tailscale serve
juno gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Juno to undo `tailscale serve`
