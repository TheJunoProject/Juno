...
Juno. All clients (CLI, web UI, macOS app, iOS/Android nodes, headless
...
- Operators may call `skills.search` and `skills.detail` (`operator.read`) for
  JunoHub discovery metadata.
- Operators may call `skills.install` (`operator.admin`) in two modes:
  - JunoHub mode: `{ source: "junohub", slug, version?, force? }` installs a
  ...
  - Gateway installer mode: `{ name, installId, dangerouslyForceUnsafeInstall?, timeoutMs? }`
    runs a declared `metadata.juno.install` action on the gateway host.
- Operators may call `skills.update` (`operator.admin`) in two modes:
  - JunoHub mode updates one tracked slug or all tracked JunoHub installs in
