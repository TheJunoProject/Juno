...
Juno supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Juno also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `juno gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.juno/.env` so the daemon can read it:
...
API keys for daemon use: `juno onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.juno/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Juno as a supported token
path. Anthropic staff has since told us that Juno-style Claude CLI usage is
...
```bash
juno models auth paste-token --provider openrouter
```
...
- If auth exists but Juno cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Juno integration path is allowed again.
- Juno therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `JUNO_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Juno retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `juno models status --probe` shows omitted
...
Run `juno models status` to confirm which profile is expiring. If an
