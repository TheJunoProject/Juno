...
- `juno gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `juno doctor` reports no blocking config/service issues.
- `juno channels status --probe` shows live per-account transport status and,
...
juno logs --follow
juno models status
juno config get agents.defaults.models
```
...
- Juno model runs fail only on normal agent turns
...
juno infer model run --model <provider/model> --prompt "hi" --json
juno logs --follow
```

Look for:

- direct tiny calls succeed, but Juno runs fail only on larger prompts
...
- direct tiny requests succeed, but Juno agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → Juno transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   Juno's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while Juno agent turns still crash
...
juno status
juno channels status --probe
juno pairing list --channel <channel> [--account <id>]
juno config get channels
juno logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `juno config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `juno devices list` then `juno devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `juno devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `juno devices rotate --scope ...` can only request operator scopes that
...
juno logs --follow
juno doctor
juno gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `juno onboard --mode local` / `juno setup` to restamp the expected local-mode config. If you are running Juno via Podman, the default config path is `~/.juno/juno.json`.
...
Use this when the Gateway starts, but logs say it restored `juno.json`.

```bash
juno logs --follow
juno config file
juno config validate
juno doctor
```
...
- A timestamped `juno.json.clobbered.*` file beside the active config
...
- Juno preserved the rejected payload as `.clobbered.*`.
...
- `.rejected.*` exists → an Juno-owned config write failed schema or clobber checks before commit.
...
2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `juno config set` or `config.patch`.
3. Run `juno config validate` before restarting.
...
Use this when `juno gateway probe` reaches something, but still prints a warning block.

```bash
juno gateway probe
juno gateway probe --json
juno gateway probe --ssh user@gateway-host
```
...
juno cron status
juno cron list
juno cron runs --id <jobId> --limit 20
juno system heartbeat last
juno logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Juno skips the model call.
...
juno approvals get --node <idOrNameOrIp>
juno logs --follow
juno status
```
...
```bash
juno browser status
juno browser start --browser-profile juno
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `juno browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
juno gateway status
juno config get gateway.mode
juno config get gateway.remote.url
juno config get gateway.auth.mode
```
...
```bash
juno config get gateway.bind
juno config get gateway.auth.mode
juno config get gateway.auth.token
juno gateway status
juno logs --follow
```
