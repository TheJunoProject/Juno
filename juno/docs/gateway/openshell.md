---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Juno agents"
read_when:
...
OpenShell is a managed sandbox backend for Juno. Instead of running Docker
containers locally, Juno delegates sandbox lifecycle to the `openshell` CLI,
...
- Juno Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Juno creates an OpenShell
...
- You edit files locally outside Juno and want those changes visible in the
...
- When the sandbox is first created, Juno seeds the remote workspace from
...
- Juno does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Juno after the initial seed,
the remote sandbox does **not** see those changes. Use
`juno sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"juno"`  | Sandbox source for first-time create                  |
...
1. Juno calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Juno calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `juno sandbox` commands
