...
Juno can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Juno tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
juno agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, Juno now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Juno will
...
2. **Builds a system prompt** using the same Juno prompt + workspace context.
...
told us Juno-style Claude CLI usage is allowed again, so Juno treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes Juno's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so Juno writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the Juno skills snapshot
two ways: the compact Juno skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Juno would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Juno to the
...
- Juno drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
Juno will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Juno appends the
...
- For Gemini CLI JSON output, Juno reads reply text from `response` and
...
- `stats.cached` is normalized into Juno `cacheRead`.
- If `stats.input` is missing, Juno derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Juno handles
...
CLI backends do **not** receive Juno tool calls directly, but a backend can
...
When bundle MCP is enabled, Juno:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`JUNO_MCP_TOKEN`)
...
If no MCP servers are enabled, Juno still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct Juno tool calls.** Juno does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Juno sessions still work
  normally.
