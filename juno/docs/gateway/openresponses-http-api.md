...
Juno’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`juno agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-juno-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-juno-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "juno"`, `model: "juno/default"`, `model: "juno/<agentId>"`, or `x-juno-agent-id`
- use `x-juno-model` when you want to override the selected agent's backend model
- use `x-juno-session-key` for explicit session routing
- use `x-juno-message-channel` when you want a non-default synthetic ingress channel context
...
For the canonical explanation of how agent-target models, `juno/default`, embeddings pass-through, and backend model overrides fit together, see [OpenAI Chat Completions](/gateway/openai-http-api#agent-first-model-contract) and [Model list and agent routing](/gateway/openai-http-api#model-list-and-agent-routing).
...
- `previous_response_id`: Juno reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Juno normalizes common OpenAI-style aliases before those counters reach
