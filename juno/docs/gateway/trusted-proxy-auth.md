...
read_when:
  - Running Juno behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Juno
...
- You run Juno behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Juno checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Juno extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Juno can verify.
...
- Juno can stay on loopback HTTP behind the proxy.
...
If Juno itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://juno.example.com
    to: http://juno-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
Juno rejects ambiguous configurations where both a `gateway.auth.token` (or `JUNO_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-juno-scopes`.

Examples:

- `x-juno-scopes: operator.read`
- `x-juno-scopes: operator.read,operator.write`
- `x-juno-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Juno honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-juno-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-juno-scopes` explicitly when you want a trusted-proxy request to
...
`juno security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Juno rejected a loopback-source trusted-proxy request.
...
3. Update Juno config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `juno security audit` and review findings
