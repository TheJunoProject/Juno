...
summary: "Gateway config reference for core Juno keys, defaults, and links to dedicated subsystem references"
read_when:
...
Core config reference for `~/.juno/juno.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

This page covers the main Juno config surfaces and links out when a subsystem has its own deeper reference. It does **not** try to inline every channel/plugin-owned command catalog or every deep memory/QMD knob on one page.

Code truth:

- `juno config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Juno uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.juno/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `juno doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `juno doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["juno-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-juno",
          requireMention: false,
...
- Juno additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `juno plugins install @juno/mattermost`.
...
- `commands.callbackUrl` must resolve to the Juno gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, Juno rejects callbacks with
...
Juno spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `juno channels add` (or channel onboarding) while still on a single-account top-level channel config, Juno promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `juno doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@juno", "juno"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@juno"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `juno.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for Juno-managed MCP server config under `mcp.servers`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Juno auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/juno" } },
}
```
...
Juno has multiple high-volume prompt/context budgets, and they are
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, Juno tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, Juno falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
...
- Environment overrides: `JUNO_AGENT_RUNTIME=<id|auto|pi>` overrides `runtime`; `JUNO_AGENT_HARNESS_FALLBACK=none` disables PI fallback for that process.
...
Replace the entire Juno-assembled system prompt with a fixed string. Set at the default level (`agents.defaults.systemPromptOverride`) or per agent (`agents.list[].systemPromptOverride`). Per-agent values take precedence; an empty or whitespace-only value is ignored. Useful for controlled prompt experiments.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Juno aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.juno/sandboxes",
        docker: {
          image: "juno-sandbox:bookworm-slim",
          containerPrefix: "juno-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/juno-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "juno-sandbox-browser:bookworm-slim",
          network: "juno-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Juno materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.juno/sandboxes`
...
In `remote` mode, host-local edits made outside Juno are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `juno.json`.
noVNC observer access uses VNC auth by default and Juno emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `juno-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from JUNO_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `JUNO_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `JUNO_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `JUNO_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.juno/workspace",
        agentDir: "~/.juno/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.juno/workspace-home" },
      { id: "work", workspace: "~/.juno/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Juno resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.juno/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.juno/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.juno/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.juno/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Juno starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.juno/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Juno treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:juno`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.juno/attachments/<uuid>/` with a `.manifest.json`.
...
Juno uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.juno/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `JUNO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Juno forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
- `models.providers.*.models.*.compat.requiresStringContent`: optional compatibility hint for string-only OpenAI-compatible chat endpoints. When `true`, Juno flattens pure text `messages[].content` arrays into plain strings before sending the request.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `juno onboard --auth-choice opencode-zen` or `juno onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `juno onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `juno onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and Juno keys that off endpoint capabilities
...
Anthropic-compatible, built-in provider. Shortcut: `juno onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `juno onboard --auth-choice synthetic-api-key`.
...
`juno onboard --auth-choice minimax-global-api` or
`juno onboard --auth-choice minimax-cn-api`.
The model catalog defaults to M2.7 only.
On the Anthropic-compatible streaming path, Juno disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.juno.install`
...
- Loaded from `~/.juno/extensions`, `<workspace>/.juno/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Juno plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Juno plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Juno applies those as sanitized agent settings, not as raw Juno config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `juno plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      juno: { cdpPort: 18800, color: "#FF4500" },
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Juno to discover `/json/version`; use WS(S)
...
- Local managed `juno` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or JUNO_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `JUNO_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `JUNO_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `JUNO_APNS_RELAY_BASE_URL` / `JUNO_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `JUNO_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
JUNO_CONFIG_PATH=~/.juno/a.json \
JUNO_STATE_DIR=~/.juno-a \
juno gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.juno-dev` + port `19001`), `--profile <name>` (uses `~/.juno-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/juno/tls/server.crt",
      keyPath: "/etc/juno/tls/server.key",
      caPath: "/etc/juno/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.juno/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-juno-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `JUNO_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.juno/workspace/canvas",
    liveReload: true,
    // enabled: false, // or JUNO_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__juno__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__juno__/a2ui/`
...
- Also serves A2UI at `/__juno__/a2ui/`.
...
- Hostname defaults to `juno`. Override with `JUNO_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.juno/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `juno dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.juno/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `juno.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.juno/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/juno-vault-resolver",
...
- Legacy OAuth imports from `~/.juno/credentials/oauth.json`.
...
- Default log file: `/tmp/juno/juno-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.juno/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$JUNO_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Juno.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `JUNO_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `juno doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `juno.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
