...
`juno doctor` is the repair + migration tool for Juno. It fixes stale
...
```bash
juno doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
juno doctor --non-interactive
```
...
- Extra workspace dir detection (`~/juno`).
...
doctor-style RPC methods, but they are **not** part of `juno doctor` CLI
repair/migration.
...
```bash
juno memory rem-backfill --path ./memory --stage-short-term
```
...
you to run `juno doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.juno/juno.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `juno doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.juno/sessions/` to `~/.juno/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.juno/agent/` to `~/.juno/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.juno/credentials/*.json` (except `oauth.json`)
  - to `~/.juno/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `juno doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.juno/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.juno` folders exist across
  home directories or when `JUNO_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.juno/juno.json` is
...
that re-auth is required and prints the exact `juno models auth login --provider ...`
command to run.
...
Discord plugin runtime packages) are present in the Juno install root.
If any are missing, doctor reports the packages and installs them in
`juno doctor --fix` / `juno doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Juno service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Juno gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`juno doctor` without `--fix`) this check
is skipped entirely.
...
- inspect pending requests with `juno devices list`
- approve the exact request with `juno devices approve <requestId>`
- rotate a fresh token with `juno devices rotate --device <deviceId> --role <role>`
- remove and re-approve a stale record with `juno devices remove <deviceId>`
...
- **Legacy workspace dirs**: warns when `~/juno` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(juno completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `juno completion --write-state` to regenerate the cache manually.
...
- `juno doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `juno doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `juno memory status --deep` to verify embedding readiness at runtime.
...
- `juno doctor` prompts before rewriting supervisor config.
- `juno doctor --yes` accepts the default repair prompts.
- `juno doctor --repair` applies recommended fixes without prompts.
- `juno doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `juno gateway install --force`.
