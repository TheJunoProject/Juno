---
summary: "Schema-accurate configuration examples for common Juno setups"
read_when:
  - Learning how to configure Juno
...
```json5
{
  agent: { workspace: "~/.juno/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.juno/juno.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.juno/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/juno" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.juno/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
