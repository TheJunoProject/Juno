...
Juno reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.juno/juno.json`.

If the file is missing, Juno uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `juno onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    juno onboard       # full onboarding flow
    juno configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    juno config get agents.defaults.workspace
    juno config set agents.defaults.heartbeat.every "2h"
    juno config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.juno/juno.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Juno only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `juno config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `juno.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`juno doctor`, `juno logs`, `juno health`, `juno status`)
- Run `juno doctor` to see exact issues
- Run `juno doctor --fix` (or `--yes`) to apply repairs

The Gateway also keeps a trusted last-known-good copy after a successful startup. If
`juno.json` is later changed outside Juno and no longer validates, startup
...
Juno-owned config writes whose persisted file hash still matches the accepted
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `juno.json`.
    ...
    ```bash
    juno config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `JUNO_APNS_RELAY_BASE_URL` and `JUNO_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `JUNO_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-juno-token`); query-string tokens are rejected.
...
The Gateway watches `~/.juno/juno.json` and applies changes automatically — no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits by restoring the last-known-good config. Juno-owned
...
`.clobbered.*` file next to `juno.json`, fix the rejected payload, then run
`juno config validate`. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-restored-last-known-good-config)
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `juno config set` for single keys.
    </Warning>
    ...
    ```bash
    juno gateway call config.get --params '{}'  # capture payload.hash
    juno gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.juno/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    juno gateway call config.patch --params '{
...
Juno reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.juno/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Juno runs your login shell and imports only the missing keys:
...
Env var equivalent: `JUNO_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${JUNO_GATEWAY_TOKEN}" } },
