...
juno nodes status
juno nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.juno`):

- `~/.juno/nodes/paired.json`
- `~/.juno/nodes/pending.json`

If you override `JUNO_STATE_DIR`, the `nodes/` folder moves with it.
