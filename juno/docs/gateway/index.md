...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `juno gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `JUNO_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `JUNO_GATEWAY_TOKEN` / `JUNO_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Juno’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `juno`, `juno/default`, and `juno/<agentId>`.
- `juno/default` is the stable alias that always maps to the configured default agent.
- Use `x-juno-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `JUNO_GATEWAY_PORT` → `gateway.port` → `18789` |
...
```bash
juno gateway status
juno gateway status --deep   # adds a system-level service scan
...
LaunchAgent labels are `ai.juno.gateway` (default) or `ai.juno.<profile>` (named profile). `juno doctor` audits and repairs service config drift.
...
```bash
juno gateway install
systemctl --user enable --now juno-gateway[-<profile>].service
juno gateway status
```
...
```ini
[Unit]
Description=Juno Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/juno gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `Juno Gateway`
(or `Juno Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Juno falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now juno-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/juno-gateway[-<profile>].service` and adjust
`ExecStart=` if your `juno` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `JUNO_CONFIG_PATH`
- Unique `JUNO_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
JUNO_CONFIG_PATH=~/.juno/a.json JUNO_STATE_DIR=~/.juno-a juno gateway --port 19001
JUNO_CONFIG_PATH=~/.juno/b.json JUNO_STATE_DIR=~/.juno-b juno gateway --port 19002
```
...
```bash
juno --dev setup
juno --dev gateway --allow-unconfigured
juno --dev status
```
