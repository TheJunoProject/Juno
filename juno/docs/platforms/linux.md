...
2. `npm i -g juno@latest`
3. `juno onboard --install-daemon`
...
Juno installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `juno gateway install` and
`juno onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/juno-gateway[-<profile>].service`:

```
[Unit]
Description=Juno Gateway (profile: <profile>, v<version>)
