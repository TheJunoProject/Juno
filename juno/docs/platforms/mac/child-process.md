...
service via the external `juno` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.juno.gateway`
  (or `ai.juno.<profile>` when using `--profile`/`JUNO_PROFILE`; legacy `com.juno.*` is supported).
