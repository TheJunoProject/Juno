...
- Same path: run the app from a fixed location (for Juno, `dist/Juno.app`).
...
```bash
sudo tccutil reset Accessibility ai.juno.mac
sudo tccutil reset ScreenCapture ai.juno.mac
...
Workaround: move files into the Juno workspace (`~/.juno/workspace`) if you want to avoid per-folder grants.
