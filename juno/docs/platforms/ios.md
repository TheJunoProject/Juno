...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `juno.internal.`), **or**
...
Run `juno devices list` again before approval.

4. Verify connection:

```bash
juno nodes status
juno gateway call node.list --params "{}"
```
...
- `JUNO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Juno iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Juno builds.
...
```bash
export JUNO_APNS_TEAM_ID="TEAMID"
export JUNO_APNS_KEY_ID="KEYID"
export JUNO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.juno/credentials/apns
chmod 700 ~/.juno/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.juno/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.juno/credentials/apns/AuthKey_KEYID.p8
export JUNO_APNS_PRIVATE_KEY_PATH="$HOME/.juno/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_juno-gw._tcp` on `local.` and, when configured, the same
...
`juno.internal.`) and Tailscale split DNS.
...
```bash
juno nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__juno__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__juno__/canvas/` and `/__juno__/a2ui/`.
...
```bash
juno nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__juno; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
juno nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `juno devices list` and approve manually.
