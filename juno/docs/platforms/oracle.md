---
summary: "Juno on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Juno on Oracle Cloud
  - Looking for low-cost VPS hosting for Juno
  - Want 24/7 Juno on a small server
title: "Oracle Cloud (Platform)"
---

# Juno on Oracle Cloud (OCI)

## Goal

Run a persistent Juno Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Juno (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname juno
...
sudo tailscale up --ssh --hostname=juno
```

This enables Tailscale SSH, so you can connect via `ssh juno` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@juno` (or use the Tailscale IP).
...
juno config set gateway.bind loopback

# Require auth for the Gateway + Control UI
juno config set gateway.auth.mode token
juno doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
juno config set gateway.tailscale.mode serve
juno config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart juno-gateway.service
```
...
# Check daemon status
systemctl --user status juno-gateway.service
...
```
https://juno.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `juno security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.juno`
- **Security audit:** `juno security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@juno
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=juno --reset
```
...
journalctl --user -u juno-gateway.service -n 50
```
...
- `~/.juno/` — `juno.json`, per-agent `auth-profiles.json`, channel/provider state, and session data
- `~/.juno/workspace/` — workspace (SOUL.md, memory, artifacts)
