---
summary: "Juno on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Juno on DigitalOcean
  - Looking for cheap VPS hosting for Juno
title: "DigitalOcean (Platform)"
---

# Juno on DigitalOcean

## Goal

Run a persistent Juno Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
juno config set gateway.bind tailnet
juno gateway restart
```
...
- `~/.juno/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u juno-gateway.service --no-pager -n 50
```
