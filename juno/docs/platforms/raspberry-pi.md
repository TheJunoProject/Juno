---
summary: "Juno on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Juno on a Raspberry Pi
  - Running Juno on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Juno on Raspberry Pi

## Goal

Run a persistent, always-on Juno Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u juno-gateway.service -f
```

## 9) Access the Juno Dashboard
...
```bash
ssh user@gateway-host 'juno dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/juno-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/juno-compile-cache
mkdir -p /var/tmp/juno-compile-cache
export JUNO_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `JUNO_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Juno, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit juno-gateway.service
```

```ini
[Service]
Environment=JUNO_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/juno-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Juno state/cache on SSD-backed storage to avoid SD-card
...
Most Juno features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled juno-gateway.service

# Enable if not
systemctl --user enable juno-gateway.service
...
```bash
# Check logs
journalctl --user -u juno-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/juno  # if using hackable install
npm run build
systemctl --user restart juno-gateway.service
```
