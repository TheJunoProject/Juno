...
This document explains how Juno manages sessions end-to-end:
...
Juno is designed around a single **Gateway process** that owns session state.
...
Juno persists sessions in two layers:
...
- Transcripts: `~/.juno/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Juno resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Juno reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
juno sessions cleanup --dry-run
juno sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.juno/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Juno intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Juno splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Juno
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Juno preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Juno consumes the events, but Pi decides when to compact).
...
Juno also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Juno bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Juno leaves it alone.
...
- If the provider fails or returns an empty result, Juno falls back to built-in LLM summarization automatically.
...
Juno supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Juno strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Juno also suppresses **draft/typing streaming** when a
...
Juno uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Juno’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `juno status`.
