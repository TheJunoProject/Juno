...
Juno integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Juno owns lifecycle when `channels.signal.autoStart=true`.
...
- Juno spawns `imsg rpc` as a child process (legacy iMessage integration).
