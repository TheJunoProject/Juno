---
summary: "How Juno builds prompt context and reports token usage + costs"
read_when:
...
Juno tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Juno assembles its own system prompt on every run. It includes:
...
For images, Juno downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Juno session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `juno status --usage` and `juno channels list` show
...
available; otherwise Juno falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Juno shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Juno can
...
Anthropic's 1M context window is currently beta-gated. Juno can inject the
...
Juno skips the `context-1m-*` beta header because Anthropic currently
