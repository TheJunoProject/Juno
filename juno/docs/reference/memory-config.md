...
This page lists every configuration knob for Juno memory search. For
...
`juno.json` unless noted otherwise.
...
When `provider` is not set, Juno selects the first available:
...
If Juno runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, Juno falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.juno/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
Juno prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not Juno config. If you need to
