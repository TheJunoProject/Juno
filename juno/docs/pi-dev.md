...
This guide summarizes a sane workflow for working on the pi integration in Juno.
...
```bash
JUNO_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm juno agent --message "Hello" --thinking low`
...
State lives under the Juno state directory. Default is `~/.juno`. If `JUNO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `juno.json` for config
