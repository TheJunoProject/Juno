...
import {
  ...
  collectPluginJunoHubReleasePathsFromGitRange,
  collectPluginJunoHubReleasePlan,
  resolveChangedJunoHubPublishablePluginPackages,
  resolveSelectedJunoHubPublishablePluginPackages,
  type PublishablePluginPackage,
} from "../scripts/lib/plugin-junohub-release.ts";
...
describe("resolveChangedJunoHubPublishablePluginPackages", () => {
...
describe("collectJunoHubPublishablePluginPackages", () => {
  it("requires the JunoHub external plugin contract", () => {
    const repoDir = createTempPluginRepo({
      includeJunoHubContract: false,
    });

    expect(() => collectJunoHubPublishablePluginPackages(repoDir)).toThrow(
...
describe("collectJunoHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    const errors = collectJunoHubVersionGateErrors({
      rootDir: repoDir,
      plugins: collectJunoHubPublishablePluginPackages(repoDir),
      gitRange: { baseRef, headRef },
    });

    expect(errors).toEqual([
      "@juno/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });

  it("does not require a version bump for the first JunoHub opt-in", () => {
    const repoDir = createTempPluginRepo({
      publishToJunoHub: false,
    });
    const baseRef = git(repoDir, ["rev-parse", "HEAD"]);
      ...
      JSON.stringify(
        {
          ...
          juno: {
            ...
            build: {
              junoVersion: "2026.4.1",
            },
            release: {
              publishToJunoHub: true,
            },
          },
        },
        null,
  ...
  it("does not require a version bump for shared release-tooling changes", () => {
    ...
    writeFileSync(join(repoDir, "scripts", "plugin-junohub-publish.sh"), "#!/usr/bin/env bash\n");
    git(repoDir, ["add", "."]);
    git(repoDir, [
      "-c",
      "user.name=Test",
...
describe("resolveSelectedJunoHubPublishablePluginPackages", () => {
...
describe("collectPluginJunoHubReleasePlan", () => {
  it("skips versions that already exist on JunoHub", async () => {
    const repoDir = createTempPluginRepo();

    const plan = await collectPluginJunoHubReleasePlan({
      rootDir: repoDir,
      selection: ["@juno/demo-plugin"],
      fetchImpl: async () => new Response("{}", { status: 200 }),
      registryBaseUrl: "https://junohub.ai",
    });
...
describe("collectPluginJunoHubReleasePathsFromGitRange", () => {
...
function createTempPluginRepo(
  options: {
    ...
    includeJunoHubContract?: boolean;
  } = {},
) {
  const repoDir = makeTempRepoRoot(tempDirs, "juno-junohub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "juno-test-root" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@juno/${currentExtensionId}`,
          version: "2026.4.1",
          juno: {
            extensions: ["./index.ts"],
            ...(options.includeJunoHubContract === false
              ? {}
              : {
                  compat: {
                    pluginApi: ">=2026.4.1",
            ...
            release: {
              publishToJunoHub: options.publishToJunoHub ?? true,
            },
          },
        },
        null,
