...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ JUNO_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ JUNO_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ JUNO_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from JUNO_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-vitest-scoped-"));
    try {
      ...
      const config = createScopedVitestConfig(["src/utils/**/*.test.ts"], {
        dir: "src",
        env: {
          JUNO_VITEST_INCLUDE_FILE: includeFile,
        },
      });
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the juno runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the juno runtime setup", () => {
  ...
  it("keeps the ui lane off both the juno runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from JUNO_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "juno-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
