...
function createTempHome(): string {
  return makeTempDir(tempDirs, "juno-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-juno.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.juno/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.juno/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".juno", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".juno", "external-plugins", "gluejuno", "juno.plugin.json"),
      '{"id":"gluejuno"}\n',
    );
    writeFile(
      path.join(realHome, ".juno", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.JUNO_LIVE_TEST_QUIET = "1";
    process.env.JUNO_CONFIG_PATH = "~/custom-juno.json5";
    process.env.JUNO_TEST_HOME = priorIsolatedHome;
    process.env.JUNO_STATE_DIR = path.join(priorIsolatedHome, ".juno");
    ...
    expect(process.env.JUNO_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".juno", "juno.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".juno", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".juno", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.JUNO_LIVE_TEST = "1";
    process.env.JUNO_LIVE_USE_REAL_HOME = "1";
    process.env.JUNO_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.JUNO_LIVE_TEST;
    delete process.env.JUNO_LIVE_GATEWAY;
    delete process.env.JUNO_LIVE_USE_REAL_HOME;
    delete process.env.JUNO_LIVE_TEST_QUIET;
