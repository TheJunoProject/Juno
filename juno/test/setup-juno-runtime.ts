...
const WORKER_RUNTIME_STATE = Symbol.for("juno.testSetupRuntimeState");
const WORKER_RUNTIME_HELPERS = Symbol.for("juno.testSetupRuntimeHelpers");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: JunoConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: JunoConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
