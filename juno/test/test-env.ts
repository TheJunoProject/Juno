...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.JUNO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "JUNO_TEST_FAST", value: process.env.JUNO_TEST_FAST },
    {
      key: "JUNO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.JUNO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "JUNO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.JUNO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "JUNO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.JUNO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "JUNO_STATE_DIR", value: process.env.JUNO_STATE_DIR },
    { key: "JUNO_CONFIG_PATH", value: process.env.JUNO_CONFIG_PATH },
    { key: "JUNO_GATEWAY_PORT", value: process.env.JUNO_GATEWAY_PORT },
    { key: "JUNO_BRIDGE_ENABLED", value: process.env.JUNO_BRIDGE_ENABLED },
    { key: "JUNO_BRIDGE_HOST", value: process.env.JUNO_BRIDGE_HOST },
    { key: "JUNO_BRIDGE_PORT", value: process.env.JUNO_BRIDGE_PORT },
    { key: "JUNO_CANVAS_HOST_PORT", value: process.env.JUNO_CANVAS_HOST_PORT },
    { key: "JUNO_TEST_HOME", value: process.env.JUNO_TEST_HOME },
    { key: "JUNO_AGENT_DIR", value: process.env.JUNO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "juno-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.JUNO_TEST_HOME = tempHome;
  process.env.JUNO_TEST_FAST = "1";
  process.env.JUNO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.JUNO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.JUNO_BRIDGE_ENABLED;
  delete process.env.JUNO_BRIDGE_HOST;
  delete process.env.JUNO_BRIDGE_PORT;
  delete process.env.JUNO_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.JUNO_STATE_DIR = path.join(tempHome, ".juno");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.JUNO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.JUNO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".juno");
  const priorIsolatedHome = params.env.JUNO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".juno")
  ) {
    realStateDir = path.join(params.realHome, ".juno");
  }
  const tempStateDir = path.join(params.tempHome, ".juno");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.JUNO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.JUNO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "juno.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "juno.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.JUNO_LIVE_TEST === "1" ||
    process.env.JUNO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.JUNO_LIVE_USE_REAL_HOME);
