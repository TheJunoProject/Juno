    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-catalog-"));
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@juno/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "juno-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-catalog-state-"));
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(path.join(os.tmpdir(), "juno-catalog-home-"));
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@juno/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              JUNO_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              JUNO_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@juno/default-env-demo",
