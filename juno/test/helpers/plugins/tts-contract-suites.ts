...
function asLegacyTtsConfig(value: unknown): JunoConfig {
  return value as JunoConfig;
}

function asLegacyJunoConfig(value: Record<string, unknown>): JunoConfig {
  return value as unknown as JunoConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): JunoConfig {
...
function createResolvedSummarizationConfig(cfg: JunoConfig): ResolvedTtsConfig {
  const rawConfig =
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: JunoConfig = asLegacyJunoConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.JUNO_TTS_PREFS;
      process.env.JUNO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.JUNO_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: JunoConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
