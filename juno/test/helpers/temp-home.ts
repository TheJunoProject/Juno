...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked JUNO_HOME.
  delete process.env.JUNO_HOME;
  process.env.JUNO_STATE_DIR = path.join(base, ".juno");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "juno-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".juno", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
