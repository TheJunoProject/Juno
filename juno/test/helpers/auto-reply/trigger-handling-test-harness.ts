...
import type { JunoConfig } from "../../../src/config/types.juno.js";
...
const piEmbeddedMocks = getSharedMocks("juno.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("juno.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("juno.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("juno.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    junoHome: process.env.JUNO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("JUNO_HOME", snapshot.junoHome);
  restoreKey("JUNO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.JUNO_STATE_DIR = join(home, ".juno");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "juno-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".juno", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): JunoConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "juno"),
