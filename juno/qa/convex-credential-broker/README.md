...
- `JUNO_QA_CONVEX_SECRET_MAINTAINER`
- `JUNO_QA_CONVEX_SECRET_CI`

Client URL policy:

- `JUNO_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `JUNO_QA_ALLOW_INSECURE_HTTP=1`.
...
Admin endpoints require `JUNO_QA_CONVEX_SECRET_MAINTAINER`.
