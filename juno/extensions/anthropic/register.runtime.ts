import { formatCliCommand, parseDurationMs } from "juno/plugin-sdk/cli-runtime";
import type {
  JunoPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type JunoConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "juno/plugin-sdk/provider-auth";
import {
  cloneFirstTemplateModel,
  type ProviderPlugin,
} from "juno/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "juno/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty } from "juno/plugin-sdk/text-runtime";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("juno models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("juno models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("juno doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: JunoPluginApi): void {
