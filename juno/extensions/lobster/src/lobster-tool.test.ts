...
import type { JunoPluginApi, JunoPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<JunoPluginApi> = {}): JunoPluginApi {
...
function fakeCtx(overrides: Partial<JunoPluginToolContext> = {}): JunoPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: JunoPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
