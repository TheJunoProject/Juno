import { NON_ENV_SECRETREF_MARKER } from "juno/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "juno/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync } from "juno/plugin-sdk/testing";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      await expect(maybeTool.execute({ query: "Juno" })).resolves.toMatchObject({
