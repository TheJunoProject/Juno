...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "juno/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "juno/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "juno/plugin-sdk/provider-web-search";
import {
  applyXaiModelCompat,
  normalizeXaiModelId,
  resolveXaiTransport,
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.juno.ai/tools/code-execution",
        });
      }
