...
} from "juno/plugin-sdk/provider-web-search";
import {
  normalizeSecretInputString,
  resolveSecretInputString,
} from "juno/plugin-sdk/secret-input";
...
function readLegacyGrokFallbackAuth(cfg?: JunoConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: JunoConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function readLegacyGrokApiKey(cfg?: JunoConfig): string | undefined {
...
export function readPluginXaiWebSearchApiKey(cfg?: JunoConfig): string | undefined {
  const resolved = readPluginXaiWebSearchApiKeyResult(cfg);
  return resolved.status === "available" ? resolved.value : undefined;
}

export function resolveFallbackXaiAuth(cfg?: JunoConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: JunoConfig): string | undefined {
