...
const TEST_CFG = {} as JunoConfig;

describe("listMicrosoftVoices", () => {
  const originalFetch = globalThis.fetch;
  const proxyEnvKeys = [
    "JUNO_DEBUG_PROXY_ENABLED",
    "JUNO_DEBUG_PROXY_DB_PATH",
    "JUNO_DEBUG_PROXY_BLOB_DIR",
    "JUNO_DEBUG_PROXY_SESSION_ID",
  ] as const;
  ...
  it("records voice discovery exchanges in debug proxy capture mode", async () => {
    ...
    process.env.JUNO_DEBUG_PROXY_ENABLED = "1";
    process.env.JUNO_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.JUNO_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.JUNO_DEBUG_PROXY_SESSION_ID = "ms-voices-session";
    ...
    const store = getDebugProxyCaptureStore(
      process.env.JUNO_DEBUG_PROXY_DB_PATH,
      process.env.JUNO_DEBUG_PROXY_BLOB_DIR,
    );
    store.upsertSession({
      ...
      sourceScope: "juno",
      sourceProcess: "juno",
      dbPath: process.env.JUNO_DEBUG_PROXY_DB_PATH,
      blobDir: process.env.JUNO_DEBUG_PROXY_BLOB_DIR,
    });
  ...
  it("does not double-capture voice discovery when the global fetch patch is installed", async () => {
    ...
    process.env.JUNO_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.JUNO_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    process.env.JUNO_DEBUG_PROXY_SESSION_ID = "ms-voices-global-session";
