...
`juno.invoke --each`) without adding new Juno code per workflow.
...
This extension depends on Juno internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Juno extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.juno/extensions` as a standalone plugin directory.
