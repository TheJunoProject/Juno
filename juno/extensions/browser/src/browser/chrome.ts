...
export function resolveJunoUserDataDir(profileName = DEFAULT_JUNO_BROWSER_PROFILE_NAME) {
...
export function buildJunoChromeLaunchArgs(params: {
...
export async function launchJunoChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveJunoUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_JUNO_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateJunoProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`juno browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopJunoChrome(
