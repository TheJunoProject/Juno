...
function localJunoProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "juno",
...
function createLocalJunoResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localJunoProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveJunoUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-reset-"));
    const profileDir = path.join(tempRoot, "juno");
    ...
    const ops = createLocalJunoResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveJunoUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-reset-no-own-"));
    const profileDir = path.join(tempRoot, "juno");
    fs.mkdirSync(profileDir, { recursive: true });
