  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-juno-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-juno-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-juno-labels", "1");
          ...
          tag.setAttribute("data-juno-labels", "1");
