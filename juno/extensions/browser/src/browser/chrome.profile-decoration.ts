...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".juno-profile-decorated");
}
 ...
 */
export function decorateJunoProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_JUNO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_JUNO_BROWSER_COLOR).toUpperCase();
