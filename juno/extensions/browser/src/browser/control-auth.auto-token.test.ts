...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => JunoConfig>(),
  writeConfigFile: vi.fn<(cfg: JunoConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<JunoConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: JunoConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): JunoConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: JunoConfig) {
