  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        defaultProfile: "juno",
        profiles: {
          juno: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.JUNO_GATEWAY_PORT;
    process.env.JUNO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.JUNO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.JUNO_GATEWAY_PASSWORD;
