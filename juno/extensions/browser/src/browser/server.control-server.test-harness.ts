...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.JUNO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "juno",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/juno" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    juno: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchJunoChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.JUNO_GATEWAY_PORT;
  process.env.JUNO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.JUNO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.JUNO_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.JUNO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.JUNO_GATEWAY_PASSWORD;
  } else {
    process.env.JUNO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
