...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredJunoTmpDir: vi.fn(() => "/tmp/juno"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-juno-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-juno-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredJunoTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredJunoTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredJunoTmpDir.mockReturnValue("/tmp/juno");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".juno-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredJunoTmpDir.mockReturnValue("/tmp/juno-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "juno-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "juno-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredJunoTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "juno-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "juno-preferred", "downloads")}${path.sep}`),
    );
  });
