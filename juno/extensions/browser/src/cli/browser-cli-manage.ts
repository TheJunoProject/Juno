...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "juno";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "juno" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (juno|existing-session). Default: juno")
    .action(
      async (
        opts: {
          name: string;
