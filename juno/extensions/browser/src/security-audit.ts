import type { JunoPluginSecurityAuditContext } from "juno/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "juno/plugin-sdk/secret-input";
import { formatCliCommand } from "juno/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "juno/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: JunoPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("juno security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.JUNO_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.JUNO_GATEWAY_PASSWORD) ||
