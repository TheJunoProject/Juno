import { CUSTOM_LOCAL_AUTH_MARKER } from "juno/plugin-sdk/provider-auth";
import type { JunoConfig } from "juno/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "juno/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "juno/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "juno/plugin-sdk/provider-setup";
import type { WizardPrompter } from "juno/plugin-sdk/setup";
...
vi.mock("juno/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("juno/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("juno/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("juno/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): JunoConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as JunoConfig),
    env: params?.env ?? {},
