...
import type { JunoPluginApi } from "juno/plugin-sdk/plugin-entry";
...
vi.mock("juno/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("juno/plugin-sdk/config-runtime")>(
...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedPiAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/juno-session-store.json"),
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as JunoPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    const mkdtempSpy = vi
      .spyOn(fs, "mkdtemp")
      .mockResolvedValue("/tmp/juno-active-memory-temp");
    ...
    expect(runEmbeddedPiAgent.mock.calls.at(-1)?.[0]?.sessionFile).toBe(
      "/tmp/juno-active-memory-temp/session.jsonl",
    );
    expect(rmSpy).toHaveBeenCalledWith("/tmp/juno-active-memory-temp", {
      recursive: true,
      force: true,
    });
  });
