...
} from "juno/plugin-sdk/agent-runtime";
import {
  resolveSessionStoreEntry,
  updateSessionStore,
  type JunoConfig,
} from "juno/plugin-sdk/config-runtime";
import { definePluginEntry, type JunoPluginApi } from "juno/plugin-sdk/plugin-entry";
...
function resolvePersistentTranscriptBaseDir(api: JunoPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: JunoPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: JunoConfig): boolean {
...
function resolveActiveMemoryPluginConfigFromConfig(cfg: JunoConfig): unknown {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredJunoTmpDir(), "juno-active-memory-"));
