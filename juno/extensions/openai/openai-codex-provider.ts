...
import { loginOpenAICodexOAuth } from "juno/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "juno/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "juno/plugin-sdk/text-runtime";
...
async function runImportOpenAICodexCliAuth(ctx: ProviderAuthContext) {
  ...
  return {
    ...
    defaultModel: OPENAI_CODEX_DEFAULT_MODEL,
    notes: ["Imported existing Codex CLI login into Juno canonical auth."],
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `juno models auth login --provider openai-codex` or `juno configure`.";
}
