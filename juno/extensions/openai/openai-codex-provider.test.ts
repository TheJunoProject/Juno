...
describe("openai codex provider", () => {
  ...
  it("returns deprecated-profile doctor guidance for legacy Codex CLI ids", () => {
    ...
    expect(
      ...
      "Deprecated profile. Run `juno models auth login --provider openai-codex` or `juno configure`.",
    );
  });
  ...
  it("uses the provider auth context env when importing Codex CLI auth", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-openai-codex-provider-"));
