...
import { normalizeProviderId } from "juno/plugin-sdk/provider-model-shared";
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-juno-turn-id": turnId,
      "x-juno-turn-attempt": attempt,
    },
    metadata: {
      juno_session_id: sessionHeaders["x-juno-session-id"] ?? "",
      juno_turn_id: turnId,
      juno_turn_attempt: attempt,
      juno_transport: ctx.transport,
    },
  };
}
