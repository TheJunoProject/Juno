...
import type { ResolvedTtsConfig } from "juno/plugin-sdk/agent-runtime";
import type { JunoConfig } from "juno/plugin-sdk/config-runtime";
import { loadConfig } from "juno/plugin-sdk/config-runtime";
import { encodePngRgba, fillPixel } from "juno/plugin-sdk/media-runtime";
...
const LIVE_MODEL_ID = process.env.JUNO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.JUNO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-1";
const LIVE_VISION_MODEL = process.env.JUNO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.JUNO_LIVE_TEST === "1";
...
function createLiveConfig(): JunoConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("transcribes synthesized speech through the registered media provider", async () => {
    ...
    expect(collapsedText).toContain("juno");
