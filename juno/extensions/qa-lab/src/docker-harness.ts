...
const QA_LAB_UI_OVERLAY_DIR = "/opt/juno-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        JUNO_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      JUNO_CONFIG_PATH: /tmp/juno/juno.json
      ...
      JUNO_PROFILE: ""
    volumes:
      - ./state:/opt/juno-scaffold:ro
      - ${repoMount}:/opt/juno-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/juno/workspace /tmp/juno/state && cp /opt/juno-scaffold/juno.json /tmp/juno/juno.json && cp -R /opt/juno-scaffold/seed-workspace/. /tmp/juno/workspace/ && ln -snf /opt/juno-repo /tmp/juno/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t juno:qa-local-prebaked --build-arg JUNO_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "juno:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "juno.json"),
  ];
