...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "juno.json"),
    ...
    expect(compose).toContain('      - "http://juno-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/juno-qa-lab-ui"');
    expect(compose).toContain(":/opt/juno-repo:ro");
    expect(compose).toContain("./state:/opt/juno-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/juno-scaffold/seed-workspace/. /tmp/juno/workspace/",
    );
    expect(compose).toContain("JUNO_CONFIG_PATH: /tmp/juno/juno.json");
    expect(compose).toContain("JUNO_STATE_DIR: /tmp/juno/state");
    expect(compose).toContain('JUNO_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("JUNO_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "juno.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/juno/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/juno",
        imageName: "juno:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("juno:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t juno:qa-local-prebaked --build-arg JUNO_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/juno",
      ),
    ]);
  });
});
