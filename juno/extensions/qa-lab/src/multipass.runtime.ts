...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/juno-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/juno";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/juno-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `juno-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "JUNO_CONFIG_PATH" &&
          key !== "JUNO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `JUNO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `JUNO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm juno qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredJunoTmpDir(), `${plan.vmName}-qa-suite-`),
  );
