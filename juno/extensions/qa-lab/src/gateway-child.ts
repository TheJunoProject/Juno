...
import type { JunoConfig } from "juno/plugin-sdk/config-runtime";
...
import { fetchWithSsrFGuard } from "juno/plugin-sdk/ssrf-runtime";
import { resolvePreferredJunoTmpDir } from "juno/plugin-sdk/temp-path";
import {
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    JUNO_HOME: params.homeDir,
    JUNO_CONFIG_PATH: params.configPath,
    JUNO_STATE_DIR: params.stateDir,
    JUNO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    JUNO_NO_RESPAWN: "1",
    JUNO_TEST_FAST: "1",
    JUNO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    JUNO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { JUNO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { JUNO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: JunoConfig) => JunoConfig;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredJunoTmpDir(), "juno-qa-suite-"),
  );
  ...
  const configPath = path.join(tempRoot, "juno.json");
  ...
  const keepTemp = process.env.JUNO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
