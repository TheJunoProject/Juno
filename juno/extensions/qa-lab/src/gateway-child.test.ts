...
vi.mock("juno/plugin-sdk/ssrf-runtime", () => ({
  fetchWithSsrFGuard: fetchWithSsrFGuardMock,
}));

vi.mock("juno/plugin-sdk/temp-path", () => ({
  resolvePreferredJunoTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/juno-qa/juno.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/juno-qa/home",
    stateDir: "/tmp/juno-qa/state",
    xdgConfigHome: "/tmp/juno-qa/xdg-config",
    xdgDataHome: "/tmp/juno-qa/xdg-data",
    xdgCacheHome: "/tmp/juno-qa/xdg-cache",
    bundledPluginsDir: "/tmp/juno-qa/bundled-plugins",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.JUNO_TEST_FAST).toBe("1");
    expect(env.JUNO_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.JUNO_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.JUNO_BUNDLED_PLUGINS_DIR).toBe("/tmp/juno-qa/bundled-plugins");
    expect(env.JUNO_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        JUNO_LIVE_OPENAI_KEY: "openai-live",
        JUNO_LIVE_ANTHROPIC_KEY: "anthropic-live",
        JUNO_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing Juno home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/juno-qa/home");
    expect(env.JUNO_HOME).toBe("/tmp/juno-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping Juno home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.JUNO_HOME).toBe("/tmp/juno-qa/home");
    expect(env.JUNO_STATE_DIR).toBe("/tmp/juno-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping Juno home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        JUNO_LIVE_ANTHROPIC_KEY: "anthropic-live",
        JUNO_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.JUNO_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.JUNO_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        JUNO_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.JUNO_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.JUNO_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        JUNO_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        JUNO_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.JUNO_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.JUNO_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          JUNO_LIVE_ANTHROPIC_KEY: "anthropic-live",
          JUNO_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          JUNO_LIVE_GEMINI_KEY: "gemini-live",
          JUNO_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.JUNO_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.JUNO_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.JUNO_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.JUNO_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        JUNO_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      __testing.assertQaArtifactDirWithinRepo("/tmp/juno-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "juno.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "juno.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "juno.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@juno/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "index.js"),
      [
        'import { normalizeAccountId } from "juno/plugin-sdk/account-id";',
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "package.json"),
      JSON.stringify({ name: "juno", type: "module" }, null, 2),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@juno/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "juno.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { JUNO_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ juno: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ juno: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ juno: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
