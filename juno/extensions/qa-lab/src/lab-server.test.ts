...
vi.mock("juno/plugin-sdk/qa-channel", async () => await import("../../qa-channel/api.js"));
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        'const fs = require("node:fs");',
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.JUNO_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "process.on('SIGTERM', () => {",
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    cleanups.push(async () => {
      delete process.env.JUNO_DEBUG_PROXY_DB_PATH;
      delete process.env.JUNO_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
