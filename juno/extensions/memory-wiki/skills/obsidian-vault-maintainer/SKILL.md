...
- Start from `juno wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `juno wiki obsidian status` before shelling out, then prefer the dedicated helpers like `juno wiki obsidian search`, `juno wiki obsidian open`, `juno wiki obsidian command`, and `juno wiki obsidian daily`.
