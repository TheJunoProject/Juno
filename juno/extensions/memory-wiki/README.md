...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Juno**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.juno-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
juno wiki doctor
juno wiki init
juno wiki ingest ./notes/alpha.md
juno wiki compile
juno wiki lint
juno wiki search "alpha"
juno wiki get entity.alpha --from 1 --lines 80

juno wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

juno wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

juno wiki bridge import
juno wiki unsafe-local import

juno wiki obsidian status
juno wiki obsidian search "alpha"
juno wiki obsidian open syntheses/alpha-summary.md
juno wiki obsidian command workspace:quick-switcher
juno wiki obsidian daily
```
...
- The compiled agent digests in `.juno-wiki/cache/agent-digest.json` and `.juno-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
