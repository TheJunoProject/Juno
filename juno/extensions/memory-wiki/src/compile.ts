...
import {
  ...
  withTrailingNewline,
} from "juno/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".juno-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".juno-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- juno:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- juno:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- juno:wiki:index:start -->",
      endMarker: "<!-- juno:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- juno:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- juno:wiki:${group.dir}:index:end -->`,
