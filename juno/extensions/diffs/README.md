...
Read-only diff viewer plugin for **Juno** agents.
...
Set plugin-wide defaults in `~/.juno/juno.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/juno",
        },
      },
    },
  },
...
Before:
Juno supports plugins.

After:
Juno supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/juno-diffs`).
