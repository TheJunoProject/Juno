...
import type { JunoPluginApi, JunoPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("juno-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<JunoPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: JunoPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<JunoPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as JunoPluginApi);
