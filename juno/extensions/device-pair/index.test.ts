...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredJunoTmpDir: vi.fn(() => path.join(os.tmpdir(), "juno-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredJunoTmpDir: pluginApiMocks.resolvePreferredJunoTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as JunoPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: JunoPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredJunoTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredJunoTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Juno iOS app:");
    expect(text).toContain("![Juno pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Juno iOS app:");
