...
const JUNO_OPENSHELL_E2E = process.env.JUNO_E2E_OPENSHELL === "1";
const JUNO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const JUNO_OPENSHELL_COMMAND =
  process.env.JUNO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "juno-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && JUNO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: JUNO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(JUNO_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(JUNO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-openshell-e2e-"));
      ...
      const gatewayName = `juno-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `juno-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/juno-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: JUNO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("juno-openshell-e2e");
