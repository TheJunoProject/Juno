...
function normalizeDenyList(cfg: JunoPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: JunoPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: JunoPluginApi) {
    ...
    const timerService: JunoPluginService = {
