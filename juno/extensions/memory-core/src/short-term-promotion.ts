...
import { formatMemoryDreamingDay } from "juno/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "juno/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "juno-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*juno-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*juno-memory-promotion:([^\n]+?)\s*-->/gi);
