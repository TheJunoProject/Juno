...
import { resolveMemoryDreamingConfig } from "juno/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "juno/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: JunoConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: JunoConfig, enabled: boolean): JunoConfig {
...
function formatStatus(cfg: JunoConfig): string {
...
export function registerDreamingCommand(api: JunoPluginApi): void {
  api.registerCommand({
    name: "dreaming",
