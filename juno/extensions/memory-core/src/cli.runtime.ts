...
import { resolveMemoryRemDreamingConfig } from "juno/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "juno/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: JunoConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredJunoTmpDir(), "juno-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: JunoConfig): string {
...
function resolveAgent(cfg: JunoConfig, agent?: string) {
...
function resolveAgentIds(cfg: JunoConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: juno memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredJunoTmpDir(), "juno-rem-backfill-"),
      );
      try {
