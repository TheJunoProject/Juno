...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("juno/plugin-sdk/memory-core-host-runtime-cli"),
    import("juno/plugin-sdk/memory-core-host-runtime-core"),
    import("juno/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("juno/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("juno/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("juno/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("juno/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await runMemoryCli(["status"]);
      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Fix: juno memory status --fix --agent main"),
      );
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("juno:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## Juno / runtime / workflow preferences and corrections",
      ...
      expect(rendered).not.toContain("jpjunohq updated and running");
      expect(close).toHaveBeenCalled();
    });
  });
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- juno:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- juno:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("juno-memory-promotion:");
