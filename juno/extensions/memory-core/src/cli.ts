...
import {
  ...
  formatHelpExamples,
  theme,
} from "juno/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["juno memory status", "Show index and provider status."],
          [
            "juno memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["juno memory status --deep", "Probe embedding provider readiness."],
          ["juno memory index --force", "Force a full reindex."],
          ['juno memory search "meeting notes"', "Quick search using positional query."],
          [
            'juno memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `juno memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "juno memory rem-backfill --path ./memory",
          ...
          ["juno memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.juno.ai/cli/memory")}\n`,
    );
