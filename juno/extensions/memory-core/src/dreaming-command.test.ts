...
function resolveStoredDreaming(config: JunoConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: JunoConfig = {}) {
  const registered: { command?: JunoPluginCommandDefinition } = {};
  let runtimeConfig: JunoConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: JunoConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: JunoPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as JunoPluginApi;
