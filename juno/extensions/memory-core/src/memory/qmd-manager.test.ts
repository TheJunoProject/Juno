...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("juno.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("juno.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("juno.qmdEmbedQueueTail");
...
vi.mock("juno/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("juno/plugin-sdk/memory-core-host-engine-foundation")
  >("juno/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("juno/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("juno/plugin-sdk/file-lock")>(
    "juno/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { JunoConfig } from "juno/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: JunoConfig }) {
