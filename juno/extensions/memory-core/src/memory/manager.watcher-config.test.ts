...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): JunoConfig {
    const defaults: NonNullable<NonNullable<JunoConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: JunoConfig) {
