...
import { withFileLock } from "juno/plugin-sdk/file-lock";
import {
  ...
  writeFileWithinRoot,
  type JunoConfig,
} from "juno/plugin-sdk/memory-core-host-engine-foundation";
import { resolveAgentContextLimits } from "juno/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Juno search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: JunoConfig;
