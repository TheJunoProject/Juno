...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "juno/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("juno/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createQmdCfg(agentId: string): JunoConfig {
  return {
...
function createBuiltinCfg(agentId: string): JunoConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "juno-qmd-workspace-"));
