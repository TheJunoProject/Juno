...
import * as configRuntimeModule from "juno/plugin-sdk/config-runtime";
import {
  RequestScopedSubagentRuntimeError,
  SUBAGENT_RUNTIME_REQUEST_SCOPE_ERROR_CODE,
} from "juno/plugin-sdk/error-runtime";
import * as memoryCoreHostRuntimeCoreModule from "juno/plugin-sdk/memory-core-host-runtime-core";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("juno.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("juno:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("juno-dreaming-backfill-");
    ...
    expect(content.match(/juno:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("juno-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- juno:dreaming:diary:start -->");
    expect(content).toContain("<!-- juno:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- juno:dreaming:diary:start -->");
    const end = content.indexOf("<!-- juno:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- juno:dreaming:light:start -->\n- Candidate: test\n<!-- juno:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("juno-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("juno-dreaming-narrative-");
    const stateDir = await createTempWorkspace("juno-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("juno-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("juno-dreaming-narrative-");
