...
metadata: { "juno": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Juno `sessions_spawn`
- **File I/O** == Juno `read`/`write`
- **Remote fetch** == Juno `web_fetch` (or `exec` with curl when POST is required)
