...
type OllamaProviderConfigOverride = Partial<{
  ...
  baseUrl: string;
  models: NonNullable<
    NonNullable<NonNullable<JunoConfig["models"]>["providers"]>[string]
  >["models"];
}>;

function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): JunoConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): JunoConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the Ollama experimental search endpoint", async () => {
    ...
    const result = await tool.execute({ query: "juno docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://juno.ai/docs" }],
    });
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest juno release" })).rejects.toThrow(
      "ollama signin",
    );
  });
