...
import { resolveDefaultSecretProviderAlias } from "juno/plugin-sdk/provider-auth";
import { resolveSecretInputString, normalizeSecretInput } from "juno/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<JunoConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: JunoConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: JunoConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: JunoConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: JunoConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: JunoConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: JunoConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: JunoConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: JunoConfig, override?: number): number {
