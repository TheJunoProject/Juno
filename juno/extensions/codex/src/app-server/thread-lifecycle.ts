import { embeddedAgentLog, type EmbeddedRunAttemptParams } from "juno/plugin-sdk/agent-harness";
...
function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  const sections = [
    "You are running inside Juno. Use Juno dynamic tools for messaging, cron, sessions, and host actions when available.",
    "Preserve the user's existing channel/session context. If sending a channel reply, use the Juno messaging tool instead of describing that you would reply.",
