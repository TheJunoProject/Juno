...
const mocks = vi.hoisted(() => ({
  ...
  resolveJunoAgentDir: vi.fn(() => "/tmp/juno-agent"),
}));
...
describe("shared Codex app-server client", () => {
  ...
  afterEach(() => {
    ...
    mocks.resolveJunoAgentDir.mockClear();
  });
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    harness.send({
      id: initialize.id,
      result: { userAgent: "juno/0.117.9 (macOS; test)" },
    });
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    second.send({
      id: initialize.id,
      result: { userAgent: "juno/0.118.0 (macOS; test)" },
    });
