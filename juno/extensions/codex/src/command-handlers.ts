import type { PluginCommandContext } from "juno/plugin-sdk/plugin-entry";
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Juno session file.";
  }
  ...
  return `Attached this Juno session to Codex thread ${effectiveThreadId}.`;
}
...
async function startThreadAction(
  ...
  if (!ctx.sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an Juno session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(ctx.sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Juno session yet.`;
  }
