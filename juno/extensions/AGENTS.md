...
- Extension production code should import from `juno/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `juno.plugin.json` and the package
  `juno` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `juno/plugin-sdk/*`.
