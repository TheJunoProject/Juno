...
The bundled ACPX extension is a thin Juno wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `juno/acpx`, not in this extension.
...
Use this flow when Juno needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `juno/acpx` repo first.
2. In Juno, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Juno and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Juno ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Juno-specific glue, move it to `juno/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
