...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: JunoPluginServiceContext): Promise<void> {
      if (process.env.JUNO_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (JUNO_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.JUNO_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: JunoPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
