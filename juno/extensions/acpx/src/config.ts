...
import type { z } from "juno/plugin-sdk/zod";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "juno-plugin-tools";

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "juno.plugin.json")) &&
...
function resolveAcpxPluginRootFromJunoLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Juno layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromJunoLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveJunoRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openJunoRoot = resolveJunoRoot(pluginRoot);
  const distEntry = path.join(openJunoRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(openJunoRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
