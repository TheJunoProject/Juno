...
import type { ReplyPayload } from "juno/plugin-sdk/reply-payload";
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "juno/plugin-sdk/speech-core";
...
vi.mock("juno/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): JunoConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    const cfg = createTtsConfig("juno-speech-core-tts-test");
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    const cfg = createTtsConfig("juno-speech-core-tts-slack-test");
    const payload: ReplyPayload = {
