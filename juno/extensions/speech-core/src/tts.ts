...
import { normalizeChannelId, type ChannelId } from "juno/plugin-sdk/channel-targets";
import type {
  JunoConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "juno/plugin-sdk/logging-core";
import {
  resolveSendableOutboundReplyParts,
  type ReplyPayload,
} from "juno/plugin-sdk/reply-payload";
import { isVerbose, logVerbose } from "juno/plugin-sdk/runtime-env";
import { resolvePreferredJunoTmpDir } from "juno/plugin-sdk/sandbox";
import {
  ...
  resolveUserPath,
  stripMarkdown,
} from "juno/plugin-sdk/text-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.JUNO_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: JunoConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: JunoConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: JunoConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: JunoConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: JunoConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: JunoConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredJunoTmpDir();
