...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "JunoKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "JunoKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.JUNO_SPARSE_PROFILE || process.env.JUNO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because JUNO_A2UI_SKIP_MISSING=1 or JUNO_SPARSE_PROFILE is set.",
      );
      return;
    }
