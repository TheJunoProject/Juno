...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.JUNO_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "juno.plugin.json");
    ...
    if (packageJson.juno && "extensions" in packageJson.juno) {
      packageJson.juno = {
        ...packageJson.juno,
        extensions: rewritePackageExtensions(packageJson.juno.extensions),
        ...(typeof packageJson.juno.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.juno.setupEntry) }
          : {}),
      };
    }
