...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "junolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
junolog - Juno Logging Utility

USAGE:
    junolog [OPTIONS]

DESCRIPTION:
    View Juno logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Juno logs flow through the macOS unified log (subsystem: ai.juno).
...
QUICK START:
    junolog -n 100             Show last 100 lines from all components
    junolog -f                 Follow logs in real-time
    junolog -e                 Show only errors
    junolog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    junolog                   Show last 50 lines from past 5 minutes (default)
    junolog -f                Stream logs continuously
    junolog -n 100            Show last 100 lines
    junolog -e                Show only recent errors
    junolog -l 30m -n 200     Show last 200 lines from past 30 minutes
    junolog -c ServerManager  Show recent ServerManager logs
    junolog -s "fail"         Search for "fail" in recent logs
    junolog --server -e       Show recent server errors
    junolog -f -d             Stream debug logs continuously
