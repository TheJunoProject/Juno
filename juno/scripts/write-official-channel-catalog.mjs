...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.juno) ? packageJson.juno : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.juno?.channel?.id) || left.name;
    const rightId = trimString(right.juno?.channel?.id) || right.name;
