...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeTempDir(pluginDir, `.juno-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.juno-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeTempDir(
    targetParentDir,
    `.juno-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `juno-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".juno-runtime-deps-stamp.json");
}
