...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@juno/${params.dirName}`) {
    return false;
  }
...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.juno?.extensions)
    ? packageJson.juno.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.juno?.setupEntry === "string" &&
    packageJson.juno.setupEntry.trim().length > 0
      ? packageJson.juno.setupEntry
      : undefined;
  if (setupEntry) {
...
function shouldStageBundledPluginRuntimeDependencies(packageJson) {
  return packageJson?.juno?.bundle?.stageRuntimeDependencies === true;
}
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "juno.plugin.json");
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "juno.plugin.json"));
    }
    if (hasPackageJson) {
