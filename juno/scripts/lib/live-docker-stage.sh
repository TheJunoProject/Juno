...
juno_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"

  ln -s /app/node_modules "$dest_dir/node_modules"
  ln -s /app/dist "$dest_dir/dist"
  if [ -d /app/dist-runtime/extensions ]; then
    export JUNO_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
  elif [ -d /app/dist/extensions ]; then
    export JUNO_BUNDLED_PLUGINS_DIR=/app/dist/extensions
  fi
}

juno_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.juno"
  ...
  export JUNO_STATE_DIR="$dest_dir"
  export JUNO_CONFIG_PATH="$dest_dir/juno.json"
}

juno_live_prepare_staged_config() {
  if [ ! -f "${JUNO_CONFIG_PATH:-}" ]; then
    return 0
  fi
