...
function isSystemThrottleDisabled(env) {
  const normalized = env.JUNO_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.JUNO_VITEST_MAX_WORKERS ?? env.JUNO_TEST_WORKERS);
  if (override !== null) {
