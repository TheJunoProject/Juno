...
export DISPLAY=:1
export HOME=/tmp/juno-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${JUNO_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${JUNO_BROWSER_CDP_SOURCE_RANGE:-}"
VNC_PORT="${JUNO_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${JUNO_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${JUNO_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${JUNO_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${JUNO_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${JUNO_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${JUNO_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${JUNO_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${JUNO_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${JUNO_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_SOURCE_RANGE}" ]]; then
  ...
  echo "[sandbox-browser] Set JUNO_BROWSER_CDP_SOURCE_RANGE to an explicit CIDR to enable CDP access." >&2
else
