...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const junoMeta =
    ...
    "juno" in source.packageJson
      ? (source.packageJson.juno as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    junoMeta &&
    typeof junoMeta.channel === "object" &&
    junoMeta.channel &&
    !Array.isArray(junoMeta.channel)
      ? (junoMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
