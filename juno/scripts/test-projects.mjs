...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.JUNO_VITEST_MAX_WORKERS || env.JUNO_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      JUNO_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.JUNO_TEST_PROJECTS_PARALLEL &&
        !process.env.JUNO_VITEST_MAX_WORKERS &&
        !process.env.JUNO_TEST_WORKERS &&
