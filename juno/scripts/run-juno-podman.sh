#!/usr/bin/env bash
# Rootless Juno in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-juno-podman.sh launch        # Start gateway
#   ./scripts/run-juno-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   juno --container juno dashboard --no-open
#   juno --container juno channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      JUNO_GATEWAY_TOKEN|JUNO_PODMAN_CONTAINER|JUNO_PODMAN_IMAGE|JUNO_IMAGE|JUNO_PODMAN_PULL|JUNO_PODMAN_GATEWAY_HOST_PORT|JUNO_GATEWAY_PORT|JUNO_PODMAN_BRIDGE_HOST_PORT|JUNO_BRIDGE_PORT|JUNO_GATEWAY_BIND|JUNO_PODMAN_USERNS|JUNO_BIND_MOUNT_OPTIONS|JUNO_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-juno-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${JUNO_CONFIG_DIR:-$EFFECTIVE_HOME/.juno}"
ENV_FILE="${JUNO_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${JUNO_CONFIG_DIR:-$EFFECTIVE_HOME/.juno}"
ENV_FILE="${JUNO_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${JUNO_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${JUNO_PODMAN_CONTAINER:-juno}"
JUNO_IMAGE="${JUNO_PODMAN_IMAGE:-${JUNO_IMAGE:-juno:local}}"
PODMAN_PULL="${JUNO_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${JUNO_PODMAN_GATEWAY_HOST_PORT:-${JUNO_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${JUNO_PODMAN_BRIDGE_HOST_PORT:-${JUNO_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${JUNO_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$JUNO_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  JUNO_CONTAINER="" JUNO_CONFIG_DIR="$config_dir" \
    juno config get gateway.bind 2>/dev/null || true
}
...
# JUNO_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${JUNO_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v juno >/dev/null 2>&1; then
    echo "Warning: juno not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    JUNO_CONTAINER="" JUNO_CONFIG_DIR="$config_dir" \
      juno config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    JUNO_CONTAINER="" JUNO_CONFIG_DIR="$config_dir" \
      juno config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  JUNO_CONTAINER="" JUNO_CONFIG_DIR="$config_dir" \
    juno config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${JUNO_GATEWAY_TOKEN:-}" ]]; then
  export JUNO_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "JUNO_GATEWAY_TOKEN" "$JUNO_GATEWAY_TOKEN"
  echo "Generated JUNO_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/juno.json"
...
PODMAN_USERNS="${JUNO_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported JUNO_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (JUNO_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${JUNO_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${JUNO_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$JUNO_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.juno/.npm \
    -e JUNO_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.juno:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.juno/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$JUNO_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME juno dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME juno devices approve --latest  # if pairing required"
