...
#
# Suggested cron: */30 * * * * /home/admin/juno/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Juno notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., juno-alerts)
...
STATE_FILE="$HOME/.juno/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Juno if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use juno
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Juno to $NOTIFY_PHONE..."
            juno send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Juno is down. Run: ssh l36 '~/juno/scripts/mobile-reauth.sh'" "urgent"
    exit 1
