...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("juno/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("juno/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced juno/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced juno/plugin-sdk/<subpath> must exist in the public package exports.",
  );
