...
EXTRA_MOUNTS="${JUNO_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${JUNO_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${JUNO_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${JUNO_DOCKER_SOCKET:-}"
TIMEZONE="${JUNO_TZ:-}"
...
read_config_gateway_token() {
  local config_path="$JUNO_CONFIG_DIR/juno.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == JUNO_GATEWAY_TOKEN=* ]]; then
      token="${line#JUNO_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${JUNO_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$JUNO_GATEWAY_PORT" "$JUNO_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$JUNO_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$JUNO_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network juno-cli service because it
  ...
  run_prestart_gateway --entrypoint node juno-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" juno-cli "$@"
}
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "JUNO_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "JUNO_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
JUNO_CONFIG_DIR="${JUNO_CONFIG_DIR:-$HOME/.juno}"
JUNO_WORKSPACE_DIR="${JUNO_WORKSPACE_DIR:-$HOME/.juno/workspace}"

validate_mount_path_value "JUNO_CONFIG_DIR" "$JUNO_CONFIG_DIR"
validate_mount_path_value "JUNO_WORKSPACE_DIR" "$JUNO_WORKSPACE_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "JUNO_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "JUNO_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "JUNO_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "JUNO_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "JUNO_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "JUNO_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$JUNO_CONFIG_DIR"
mkdir -p "$JUNO_WORKSPACE_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$JUNO_CONFIG_DIR/identity"
mkdir -p "$JUNO_CONFIG_DIR/agents/main/agent"
mkdir -p "$JUNO_CONFIG_DIR/agents/main/sessions"

export JUNO_CONFIG_DIR
export JUNO_WORKSPACE_DIR
export JUNO_GATEWAY_PORT="${JUNO_GATEWAY_PORT:-18789}"
export JUNO_BRIDGE_PORT="${JUNO_BRIDGE_PORT:-18790}"
export JUNO_GATEWAY_BIND="${JUNO_GATEWAY_BIND:-lan}"
export JUNO_IMAGE="$IMAGE_NAME"
export JUNO_DOCKER_APT_PACKAGES="${JUNO_DOCKER_APT_PACKAGES:-}"
export JUNO_EXTENSIONS="${JUNO_EXTENSIONS:-}"
export JUNO_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export JUNO_HOME_VOLUME="$HOME_VOLUME_NAME"
export JUNO_ALLOW_INSECURE_PRIVATE_WS="${JUNO_ALLOW_INSECURE_PRIVATE_WS:-}"
export JUNO_SANDBOX="$SANDBOX_ENABLED"
export JUNO_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export JUNO_TZ="$TIMEZONE"
...
if [[ -z "${JUNO_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    JUNO_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $JUNO_CONFIG_DIR/juno.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      JUNO_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      JUNO_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      JUNO_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${JUNO_CONFIG_DIR}:/home/node/.juno"
    gateway_workspace_mount="${JUNO_WORKSPACE_DIR}:/home/node/.juno/workspace"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${JUNO_INSTALL_DOCKER_CLI:-}" ]]; then
    export JUNO_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  JUNO_EXTRA_MOUNTS \
  JUNO_HOME_VOLUME \
  JUNO_DOCKER_APT_PACKAGES \
  JUNO_EXTENSIONS \
  JUNO_SANDBOX \
  JUNO_DOCKER_SOCKET \
  DOCKER_GID \
  JUNO_INSTALL_DOCKER_CLI \
  JUNO_ALLOW_INSECURE_PRIVATE_WS \
  JUNO_TZ

if [[ "$IMAGE_NAME" == "juno:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "JUNO_DOCKER_APT_PACKAGES=${JUNO_DOCKER_APT_PACKAGES}" \
    --build-arg "JUNO_EXTENSIONS=${JUNO_EXTENSIONS}" \
    --build-arg "JUNO_INSTALL_DOCKER_CLI=${JUNO_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Juno metadata subdirectory
# (.juno/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh juno-gateway -c \
  'find /home/node/.juno -xdev -exec chown node:node {} +; \
   [ -d /home/node/.juno/workspace/.juno ] && chown -R node:node /home/node/.juno/workspace/.juno || true'

echo ""
echo "==> Onboarding (interactive)"
echo "Docker setup pins Gateway mode to local."
echo "Gateway runtime bind comes from JUNO_GATEWAY_BIND (default: lan)."
echo "Current runtime bind: $JUNO_GATEWAY_BIND"
echo "Gateway token: $JUNO_GATEWAY_TOKEN"
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm juno-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm juno-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm juno-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.juno.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d juno-gateway

# --- Sandbox setup (opt-in via JUNO_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: juno-sandbox:bookworm-slim"
    run_docker_build \
      -t "juno-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker juno-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg JUNO_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (JUNO_IMAGE=juno:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
YAML
    ...
    echo "WARNING: JUNO_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.juno.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate juno-gateway
  fi
else
...
echo "Config: $JUNO_CONFIG_DIR"
echo "Workspace: $JUNO_WORKSPACE_DIR"
echo "Token: $JUNO_GATEWAY_TOKEN"
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f juno-gateway"
echo "  ${COMPOSE_HINT} exec juno-gateway node dist/index.js health --token \"$JUNO_GATEWAY_TOKEN\""
