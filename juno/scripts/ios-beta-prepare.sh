...
- regenerates apps/ios/Juno.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${JUNO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid JUNO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid JUNO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid JUNO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid JUNO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing JUNO_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(JUNO_URL_SLASH)$(JUNO_URL_SLASH)#g'
)"
...
JUNO_CODE_SIGN_STYLE = Automatic
JUNO_DEVELOPMENT_TEAM = ${TEAM_ID}
JUNO_IOS_SELECTED_TEAM = ${TEAM_ID}
JUNO_APP_BUNDLE_ID = ai.juno.client
JUNO_SHARE_BUNDLE_ID = ai.juno.client.share
JUNO_ACTIVITY_WIDGET_BUNDLE_ID = ai.juno.client.activitywidget
JUNO_WATCH_APP_BUNDLE_ID = ai.juno.client.watchkitapp
JUNO_WATCH_EXTENSION_BUNDLE_ID = ai.juno.client.watchkitapp.extension
JUNO_APP_PROFILE =
JUNO_SHARE_PROFILE =
JUNO_PUSH_TRANSPORT = relay
JUNO_PUSH_DISTRIBUTION = official
JUNO_URL_SLASH = /
JUNO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
JUNO_PUSH_APNS_ENVIRONMENT = production
EOF
