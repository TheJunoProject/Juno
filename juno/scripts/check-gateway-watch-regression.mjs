...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  JUNO_SKIP_ACPX_RUNTIME: "1",
  JUNO_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$JUNO_WATCH_PID_FILE"',
    'mkdir -p "$JUNO_HOME/.juno"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$JUNO_HOME/.juno/juno.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    JUNO_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    JUNO_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "juno-gateway-watch-"));
