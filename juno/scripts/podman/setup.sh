#!/usr/bin/env bash
# One-time host setup for rootless Juno in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.juno by default, and uses the repo-local
# launch script at ./scripts/run-juno-podman.sh.
#
...
#   Or set JUNO_PODMAN_QUADLET=1 (or 0) to choose without a flag.
#
# After this, start the gateway manually:
#   ./scripts/run-juno-podman.sh launch
#   ./scripts/run-juno-podman.sh launch setup
# Or, if you used --quadlet:
#   systemctl --user start juno.service
set -euo pipefail

REPO_PATH="${JUNO_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-juno-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/juno.container.in"
JUNO_USER="$(id -un)"
JUNO_HOME="${HOME:-}"
JUNO_CONFIG_DIR="${JUNO_CONFIG_DIR:-}"
JUNO_WORKSPACE_DIR="${JUNO_WORKSPACE_DIR:-}"
JUNO_IMAGE="${JUNO_PODMAN_IMAGE:-${JUNO_IMAGE:-juno:local}}"
JUNO_CONTAINER_NAME="${JUNO_PODMAN_CONTAINER:-juno}"
PLATFORM_NAME="$(uname -s 2>/dev/null || echo unknown)"
HOST_GATEWAY_PORT="${JUNO_PODMAN_GATEWAY_HOST_PORT:-${JUNO_GATEWAY_PORT:-18789}}"
...
PY
  ...
  echo "Missing dependency: need openssl or python3 (or od) to generate JUNO_GATEWAY_TOKEN." >&2
  exit 1
}
...
if [[ -n "${JUNO_PODMAN_QUADLET:-}" ]]; then
  case "${JUNO_PODMAN_QUADLET,,}" in
...
if [[ "$JUNO_IMAGE" == "juno:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
  echo "Dockerfile not found at $REPO_PATH. Set JUNO_REPO_PATH to the repo root." >&2
  exit 1
fi
...
if [[ -z "$JUNO_HOME" ]]; then
  JUNO_HOME="$(resolve_user_home "$JUNO_USER")"
fi
if [[ -z "$JUNO_HOME" ]]; then
  echo "Unable to resolve HOME for user $JUNO_USER." >&2
  exit 1
fi
if [[ -z "$JUNO_CONFIG_DIR" ]]; then
  JUNO_CONFIG_DIR="$JUNO_HOME/.juno"
fi
if [[ -z "$JUNO_WORKSPACE_DIR" ]]; then
  JUNO_WORKSPACE_DIR="$JUNO_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$JUNO_HOME"
validate_mount_source_path "config directory" "$JUNO_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$JUNO_WORKSPACE_DIR"
validate_container_name "$JUNO_CONTAINER_NAME"
validate_image_name "$JUNO_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$JUNO_CONFIG_DIR" "$JUNO_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$JUNO_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$JUNO_WORKSPACE_DIR"

BUILD_ARGS=()
if [[ -n "${JUNO_DOCKER_APT_PACKAGES:-}" ]]; then
  BUILD_ARGS+=(--build-arg "JUNO_DOCKER_APT_PACKAGES=${JUNO_DOCKER_APT_PACKAGES}")
fi
if [[ -n "${JUNO_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "JUNO_EXTENSIONS=${JUNO_EXTENSIONS}")
fi

if [[ "$JUNO_IMAGE" == "juno:local" ]]; then
  echo "Building image $JUNO_IMAGE ..."
  podman build -t "$JUNO_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$JUNO_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $JUNO_IMAGE"
  else
    echo "Pulling image $JUNO_IMAGE ..."
    podman pull "$JUNO_IMAGE"
  fi
fi

ENV_FILE="$JUNO_CONFIG_DIR/.env"
if [[ ! -f "$ENV_FILE" ]]; then
  TOKEN="$(generate_token_hex_32)"
  (
    umask 077
    write_file_atomically "$ENV_FILE" 600 <<EOF
JUNO_GATEWAY_TOKEN=$TOKEN
EOF
  )
  echo "Generated JUNO_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "JUNO_PODMAN_CONTAINER" "$JUNO_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "JUNO_PODMAN_IMAGE" "$JUNO_IMAGE"

CONFIG_JSON="$JUNO_CONFIG_DIR/juno.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  QUADLET_DIR="$JUNO_HOME/.config/containers/systemd"
  QUADLET_DST="$QUADLET_DIR/juno.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  JUNO_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$JUNO_HOME")"
  JUNO_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$JUNO_CONFIG_DIR")"
  JUNO_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$JUNO_WORKSPACE_DIR")"
  JUNO_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$JUNO_IMAGE")"
  JUNO_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$JUNO_CONTAINER_NAME")"
  sed \
    -e "s|{{JUNO_HOME}}|$JUNO_HOME_ESCAPED|g" \
    -e "s|{{JUNO_CONFIG_DIR}}|$JUNO_CONFIG_ESCAPED|g" \
    -e "s|{{JUNO_WORKSPACE_DIR}}|$JUNO_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$JUNO_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$JUNO_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start juno.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start juno.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-juno-podman.sh launch"
echo "  ./scripts/run-juno-podman.sh launch setup"
echo "  juno --container $JUNO_CONTAINER_NAME dashboard --no-open"
