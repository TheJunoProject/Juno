...
function removeStaleJunoSelfReference(sourcePluginNodeModulesDir, repoRoot) {
  if (!fs.existsSync(sourcePluginNodeModulesDir)) {
    return;
  }

  const selfReferencePath = path.join(sourcePluginNodeModulesDir, "juno");
  try {
...
function ensureJunoExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "juno");
...
function shouldCopyRuntimeFile(sourcePath) {
  ...
  return (
    ...
    relativePath.endsWith("/juno.plugin.json") ||
...
function linkPluginNodeModules(params) {
  ...
  removeStaleJunoSelfReference(params.sourcePluginNodeModulesDir, params.repoRoot);
  ensureSymlink(
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureJunoExtensionAlias({ repoRoot, distExtensionsRoot });
