...
type DriverMode = "token" | "webhook" | "juno";
...
function resolveStateDir(): string {
  const override = process.env.JUNO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.JUNO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".juno");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver juno] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Juno spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|juno> Driver transport mode (default: token)\n" +
    ...
    "  --juno-bin <path>        Juno CLI binary for driver=juno (default: juno)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  JUNO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  JUNO_DISCORD_SMOKE_DRIVER\n" +
    "  JUNO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  JUNO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  JUNO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  JUNO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  JUNO_DISCORD_SMOKE_AGENT\n" +
    "  JUNO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  JUNO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  JUNO_DISCORD_SMOKE_POLL_MS\n" +
    "  JUNO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  JUNO_DISCORD_SMOKE_JUNO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.JUNO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.JUNO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "juno"
        ? "juno"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.JUNO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.JUNO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.JUNO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.JUNO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.JUNO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.JUNO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.JUNO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.JUNO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.JUNO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.JUNO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const junoBin =
    resolveArg("--juno-bin") || process.env.JUNO_DISCORD_SMOKE_JUNO_BIN || "juno";
...
async function junoCliJson<T>(params: { junoBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.junoBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`juno ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithOpenjuno(params: {
  junoBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await junoCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    junoBin: params.junoBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "juno") {
    return await readMessagesWithOpenjuno({
      junoBin: params.args.junoBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `juno-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("juno message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "juno"
            ? await readMessagesWithOpenjuno({
                junoBin: args.junoBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Juno.`,
        diagnostics: {
          bindingCandidates: [
            {
