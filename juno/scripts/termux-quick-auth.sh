...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/juno/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Juno Re-Auth" -c "After getting key, run: ssh $SERVER '~/juno/scripts/mobile-reauth.sh'" --id juno-auth
        ;;
    *)
