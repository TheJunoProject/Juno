...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[juno] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.JUNO_WATCH_MODE = "1";
  childEnv.JUNO_WATCH_SESSION = watchSession;
  ...
  childEnv.JUNO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.JUNO_WATCH_COMMAND = deps.args.join(" ");
  }
