#!/usr/bin/env bash
# Reset Juno like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${JUNO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Juno.app/Contents/MacOS/Juno"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Juno"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Juno"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Juno"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.juno.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/juno-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${JUNO_RESTART_LOG:-/tmp/juno-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${JUNO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.juno/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  JUNO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node juno.mjs daemon install --force --runtime node"
      log "  node juno.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.juno/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.juno.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Juno instances"
kill_all_juno
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Juno"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Juno.app" ]]; then
    APP_BUNDLE="/Applications/Juno.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Juno.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Juno.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Juno.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set JUNO_APP_BUNDLE to your installed Juno.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node juno.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node juno.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".juno", "juno.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: Juno is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.juno.gateway.plist' | head -n 40 || true"
fi
