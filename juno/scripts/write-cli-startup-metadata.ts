...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.juno?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.juno?.channel?.order;
      const labelRaw = parsed.juno?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".juno-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "juno-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "juno-build",
    ...
    JUNO_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    JUNO_PLUGIN_DISCOVERY_CACHE_MS: "0",
    JUNO_PLUGIN_MANIFEST_CACHE_MS: "0",
