...
# Juno Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Juno."
...
GUM_VERSION="${JUNO_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Juno Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://juno.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$JUNO_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.juno.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_juno_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.juno-* "$npm_root"/juno 2>/dev/null || true
}

extract_juno_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_juno_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/juno" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/juno"|"/usr/local/bin/juno")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/juno/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale juno symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing juno binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Juno package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_juno_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Juno npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*juno" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_juno_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_juno_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Juno npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an juno binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES=()
TAGLINES+=("Your terminal just grew junos—type something and let the bot pinch the busywork.")
...
TAGLINES+=("Automation with junos: minimal fuss, maximal pinch.")
...
TAGLINES+=("Junos out, commit in—let's ship something mildly responsible.")
...
HOLIDAY_CHRISTMAS="Christmas: Ho ho ho—Santa's little juno-sistant is here to ship joy, roll back chaos, and stash the keys safely."
...
pick_tagline() {
    ...
    if [[ -n "${JUNO_TAGLINE_INDEX:-}" ]]; then
        if [[ "${JUNO_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((JUNO_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${JUNO_NO_ONBOARD:-0}
NO_PROMPT=${JUNO_NO_PROMPT:-0}
DRY_RUN=${JUNO_DRY_RUN:-0}
INSTALL_METHOD=${JUNO_INSTALL_METHOD:-}
JUNO_VERSION=${JUNO_VERSION:-latest}
USE_BETA=${JUNO_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/juno"
GIT_DIR=${JUNO_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${JUNO_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${JUNO_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${JUNO_VERBOSE:-0}"
VERIFY_INSTALL="${JUNO_VERIFY_INSTALL:-0}"
JUNO_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Juno installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/juno)
...
Environment variables:
  JUNO_INSTALL_METHOD=git|npm
  JUNO_VERSION=latest|next|main|<semver>|<spec>
  JUNO_BETA=0|1
  JUNO_GIT_DIR=...
  JUNO_GIT_UPDATE=0|1
  JUNO_NO_PROMPT=1
  JUNO_VERIFY_INSTALL=1
  JUNO_DRY_RUN=1
  JUNO_NO_ONBOARD=1
  JUNO_VERBOSE=1
  JUNO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Juno checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Juno source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_juno_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"juno"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://juno.ai/install.sh | bash"
}
...
ensure_juno_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/juno" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/juno" ]]; then
        ln -sf "$npm_root/juno/dist/entry.js" "${npm_bin}/juno"
        ui_info "Created juno bin link at ${npm_bin}/juno"
    fi
    return 0
}

# Check for existing Juno installation
check_existing_juno() {
    if [[ -n "$(type -P juno 2>/dev/null || true)" ]]; then
        ui_info "Existing Juno installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make juno show as \"command not found\" in new terminals."
...
warn_juno_not_found() {
    ui_warn "Installed, but juno is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t juno 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named juno; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_juno_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P juno 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/juno" ]]; then
        echo "${npm_bin}/juno"
        return 0
    fi
...
install_juno_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/juno/juno.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Juno from git checkout: ${repo_dir}"
    else
        ui_info "Installing Juno from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Juno" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Juno" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/juno" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/juno"
    ui_success "Juno wrapper installed to \$HOME/.local/bin/juno"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view juno dist-tags.beta 2>/dev/null || true)"
...
install_juno() {
    local package_name="juno"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            JUNO_VERSION="$beta_version"
    ...
    if [[ -z "${JUNO_VERSION}" ]]; then
        JUNO_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${JUNO_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${JUNO_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Juno v${resolved_version}"
    else
        ui_info "Installing Juno (${JUNO_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${JUNO_VERSION}")"

    if ! install_juno_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_juno_paths
        install_juno_npm "${install_spec}"
    fi

    if [[ "${JUNO_VERSION}" == "latest" && "${package_name}" == "juno" ]]; then
        if ! resolve_juno_bin &> /dev/null; then
            ui_warn "npm install juno@latest failed; retrying juno@next"
            cleanup_npm_juno_paths
            install_juno_npm "juno@next"
        fi
    fi

    ensure_juno_bin_link || true

    ui_success "Juno installed"
}
...
run_doctor() {
    ...
    local juno="${JUNO_BIN:-}"
    if [[ -z "$juno" ]]; then
        juno="$(resolve_juno_bin || true)"
    fi
    if [[ -z "$juno" ]]; then
        ui_info "Skipping doctor (juno not on PATH yet)"
        warn_juno_not_found
        return 0
    fi
    run_quiet_step "Running doctor" "$juno" doctor --non-interactive || true
...
maybe_open_dashboard() {
    ...
    if ! "$juno" dashboard --help >/dev/null 2>&1; then
        return 0
    fi
    "$juno" dashboard || true
}
...
resolve_workspace_dir() {
    local profile="${JUNO_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.juno/workspace-${profile}"
    else
        echo "${HOME}/.juno/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${JUNO_CONFIG_PATH:-$HOME/.juno/juno.json}"
    if [[ -f "${config_path}" || -f "$HOME/.junodbot/junodbot.json" ]]; then
        return
    fi
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run juno onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$juno" ]]; then
        ui_info "BOOTSTRAP.md found but juno not on PATH; skipping onboarding"
        warn_juno_not_found
        return
    fi

    "$juno" onboard || {
        ui_error "Onboarding failed; run juno onboard to retry"
        return
    }
}
...
if ! declare -F extract_juno_semver >/dev/null 2>&1; then
...
resolve_juno_version() {
    ...
    local juno="${JUNO_BIN:-}"
    if [[ -z "$juno" ]] && command -v juno &> /dev/null; then
        juno="$(command -v juno)"
    fi
    if [[ -n "$juno" ]]; then
        raw_version_output=$("$juno" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_juno_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/juno/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/juno/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$("$juno" daemon status --json 2>/dev/null || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if ! is_gateway_daemon_loaded "$juno"; then
        return 0
    fi

    ui_info "Refreshing loaded gateway service"
    if run_quiet_step "Refreshing gateway service" "$juno" gateway install --force; then
    ...
    if run_quiet_step "Restarting gateway service" "$juno" gateway restart; then
    ...
    run_quiet_step "Probing gateway service" "$juno" gateway status --deep || true
}
...
verify_installation() {
    ...
    if [[ -z "$juno" ]]; then
        ui_error "Install verify failed: juno not on PATH yet"
        warn_juno_not_found
        return 1
    fi

    run_quiet_step "Checking Juno version" "$juno" --version || return 1

    if is_gateway_daemon_loaded "$juno"; then
        run_quiet_step "Checking gateway service" "$juno" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: juno gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_juno_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Juno checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set JUNO_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    JUNO_BIN="$(resolve_juno_bin || true)"
    ...
    installed_version=$(resolve_juno_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Juno installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Juno installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "The lobster has molted. Harder shell, sharper junos."
        ...
        local completion_messages=(
            ...
            "cracks junos Alright, what are we building?"
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/juno"
        ui_kv "Update command" "juno update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://juno.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running juno doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                JUNO_UPDATE_IN_PROGRESS=1 "$juno" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                JUNO_UPDATE_IN_PROGRESS=1 "$juno" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                JUNO_UPDATE_IN_PROGRESS=1 "$juno" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run juno doctor and juno plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run juno onboard later"
        else
            local config_path="${JUNO_CONFIG_PATH:-$HOME/.juno/juno.json}"
            if [[ -f "${config_path}" || -f "$HOME/.junodbot/junodbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$juno" ]]; then
                    ui_info "Skipping onboarding (juno not on PATH yet)"
                    warn_juno_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run juno onboard to finish setup"
            return 0
        fi
    fi
    ...
    if command -v juno &> /dev/null; then
        ...
        if [[ -n "$juno" ]] && is_gateway_daemon_loaded "$juno"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (juno daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if JUNO_UPDATE_IN_PROGRESS=1 "$juno" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: juno daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${JUNO_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
