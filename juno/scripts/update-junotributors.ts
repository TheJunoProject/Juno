...
import type { ApiContributor, Entry, MapConfig, User } from "./update-junotributors.types.js";

const REPO = "juno/juno";
const PER_LINE = 10;
const AVATAR_PROBE_SIZE = 40;
const AVATAR_SIZE = 48;
const JUNOTRIBUTORS_START = "<!-- junotributors:start -->";
const JUNOTRIBUTORS_END = "<!-- junotributors:end -->";
const JUNOTRIBUTORS_HIDDEN_START = "<!-- junotributors:hidden:start";
const JUNOTRIBUTORS_HIDDEN_END = "junotributors:hidden:end -->";

const mapPath = resolve("scripts/junotributors-map.json");
...
const block = `${JUNOTRIBUTORS_START}\n${markdownLines.join("\n")}\n${JUNOTRIBUTORS_END}`;
...
const readmeWithoutMeta = hiddenRange
  ...
  : currentReadme;
const range = findJunotributorsRange(readmeWithoutMeta);

if (!range) {
  throw new Error("README.md missing junotributors block");
}
...
console.log(
  `Updated README junotributors: ${visibleEntries.length} visible (${entries.length - visibleEntries.length} default-avatar entries hidden)`,
);
...
async function probeDefaultGitHubAvatar(login: string): Promise<boolean> {
  try {
    const response = await fetch(`https://github.com/${login}.png?size=${AVATAR_PROBE_SIZE}`, {
      headers: { "user-agent": "juno-junotributors" },
...
function parseReadmeEntries(
  ...
  const range = findJunotributorsRange(content);
  if (!range) {
    return [];
  }
...
function buildHiddenReadmeBlock(entries: Entry[], visibleEntries: Entry[]): string {
  ...
  if (hiddenLogins.length === 0) {
    return `${JUNOTRIBUTORS_HIDDEN_START}\n${notice}\n${JUNOTRIBUTORS_HIDDEN_END}\n`;
  }
  return `${JUNOTRIBUTORS_HIDDEN_START}\n${notice}\n${hiddenLogins.join("\n")}\n${JUNOTRIBUTORS_HIDDEN_END}\n`;
}

function findJunotributorsRange(content: string): { start: number; end: number } | null {
  const markerStart = content.indexOf(JUNOTRIBUTORS_START);
  const markerEnd = content.indexOf(JUNOTRIBUTORS_END, markerStart);
  if (markerStart !== -1 && markerEnd !== -1) {
    return {
      start: markerStart,
      end: markerEnd + JUNOTRIBUTORS_END.length,
    };
  }
...
function findHiddenReadmeRange(content: string): { start: number; end: number } | null {
  const markerStart = content.indexOf(JUNOTRIBUTORS_HIDDEN_START);
  const markerEnd = content.indexOf(JUNOTRIBUTORS_HIDDEN_END, markerStart);
  if (markerStart === -1 || markerEnd === -1) {
    return null;
  }
  return {
    start: markerStart,
    end: markerEnd + JUNOTRIBUTORS_HIDDEN_END.length,
  };
}
