#!/bin/bash
# Setup Juno Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'juno-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Juno message: Send warning via Juno itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/juno-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/juno-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/juno-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now juno-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/JunodAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/JunodAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Junod*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (JunodAuth) shows status and opens auth URL if needed."
echo "The full widget (JunodAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status juno-auth-monitor.timer"
echo ""
echo "Quick commands:"
