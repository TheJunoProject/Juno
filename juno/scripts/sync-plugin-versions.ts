...
const JUNO_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncJunoDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.juno;
  if (!current || current === "workspace:*" || !JUNO_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.juno?.compat;
  const current = compat?.pluginApi;
  if (!current || !JUNO_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildJunoVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.juno?.build;
  const current = build?.junoVersion;
  if (!current) {
    return false;
  }
  ...
  build.junoVersion = targetVersion;
  return true;
}
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Juno release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncJunoDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncJunoDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildJunoVersionChanged = syncBuildJunoVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildJunoVersionChanged;
    if (!packageChanged) {
