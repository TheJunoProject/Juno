# Juno Installer for Windows (PowerShell)
# Usage: iwr -useb https://juno.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://juno.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\juno",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Install-JunoNpm {
    ...
    try {
        ...
        Write-Host "Juno installed" -Level success
        return $true
    } catch {
...
function Install-JunoGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Juno from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/juno/juno.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\juno\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\juno.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Juno from git to $GitDir" -Level info
        } else {
            Install-JunoGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Juno via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-JunoNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'juno onboard' to complete setup" -Level info
    }
