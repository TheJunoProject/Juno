...
IMAGE_NAME="juno-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/juno-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/juno-bin/systemctl

  cat > /tmp/juno-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  npm_log="/tmp/juno-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/juno"
	  npm_root="/tmp/npm-prefix/lib/node_modules/juno"
  ...
  run_flow() {
    ...
    local install_log="/tmp/juno-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/juno-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/juno-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/juno-gateway.service"
    if [ ! -f "$unit_path" ]; then
