...
IMAGE_NAME="${JUNO_IMAGE:-juno-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="juno-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t juno-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "JUNO_SKIP_GMAIL_WATCHER=1" \
  -e "JUNO_SKIP_CRON=1" \
  -e "JUNO_SKIP_CANVAS_HOST=1" \
  -e "JUNO_STATE_DIR=/tmp/juno-state" \
  -e "JUNO_CONFIG_PATH=/tmp/juno-state/juno.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "JUNO_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
