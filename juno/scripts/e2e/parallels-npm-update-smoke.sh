...
UPDATE_TARGET=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/juno-parallels-npm-update.XXXXXX)"
...
PROGRESS_STALE_S=60
TIMEOUT_UPDATE_S="${JUNO_PARALLELS_NPM_UPDATE_TIMEOUT_S:-900}"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: juno@latest
  --update-target <target>    Target passed to guest 'juno update --tag'.
                             Default: host-served tgz packed from current checkout.
                             Examples: latest, beta, 2026.4.10, http://host/juno.tgz
...
resolve_latest_version() {
  npm view juno version --userconfig "$(mktemp)"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/juno-main-$CURRENT_HEAD_SHORT.tgz"
...
write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/juno-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/juno warnings do not
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$JunoPath,
    [int]$Attempts = 20,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      Invoke-Logged 'juno gateway status' { & $JunoPath gateway status --deep --require-rpc }
      return
    } catch {
...
function Stop-JunoGatewayProcesses {
  Write-ProgressLog 'update.stop-old-gateway'
  $patterns = @(
    'juno-gateway',
    'juno.*gateway --port 18789',
    'juno.*gateway run',
    'juno\.mjs gateway',
...
function Complete-WorkspaceSetup {
  $workspace = $env:JUNO_WORKSPACE_DIR
  if (-not $workspace) {
    $workspace = Join-Path $env:USERPROFILE '.juno\workspace'
  }
  $stateDir = Join-Path $workspace '.juno'
...
function Restart-GatewayWithRecovery {
  ...
  if ($null -ne $restartCompleted) {
    ...
    if ($restartResult.ExitCode -ne 0) {
      ...
      "juno gateway restart failed with exit code $($restartResult.ExitCode)" | Tee-Object -FilePath $LogPath -Append | Out-Null
    }
  } else {
    $restartFailed = $true
    Stop-Job $restartJob -ErrorAction SilentlyContinue
    Write-ProgressLog 'update.restart-gateway.timeout'
    'juno gateway restart timed out after 20s; continuing to RPC readiness checks' | Tee-Object -FilePath $LogPath -Append | Out-Null
  }
  Remove-Job $restartJob -Force -ErrorAction SilentlyContinue

  Write-ProgressLog 'update.gateway-status'
  try {
    Wait-GatewayRpcReady -JunoPath $JunoPath
    return
  } catch {
    ...
    Invoke-Logged 'juno gateway start' { & $JunoPath gateway start }
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Juno\deps\portable-git\cmd;$env:LOCALAPPDATA\Juno\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Juno\deps\portable-git\usr\bin;$env:PATH"
  ...
  $juno = Join-Path $env:APPDATA 'npm\juno.cmd'
  Stop-JunoGatewayProcesses
  Write-ProgressLog 'update.juno-update'
  Invoke-Logged 'juno update' { & $juno update --tag $UpdateTarget --yes --json }
  Write-ProgressLog 'update.verify-version'
  $version = Invoke-CaptureLogged 'juno --version' { & $juno --version }
  ...
  Invoke-Logged 'juno update status' { & $juno update status --json }
  Write-ProgressLog 'update.set-model'
  Invoke-Logged 'juno models set' { & $juno models set $ModelId }
  ...
  Restart-GatewayWithRecovery -JunoPath $juno
  Complete-WorkspaceSetup
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'juno agent' { & $juno agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/juno-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_macos_update_after_transport_loss() {
  ...
  local script_path="/tmp/juno-npm-update-macos-recover.sh"
...
busy="\$(/bin/ps -axo command | /usr/bin/egrep 'juno update|npm install|pnpm install|pnpm run build' | /usr/bin/egrep -v 'egrep|juno-npm-update-macos-recover' || true)"
if [ -n "\$busy" ]; then
  printf 'update still has active npm/pnpm/juno processes\n%s\n' "\$busy" >&2
  exit 1
fi
version="\$(/opt/homebrew/bin/juno --version)"
printf '%s\n' "\$version"
...
/opt/homebrew/bin/juno gateway status --deep --require-rpc >/dev/null 2>&1 || /opt/homebrew/bin/juno gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6; do
  if /opt/homebrew/bin/juno gateway status --deep --require-rpc; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  /opt/homebrew/bin/juno gateway start || true
  for _ in 1 2 3 4 5 6; do
    if /opt/homebrew/bin/juno gateway status --deep --require-rpc; then
      gateway_ready=1
      break
...
workspace="\${JUNO_WORKSPACE_DIR:-\$HOME/.juno/workspace}"
mkdir -p "\$workspace/.juno"
...
- Purpose: Parallels npm update smoke test assistant.
IDENTITY_EOF
cat > "\$workspace/.juno/workspace-state.json" <<'STATE_EOF'
{
  "version": 1,
  "setupCompletedAt": "2026-01-01T00:00:00.000Z"
}
...
/opt/homebrew/bin/juno models set "$MODEL_ID"
/opt/homebrew/bin/juno agent --agent main --session-id "parallels-npm-update-macos-transport-recovery-$expected_needle" --message "Reply with exact ASCII text OK only." --json
EOF
...
\$ErrorActionPreference = 'Stop'
\$juno = Join-Path \$env:APPDATA 'npm\\juno.cmd'
if (-not (Test-Path \$juno)) {
  throw "juno shim missing: \$juno"
}
\$busy = Get-CimInstance Win32_Process |
  Where-Object {
    \$_.CommandLine -and
    (\$_.CommandLine -match 'juno update|npm install|pnpm install|pnpm run build')
  }
if (\$busy) {
  throw 'update still has active npm/pnpm/juno processes'
}
...
\$gatewayReady = \$false
for (\$i = 0; \$i -lt 6; \$i++) {
  & \$juno gateway status --deep --require-rpc
...
'@ | Set-Content -Path (Join-Path \$stateDir 'workspace-state.json') -Encoding UTF8
Remove-Item (Join-Path \$workspace 'BOOTSTRAP.md') -Force -ErrorAction SilentlyContinue
& \$juno agent --agent main --session-id 'parallels-npm-update-windows-transport-recovery-$expected_needle' --message 'Reply with exact ASCII text OK only.' --json
EOF
  )"
  local rc=$?
  set -e
...
text = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8", errors="replace")
matches = re.findall(r"Juno [^\r\n]+", text)
matches = [match for match in matches if re.search(r"Juno \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="juno-update-$RANDOM-$RANDOM.ps1"
  log_name="juno-update-$RANDOM-$RANDOM.log"
  done_name="juno-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/juno-update-log-state.XXXXXX")"
  : >"$log_state_path"
  provider_key_b64="$(
...
run_macos_update() {
  ...
  cat <<EOF | prlctl exec "$MACOS_VM" /usr/bin/tee /tmp/juno-main-update.sh >/dev/null
set -euo pipefail
export PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin
...
cd "\$HOME"
stop_juno_gateway_processes() {
  /opt/homebrew/bin/juno gateway stop >/dev/null 2>&1 || true
  /usr/bin/pkill -9 -f juno-gateway || true
  /usr/bin/pkill -9 -f 'juno gateway run' || true
  /usr/bin/pkill -9 -f 'juno.mjs gateway' || true
...
stop_juno_gateway_processes
/opt/homebrew/bin/juno update --tag "$update_target" --yes --json
...
/opt/homebrew/bin/juno update status --json
/opt/homebrew/bin/juno models set "$MODEL_ID"
...
/opt/homebrew/bin/juno gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/juno gateway status --deep --require-rpc >/dev/null 2>&1; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  stop_juno_gateway_processes
  /opt/homebrew/bin/juno gateway run --bind loopback --port 18789 --force >/tmp/juno-parallels-npm-update-macos-gateway.log 2>&1 </dev/null &
  for _ in 1 2 3 4 5 6 7 8; do
    if /opt/homebrew/bin/juno gateway status --deep --require-rpc >/dev/null 2>&1; then
      gateway_ready=1
      break
...
if [ "\$gateway_ready" != "1" ]; then
  tail -n 120 /tmp/juno-parallels-npm-update-macos-gateway.log 2>/dev/null || true
fi
/opt/homebrew/bin/juno gateway status --deep --require-rpc
workspace="\${JUNO_WORKSPACE_DIR:-\$HOME/.juno/workspace}"
mkdir -p "\$workspace/.juno"
cat > "\$workspace/IDENTITY.md" <<'IDENTITY_EOF'
# Identity
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
/opt/homebrew/bin/juno agent --agent main --session-id parallels-npm-update-macos-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  macos_desktop_user_exec /bin/bash /tmp/juno-main-update.sh
}
...
run_linux_update() {
  local update_target="$1"
  local expected_needle="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/juno-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
stop_juno_gateway_processes() {
  juno gateway stop >/dev/null 2>&1 || true
  pkill -9 -f juno-gateway || true
  pkill -9 -f 'juno gateway run' || true
  pkill -9 -f 'juno.mjs gateway' || true
...
stop_juno_gateway_processes
juno update --tag "$update_target" --yes --json
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
juno agent --local --agent main --session-id parallels-npm-update-linux-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/juno-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="juno@$LATEST_VERSION"
fi
resolve_current_head
...
say "Run same-guest juno update to $UPDATE_TARGET_EFFECTIVE"
