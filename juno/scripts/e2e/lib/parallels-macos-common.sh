...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/juno-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/juno-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${JUNO_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${JUNO_SECRET_ENV_NAME:-}" ] && [ -n "${JUNO_SECRET_FILE:-}" ] && [ -f "$JUNO_SECRET_FILE" ]; then
  secret_value="$(cat "$JUNO_SECRET_FILE")"
  rm -f "$JUNO_SECRET_FILE"
  export "${JUNO_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "JUNO_SECRET_ENV_NAME=$api_key_env" \
      "JUNO_SECRET_FILE=$secret_path" \
      "JUNO_WRAPPER_FILE=$wrapper_path" \
