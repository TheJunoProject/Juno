...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/juno-bin:$PATH"
  mkdir -p /tmp/juno-bin
  cat > /tmp/juno-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$JUNO_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_juno_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/juno-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/juno-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$JUNO_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/juno-e2e-$1.XXXXXX"
  }

  set_isolated_juno_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export JUNO_HOME="$home_dir"
    export JUNO_STATE_DIR="$home_dir/.juno"
    export JUNO_CONFIG_PATH="$JUNO_STATE_DIR/juno.json"
    mkdir -p "$JUNO_STATE_DIR"
  }
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/juno-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_juno_env "$home_dir"
    run_case_logged local-basic node "$JUNO_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$JUNO_STATE_DIR/workspace"
    config_path="$JUNO_CONFIG_PATH"
    sessions_dir="$JUNO_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_juno_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$JUNO_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$JUNO_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$JUNO_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$JUNO_ENTRY\" configure --section skills" send_skills_flow
