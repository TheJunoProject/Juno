...
MODEL="${JUNO_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${JUNO_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${JUNO_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${JUNO_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${JUNO_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${JUNO_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="juno-openwebui-e2e-$$"
GW_NAME="juno-openwebui-gateway-$$"
OW_NAME="juno-openwebui-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "JUNO_GATEWAY_TOKEN=$TOKEN" \
  -e "JUNO_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/juno-openwebui-config.XXXXXX.json)"
    JUNO_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.JUNO_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.JUNO_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.JUNO_OPENWEBUI_MODEL },
];
...
EOF
    cat > "$workspace/.juno/workspace-state.json" <<'"'"'EOF'"'"'
{
  "version": 1,
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Juno E2E" \
  -e WEBUI_SECRET_KEY="juno-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Juno E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="juno/default" \
...
echo "Running Open WebUI -> Juno smoke..."
if ! docker exec \
