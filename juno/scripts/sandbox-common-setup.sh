...
BASE_IMAGE="${BASE_IMAGE:-juno-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-juno-sandbox-common:bookworm-slim}"
...
JUNO_DOCKER_BUILD_USE_BUILDX="${JUNO_DOCKER_BUILD_USE_BUILDX:-0}"
JUNO_DOCKER_BUILD_CACHE_FROM="${JUNO_DOCKER_BUILD_CACHE_FROM:-}"
JUNO_DOCKER_BUILD_CACHE_TO="${JUNO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${JUNO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${JUNO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${JUNO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${JUNO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${JUNO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=juno.sandbox=1)
NOTE
