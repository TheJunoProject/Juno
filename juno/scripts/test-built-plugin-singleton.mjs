...
const { clearPluginCommands, getPluginCommandSpecs, loadJunoPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadJunoPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "juno-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "juno.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'juno/plugin-sdk';",
...
const registry = loadJunoPlugins({
  ...
  env: {
    ...process.env,
    JUNO_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
