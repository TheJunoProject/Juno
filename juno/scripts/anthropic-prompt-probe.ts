...
// Usage:
// JUNO_PROMPT_TRANSPORT=direct|gateway
// JUNO_PROMPT_MODE=extra|override
// JUNO_PROMPT_TEXT='...'
// JUNO_PROMPT_CAPTURE=1
...
import { resolveJunoAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.JUNO_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
...
const NODE_BIN = process.env.JUNO_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.JUNO_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.JUNO_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.JUNO_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.JUNO_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.JUNO_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing JUNO_PROMPT_TEXT or JUNO_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set JUNO_LIVE_SETUP_TOKEN_VALUE or JUNO_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["juno.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        JUNO_CONFIG_PATH: params.configPath,
        JUNO_STATE_DIR: params.stateDir,
        JUNO_AGENT_DIR: params.agentDir,
        JUNO_GATEWAY_TOKEN: params.gatewayToken,
        JUNO_SKIP_CHANNELS: "1",
        JUNO_SKIP_GMAIL_WATCHER: "1",
        JUNO_SKIP_CANVAS_HOST: "1",
        JUNO_SKIP_BROWSER_CONTROL_SERVER: "1",
        JUNO_DISABLE_BONJOUR: "1",
        JUNO_SKIP_CRON: "1",
        JUNO_TEST_MINIMAL_GATEWAY: "1",
        JUNO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "juno-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "juno.json");
