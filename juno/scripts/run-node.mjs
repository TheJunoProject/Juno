...
const extensionRestartMetadataFiles = new Set(["juno.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.JUNO_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.JUNO_BUILD_PRIVATE_QA === "1" &&
...
const BUILD_REASON_LABELS = {
  force_build: "forced by JUNO_FORCE_BUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "JUNO_RUN_NODE_OUTPUT_LOG";
...
const logRunner = (message, deps) => {
  if (deps.env.JUNO_RUNNER_LOG === "0") {
    return;
  }
  const line = `[juno] ${message}\n`;
...
const runJuno = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["juno.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[juno] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.JUNO_WATCH_MODE === "1";
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.JUNO_BUILD_PRIVATE_QA = "1";
    deps.env.JUNO_ENABLE_PRIVATE_QA_CLI = "1";
  }
