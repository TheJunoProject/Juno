...
const EXPECTED_REPOSITORY_URL = "https://github.com/juno/juno";
...
const skipPackValidationEnv = "JUNO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "juno") {
    errors.push(`package.json name must be "juno"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.juno !== "juno.mjs") {
    errors.push(
      `package.json bin.juno must be "juno.mjs"; found "${pkg.bin?.juno ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`juno-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `juno-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
