 ...
 * `juno update`. Use it to verify the completion installation flow
...
\`juno update\`, \`juno doctor\`, and \`juno onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Juno Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`juno completion --install\` later to enable.`));
    return;
  }
