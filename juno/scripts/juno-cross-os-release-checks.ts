...
const PUBLISHED_INSTALLER_BASE_URL = "https://juno.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.JUNO_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.JUNO_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.JUNO_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `juno@${previousVersion}` : "juno@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.JUNO_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.JUNO_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        JUNO_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `juno-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".juno");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    JUNO_HOME: lane.homeDir,
    JUNO_STATE_DIR: lane.stateDir,
    JUNO_CONFIG_PATH: join(lane.stateDir, "juno.json"),
    JUNO_DISABLE_BONJOUR: "1",
    JUNO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.JUNO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "juno@latest") {
    return "";
  }
  if (trimmed.startsWith("juno@")) {
    return trimmed.slice("juno@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "juno@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view juno@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'juno.cmd'),
      (Join-Path $npmPrefix 'juno.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command juno -ErrorAction Stop
...
Write-Output "__JUNO_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__JUNO_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed juno path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v juno >/dev/null 2>&1",
    'printf "__JUNO_PATH__=%s\\n" "$(command -v juno)"',
    "juno --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__JUNO_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed juno path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `juno update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.juno/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.JUNO_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.JUNO_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.JUNO_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.JUNO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>Juno Control</title>") &&
          html.includes("<juno-app></juno-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>Juno Control</title>")} app=${html.includes("<juno-app></juno-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
function installedPackageRoot(prefixDir) {
  return process.platform === "win32"
    ? join(prefixDir, "node_modules", "juno")
    : join(prefixDir, "lib", "node_modules", "juno");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "juno.mjs");
}
