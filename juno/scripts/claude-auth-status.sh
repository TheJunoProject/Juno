...
# Checks both Claude Code and Juno auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
JUNO_AUTH="$HOME/.juno/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    juno models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    juno_status=$(check_juno_auth 2>/dev/null || true)

    claude_expires=0
    juno_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        juno_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        juno_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$JUNO_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        '{
            ...
            juno: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$juno_status" == EXPIRED* ]] || [[ "$juno_status" == MISSING* ]]; then
        echo "JUNO_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$juno_status" == EXPIRING* ]]; then
        echo "JUNO_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Juno Auth (~/.juno/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$JUNO_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$JUNO_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'juno doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active juno >/dev/null 2>&1; then
    echo -e "Juno service: ${GREEN}running${NC}"
else
    echo -e "Juno service: ${RED}NOT running${NC}"
fi
