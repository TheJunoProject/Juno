...
if [[ "${mode}" != "--dry-run" && "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/plugin-junohub-publish.sh [--dry-run|--publish] <package-dir>" >&2
  exit 2
fi
...
if ! command -v junohub >/dev/null 2>&1; then
  echo "junohub CLI is required on PATH" >&2
  exit 1
fi
...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-juno/juno}}"
source_commit="${SOURCE_COMMIT:-$(git rev-parse HEAD)}"
source_ref="${SOURCE_REF:-$(git symbolic-ref -q HEAD || true)}"
junohub_workdir="${JUNODHUB_WORKDIR:-${JUNOHUB_WORKDIR:-$(pwd)}}"
...
echo "Resolved JunoHub workdir: ${junohub_workdir}"
echo "Publish auth: GitHub Actions OIDC via JunoHub short-lived token"
...
if [[ "${mode}" == "--dry-run" ]]; then
  JUNOHUB_WORKDIR="${junohub_workdir}" "${publish_cmd[@]}" --dry-run
  exit 0
fi

JUNOHUB_WORKDIR="${junohub_workdir}" "${publish_cmd[@]}"
