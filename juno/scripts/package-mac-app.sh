...
# Build and bundle Juno into a minimal .app we can open.
# Outputs to dist/Juno.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Juno.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Juno"
BUNDLE_ID="${BUNDLE_ID:-ai.juno.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/juno/juno/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Juno/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :JunoBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :JunoGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Juno"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Juno"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Juno"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Juno" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Juno/Resources/Juno.icns" "$APP_ROOT/Contents/Resources/Juno.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Juno/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying JunoKit resources"
JUNOKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/JunoKit_JunoKit.bundle"
if [ -d "$JUNOKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/JunoKit_JunoKit.bundle"
  cp -R "$JUNOKIT_BUNDLE" "$APP_ROOT/Contents/Resources/JunoKit_JunoKit.bundle"
else
  echo "WARN: JunoKit resource bundle not found at $JUNOKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Juno"
killall -q Juno 2>/dev/null || true
