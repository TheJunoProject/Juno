...
@MainActor
public struct JunoChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: JunoChatViewModel
    ...
    private var visibleMessages: [JunoChatMessage] {
        let base: [JunoChatMessage]
    ...
    private func mergeToolResults(in messages: [JunoChatMessage]) -> [JunoChatMessage] {
    ...
    private func isToolResultMessage(_ message: JunoChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: JunoChatMessage) -> Bool {
    ...
    private func primaryText(in message: JunoChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: JunoChatMessage) -> Bool {
    ...
    private func toolCalls(in message: JunoChatMessage) -> [JunoChatMessageContent] {
    ...
    private func inlineToolResults(in message: JunoChatMessage) -> [JunoChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: JunoChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: JunoChatMessage) -> String {
