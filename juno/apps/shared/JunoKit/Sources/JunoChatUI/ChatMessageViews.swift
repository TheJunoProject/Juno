...
struct ChatMessageBubble: View {
    let message: JunoChatMessage
    let style: JunoChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? JunoChatTheme.userText : JunoChatTheme.assistantText
    ...
    private var inlineAttachments: [JunoChatMessageContent] {
    ...
    private var toolCalls: [JunoChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [JunoChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? JunoChatTheme.userBubble
        }
        if self.style == .onboarding {
            return JunoChatTheme.onboardingAssistantBubble
        }
        return JunoChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return JunoChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: JunoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? JunoChatTheme.userText : JunoChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: JunoChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(JunoChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [JunoChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: JunoChatTheme.assistantText)
            }
        }
    }
}
