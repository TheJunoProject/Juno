...
enum JunoPlatformImageFactory {
    static func image(_ image: JunoPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
