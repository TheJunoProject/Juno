...
private let chatUILogger = Logger(subsystem: "ai.juno", category: "JunoChatUI")

@MainActor
@Observable
public final class JunoChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [JunoChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [JunoChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [JunoPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [JunoChatPendingToolCall] = []
    public private(set) var sessions: [JunoChatSessionEntry] = []
    private let transport: any JunoChatTransport
    private var sessionDefaults: JunoChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: JunoChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any JunoChatTransport,
    ...
    public var sessionChoices: [JunoChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [JunoChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: JunoPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [JunoChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: JunoChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: JunoChatMessage) -> JunoChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> JunoChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return JunoChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: JunoChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: JunoChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: JunoChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [JunoChatMessage],
        incoming: [JunoChatMessage]) -> [JunoChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [JunoChatMessage]) -> [JunoChatMessage] {
        var result: [JunoChatMessage] = []
    ...
    private static func dedupeKey(for message: JunoChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [JunoChatMessageContent] = [
            JunoChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> JunoChatAttachmentPayload in
            JunoChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> JunoChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = JunoChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: JunoChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: JunoChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: JunoAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = JunoChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> JunoPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
