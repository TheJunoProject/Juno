...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> JunoChatHistoryPayload
{
    JunoChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> JunoChatSessionEntry {
    JunoChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> JunoChatSessionEntry
{
    JunoChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> JunoChatModelChoice {
    JunoChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [JunoChatHistoryPayload],
    sessionsResponses: [JunoChatSessionsListResponse] = [],
    modelResponses: [[JunoChatModelChoice]] = [],
    ...
    -> (TestChatTransport, JunoChatViewModel)
{
...
private func sendUserMessage(_ vm: JunoChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, JunoChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [JunoChatHistoryPayload]
    private let sessionsResponses: [JunoChatSessionsListResponse]
    private let modelResponses: [[JunoChatModelChoice]]
    ...
    private let stream: AsyncStream<JunoChatTransportEvent>
    private let continuation: AsyncStream<JunoChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<JunoChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<JunoChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> JunoChatHistoryPayload {
        ...
        return self.historyResponses.last ?? JunoChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [JunoChatAttachmentPayload]) async throws -> JunoChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return JunoChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> JunoChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? JunoChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [JunoChatModelChoice] {
    ...
    func emit(_ evt: JunoChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = JunoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = JunoChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: JunoChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = JunoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JunoChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(JunoChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == JunoChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = JunoChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: JunoChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = JunoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = JunoChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = JunoChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { JunoChatViewModel(sessionKey: "main", transport: transport) }
