...
- Play build: `apps/android/build/release-bundles/juno-<version>-play-release.aab`
- Third-party build: `apps/android/build/release-bundles/juno-<version>-third-party-release.aab`
...
```bash
pnpm juno gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__juno__/canvas/` (see `docs/platforms/android.md`).
...
Current Juno Android implication:
...
2) Android app is connected to that gateway and `juno nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device (do not run gateway with `JUNO_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__juno__/`).
...
- `JUNO_ANDROID_GATEWAY_URL=ws://...` (default: from your local Juno config)
- `JUNO_ANDROID_GATEWAY_TOKEN=...`
- `JUNO_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - approve pending device pairing (`juno devices approve --latest`) and rerun.
