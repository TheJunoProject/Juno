...
import ai.juno.app.gateway.GatewaySession
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(command: String, paramsJson: String?): GatewaySession.InvokeResult {
    ...
    return when (command) {
      ...
      JunoCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      JunoCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      JunoCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      JunoCanvasA2UICommand.Push.rawValue, JunoCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      JunoCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      JunoCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      JunoCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      JunoLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      JunoDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      JunoDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      JunoDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      JunoDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)

      // Notifications command
      JunoNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      JunoNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      JunoSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Photos command
      ai.juno.app.protocol.JunoPhotosCommand.Latest.rawValue -> photosHandler.handlePhotosLatest(
        paramsJson,
      )

      // Contacts command
      JunoContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      JunoContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      JunoCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      JunoCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      JunoMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      JunoMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      JunoSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      JunoSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      JunoCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
