...
private const val NOTIFICATION_CHANNEL_BASE_ID = "juno.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Juno Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Juno Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Juno Active")
      }
