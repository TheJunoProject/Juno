...
internal fun openJunoAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Juno Android gateway connection failure.
    ...
    - tell me whether `juno devices list` should show a pending pairing request
    - if more signal is needed, ask for `juno qr --json`, `juno devices list`, and `juno nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${openJunoAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Juno gateway diagnostics", report))
