...
val androidStoreFile = providers.gradleProperty("JUNO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("JUNO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("JUNO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("JUNO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set JUNO_ANDROID_STORE_FILE, " +
            "JUNO_ANDROID_STORE_PASSWORD, JUNO_ANDROID_KEY_ALIAS, and " +
            "JUNO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/JunoKit/Sources/JunoKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.juno.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "JUNO_ENABLE_SMS", "false")
            buildConfigField("boolean", "JUNO_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "JUNO_ENABLE_SMS", "true")
            buildConfigField("boolean", "JUNO_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "juno-$versionName-$buildType.apk"
                    } else {
                        "juno-$versionName-$flavorName-$buildType.apk"
                    }
