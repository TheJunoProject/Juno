...
@MainActor
enum JunoAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class JunoAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.juno.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.juno.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.juno.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? JunoAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "juno.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "juno.watch.promptId"
    static let sessionKeyKey = "juno.watch.sessionKey"
    static let actionPrimaryIDKey = "juno.watch.action.primary.id"
    static let actionPrimaryLabelKey = "juno.watch.action.primary.label"
    static let actionSecondaryIDKey = "juno.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "juno.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "juno.watch.action.primary"
    static let actionSecondaryIdentifier = "juno.watch.action.secondary"
    static let actionIdentifierPrefix = "juno.watch.action."
    static let actionIDKeyPrefix = "juno.watch.action.id."
    static let actionLabelKeyPrefix = "juno.watch.action.label."
    static let categoryPrefix = "juno.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: JunoWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> JunoWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return JunoWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Juno" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [JunoWatchAction]) -> [UNNotificationAction] {
...
struct JunoApp: App {
    ...
    @UIApplicationDelegateAdaptor(JunoAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        JunoAppModelRegistry.appModel = appModel
...
extension JunoApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Juno: installing uncaught exception handler")
