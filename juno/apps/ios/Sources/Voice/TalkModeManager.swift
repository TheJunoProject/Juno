...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__JUNO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<JunoTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.juno", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = JunoTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> JunoTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return JunoTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> JunoTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> JunoTalkPTTStopPayload {
    ...
    func cancelPushToTalk() async -> JunoTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = JunoTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: JunoTalkPTTStopPayload) {
