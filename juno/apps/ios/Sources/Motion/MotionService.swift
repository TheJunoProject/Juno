...
final class MotionService: MotionServicing {
    func activities(params: JunoMotionActivityParams) async throws -> JunoMotionActivityPayload {
        ...
        let mapped: [JunoMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return JunoMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: JunoPedometerParams) async throws -> JunoPedometerPayload {
        ...
        let payload: JunoPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = JunoPedometerPayload(
