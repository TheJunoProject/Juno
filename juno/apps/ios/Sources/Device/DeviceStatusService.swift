...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> JunoDeviceStatusPayload {
        ...
        return JunoDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> JunoBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: JunoBatteryState = switch device.batteryState {
        ...
        return JunoBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> JunoThermalStatusPayload {
        let state: JunoThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return JunoThermalStatusPayload(state: state)
    }

    private func storageStatus() -> JunoStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return JunoStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
