...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "juno.exec-approval"
    static let reviewActionIdentifier = "juno.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.openJunoPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.openJunoPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func openJunoPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
