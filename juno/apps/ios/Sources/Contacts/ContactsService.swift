...
final class ContactsService: ContactsServicing {
    ...
    func search(params: JunoContactsSearchParams) async throws -> JunoContactsSearchPayload {
        ...
        return JunoContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: JunoContactsAddParams) async throws -> JunoContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return JunoContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return JunoContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> JunoContactPayload {
        JunoContactPayload(
