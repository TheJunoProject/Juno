...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [JunoCapability.canvas.rawValue, JunoCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(JunoCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(JunoCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = JunoLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(JunoCapability.location.rawValue) }

        caps.append(JunoCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(JunoCapability.watch.rawValue)
        }
        caps.append(JunoCapability.photos.rawValue)
        caps.append(JunoCapability.contacts.rawValue)
        caps.append(JunoCapability.calendar.rawValue)
        caps.append(JunoCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(JunoCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            JunoScreenCommand.record.rawValue,
            JunoSystemCommand.notify.rawValue,
            JunoChatCommand.push.rawValue,
        ...
        if caps.contains(JunoCapability.camera.rawValue) {
            commands.append(JunoCameraCommand.list.rawValue)
            commands.append(JunoCameraCommand.snap.rawValue)
            commands.append(JunoCameraCommand.clip.rawValue)
        }
        if caps.contains(JunoCapability.location.rawValue) {
            commands.append(JunoLocationCommand.get.rawValue)
        }
        if caps.contains(JunoCapability.device.rawValue) {
            commands.append(JunoDeviceCommand.status.rawValue)
            commands.append(JunoDeviceCommand.info.rawValue)
        }
        if caps.contains(JunoCapability.watch.rawValue) {
            commands.append(JunoWatchCommand.status.rawValue)
            commands.append(JunoWatchCommand.notify.rawValue)
        }
        if caps.contains(JunoCapability.photos.rawValue) {
            commands.append(JunoPhotosCommand.latest.rawValue)
        }
        if caps.contains(JunoCapability.contacts.rawValue) {
            commands.append(JunoContactsCommand.search.rawValue)
            commands.append(JunoContactsCommand.add.rawValue)
        }
        if caps.contains(JunoCapability.calendar.rawValue) {
            commands.append(JunoCalendarCommand.events.rawValue)
            commands.append(JunoCalendarCommand.add.rawValue)
        }
        if caps.contains(JunoCapability.reminders.rawValue) {
            commands.append(JunoRemindersCommand.list.rawValue)
            commands.append(JunoRemindersCommand.add.rawValue)
        }
        if caps.contains(JunoCapability.motion.rawValue) {
            commands.append(JunoMotionCommand.activity.rawValue)
            commands.append(JunoMotionCommand.pedometer.rawValue)
        }
