...
final class RemindersService: RemindersServicing {
    func list(params: JunoRemindersListParams) async throws -> JunoRemindersListPayload {
        ...
        let payload: [JunoReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return JunoRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: JunoRemindersAddParams) async throws -> JunoRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = JunoReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return JunoRemindersAddPayload(reminder: payload)
    }
