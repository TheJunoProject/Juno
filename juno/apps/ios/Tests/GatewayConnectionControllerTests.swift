...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": JunoLocationMode.always.rawValue,
            ...
            #expect(caps.contains(JunoCapability.canvas.rawValue))
            #expect(caps.contains(JunoCapability.screen.rawValue))
            #expect(caps.contains(JunoCapability.camera.rawValue))
            #expect(caps.contains(JunoCapability.location.rawValue))
            #expect(caps.contains(JunoCapability.voiceWake.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": JunoLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(JunoLocationCommand.get.rawValue))
        }
    }
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(JunoSystemCommand.notify.rawValue))
            #expect(!commands.contains(JunoSystemCommand.run.rawValue))
            #expect(!commands.contains(JunoSystemCommand.which.rawValue))
            #expect(!commands.contains(JunoSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(JunoSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.juno.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.juno.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.juno.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.juno.gateway", account: "lastConnection")
