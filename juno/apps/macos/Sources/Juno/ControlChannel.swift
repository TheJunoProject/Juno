...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: JunoProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.juno", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: JunoKit.AnyCodable]()) {
                $0[$1.key] = JunoKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: JunoProtocol.AnyCodable?) -> [String: JunoProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: JunoProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: JunoKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: JunoProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("juno.control.heartbeat")
    static let controlAgentEvent = Notification.Name("juno.control.agent")
}
