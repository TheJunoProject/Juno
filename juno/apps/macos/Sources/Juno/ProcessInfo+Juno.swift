...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.juno.mac`) even if the shipped
    ...
    {
        if environment["JUNO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "juno.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "juno.nixMode") { return true }
