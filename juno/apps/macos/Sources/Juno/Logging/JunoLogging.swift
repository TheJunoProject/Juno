...
enum JunoLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = JunoOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = JunoFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        JunoLogging.bootstrapIfNeeded()
        let label = JunoLogging.makeLabel(subsystem: subsystem, category: category)
...
struct JunoOSLogHandler: AppLogLevelBackedHandler {
...
struct JunoFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = JunoLogging.parseLabel(self.label)
