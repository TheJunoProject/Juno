...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.juno", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> JunoSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: JunoSessionPreviewEntry,
    ...
    private static func cache(payload: JunoSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: JunoChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [JunoKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [JunoKit.AnyCodable]) -> [JunoChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(JunoChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: JunoChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: JunoChatMessage) -> Bool {
    ...
    private static func toolNames(for message: JunoChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: JunoChatMessage) -> String? {
