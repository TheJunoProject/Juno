...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["JUNO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/juno")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("juno-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("juno-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/juno/ (rolling or stdout), or nil if none exist.
