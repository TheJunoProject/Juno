...
enum CommandResolver {
    private static let projectRootDefaultsKey = "juno.gatewayProjectRootPath"
    private static let helperName = "juno"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let junoEntry = root.appendingPathComponent("juno.mjs").path
        if FileManager().isReadableFile(atPath: junoEntry) { return junoEntry }
        let binEntry = root.appendingPathComponent("bin/juno.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/juno")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let junoPaths = self.junoManagedPaths(home: home)
        if !junoPaths.isEmpty {
            extras.insert(contentsOf: junoPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + junoPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func junoManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".juno"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func junoExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectJunoExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("juno.mjs").path,
            root.appendingPathComponent("bin/juno.js").path,
        ]
    ...
    static func hasAnyJunoInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.junoExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let junoPath = self.projectJunoExecutable(projectRoot: root) {
            return [junoPath, subcommand] + extraArgs
        }
        if let junoPath = self.junoExecutable(searchPaths: searchPaths) {
            return [junoPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "juno", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            juno entrypoint missing (looked for dist/index.js or juno.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real juno CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/juno"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/juno.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/juno.mjs"
            node "$PRJ/juno.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/juno.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/juno.js"
            node "$PRJ/bin/juno.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent juno"
          pnpm --silent juno \(quotedArgs);
        else
          echo "juno CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? JunoConfigFile.loadDict()
