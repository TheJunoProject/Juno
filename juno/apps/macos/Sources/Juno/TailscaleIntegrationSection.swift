...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.juno/juno.json. Prefer JUNO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.juno/juno.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.juno/juno.json. Restart the gateway to apply."
        }
