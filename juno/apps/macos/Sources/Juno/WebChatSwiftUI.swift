...
private let webChatSwiftLogger = Logger(subsystem: "ai.juno", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "juno.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: JunoChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> JunoChatSessionsListResponse {
        ...
        let defaults = decoded.defaults.map {
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return JunoChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [JunoChatAttachmentPayload]) async throws -> JunoChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> JunoChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                JunoGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: JunoProtocol.ModelChoice) -> JunoChatModelChoice {
        JunoChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<JunoChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any JunoChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = JunoChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: JunoChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<JunoChatView>) -> NSViewController
    {
