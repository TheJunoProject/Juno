...
struct JunoMacCLI {
    static func main() async {
        ...
        default:
            fputs("juno-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      juno-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      juno-mac discover [--timeout <ms>] [--json] [--include-local]
      juno-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      juno-mac connect
      juno-mac connect --url ws://127.0.0.1:18789 --json
      juno-mac discover --timeout 3000 --json
      juno-mac wizard --mode local
    """)
}
