...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: JunoProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: JunoProtocol.AnyCodable] = [
            "presence": .init([JunoProtocol.AnyCodable(entry)]),
        ]
        let payload = JunoProtocol.AnyCodable(payloadMap)
