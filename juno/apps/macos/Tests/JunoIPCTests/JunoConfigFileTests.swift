...
@Suite(.serialized)
struct JunoConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("juno-config-\(UUID().uuidString)")
            .appendingPathComponent("juno.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["JUNO_CONFIG_PATH": override]) {
            #expect(JunoConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["JUNO_CONFIG_PATH": override]) {
            ...
            #expect(JunoConfigFile.remoteGatewayPort() == 19999)
            #expect(JunoConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(JunoConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(JunoConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["JUNO_CONFIG_PATH": override]) {
            ...
            JunoConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = JunoConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["JUNO_CONFIG_PATH": override]) {
            ...
            JunoConfigFile.clearRemoteGatewayUrl()
            let root = JunoConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("juno-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "JUNO_CONFIG_PATH": nil,
            "JUNO_STATE_DIR": dir,
        ]) {
            #expect(JunoConfigFile.stateDirURL().path == dir)
            #expect(JunoConfigFile.url().path == "\(dir)/juno.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("juno-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("juno.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "JUNO_STATE_DIR": stateDir.path,
            "JUNO_CONFIG_PATH": configPath.path,
        ]) {
            JunoConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-juno-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = JunoConfigFile.loadDict()
