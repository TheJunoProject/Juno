...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("JUNO_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("JUNO_WIDE_AREA_DOMAIN", "juno.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("JUNO_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("JUNO_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._juno-gw._tcp.juno.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.juno.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Juno)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/juno/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Juno)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/juno/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._juno-gw._tcp.juno.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
