...
let junohubSearchTimer: ReturnType<typeof setTimeout> | null = null;
function lazyRender<M>(getter: () => M | null, render: (mod: M) => unknown) {
  const mod = getter();
  return mod ? render(mod) : nothing;
}

const UPDATE_BANNER_DISMISS_KEY = "juno:control-ui:update-banner-dismissed:v1";
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">Juno</span>
                      </span>
                    `}
              </div>
              <button
        ...
        ${state.tab === "skills"
          ? lazyRender(lazySkills, (m) =>
              m.renderSkills({
                ...
                junohubQuery: state.junohubSearchQuery,
                junohubResults: state.junohubSearchResults,
                junohubSearchLoading: state.junohubSearchLoading,
                junohubSearchError: state.junohubSearchError,
                junohubDetail: state.junohubDetail,
                junohubDetailSlug: state.junohubDetailSlug,
                junohubDetailLoading: state.junohubDetailLoading,
                junohubDetailError: state.junohubDetailError,
                junohubInstallSlug: state.junohubInstallSlug,
                junohubInstallMessage: state.junohubInstallMessage,
                ...
                onJunoHubQueryChange: (query) => {
                  setJunoHubSearchQuery(state, query);
                  if (junohubSearchTimer) {
                    clearTimeout(junohubSearchTimer);
                  }
                  junohubSearchTimer = setTimeout(() => searchJunoHub(state, query), 300);
                },
                onJunoHubDetailOpen: (slug) => loadJunoHubDetail(state, slug),
                onJunoHubDetailClose: () => closeJunoHubDetail(state),
                onJunoHubInstall: (slug) => installFromJunoHub(state, slug),
              }),
            )
          : nothing}
        ${state.tab === "nodes"
