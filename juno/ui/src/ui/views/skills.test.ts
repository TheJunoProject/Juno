...
function createProps(overrides: Partial<SkillsProps> = {}): SkillsProps {
  ...
  return {
    ...
    onJunoHubQueryChange: () => undefined,
    onJunoHubDetailOpen: () => undefined,
    onJunoHubDetailClose: () => undefined,
    onJunoHubInstall: () => undefined,
    ...overrides,
  };
}
...
describe("renderSkills", () => {
  ...
  it("opens detail dialogs and routes JunoHub actions", async () => {
    const container = document.createElement("div");
    const onDetailClose = vi.fn();
    const showModal = vi.fn(function (this: HTMLDialogElement) {
      this.setAttribute("open", "");
    });
    const onJunoHubDetailOpen = vi.fn();
    const onJunoHubInstall = vi.fn();
          ...
          junohubResults: [
            {
              ...
              slug: "github",
              displayName: "GitHub",
              summary: "GitHub integration for Juno",
              version: "1.2.3",
            },
          ],
          onJunoHubDetailOpen,
    ...
    expect(text).toContain("GitHub integration for Juno");
    ...
    expect(onJunoHubDetailOpen).toHaveBeenCalledTimes(1);
    expect(onJunoHubDetailOpen).toHaveBeenCalledWith("github");
    expect(onJunoHubInstall).toHaveBeenCalledTimes(1);
    expect(onJunoHubInstall).toHaveBeenCalledWith("github");

    onJunoHubInstall.mockClear();
    showModal.mockClear();

    render(
      renderSkills(
        createProps({
          junohubSearchError: "rate limited",
          junohubInstallMessage: { kind: "success", text: "Installed github" },
          junohubDetailSlug: "github",
          junohubDetail: {
            skill: {
              slug: "github",
              displayName: "GitHub",
    ...
    expect(text).toContain("By Juno (@juno)");
