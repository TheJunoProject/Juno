...
function clearDeleteConfirmSkip() {
  try {
    getSafeLocalStorage()?.removeItem("juno:skipDeleteConfirm");
  } catch {
    /* noop */
  }
}
...
describe("chat view", () => {
  it("uses the assistant avatar URL or bundled logo fallbacks", () => {
    ...
    expect(groupedLogo?.getAttribute("src")).toBe("/juno/favicon.svg");
  });
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
            ...
            toolMessages: [
              {
                ...
                content: JSON.stringify({
                  ...
                  view: {
                    ...
                    url: `/__juno__/canvas/documents/cv_inline_${params.suffix}/index.html`,
                    title: "Inline demo",
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__juno__/canvas/documents/cv_inline_default/index.html",
    );
          ...
          toolMessages: [
            {
              ...
              content: JSON.stringify({
                ...
                view: {
                  ...
                  url: "/__juno__/canvas/documents/cv_inline_visible/index.html",
                  title: "Inline demo",
                  preferred_height: 360,
                },
                presentation: {
          ...
          toolMessages: [
            {
              ...
              content: JSON.stringify({
                ...
                view: {
                  backend: "canvas",
                  id: "cv_nearest_turn",
                  url: "/__juno__/canvas/documents/cv_nearest_turn/index.html",
              ...
              content: JSON.stringify({
                selected_item: {
                  ...
                  view: {
                    ...
                    url: "/__juno__/canvas/documents/cv_generic_inline/index.html",
  ...
  it("keeps user transcript images visible after history reload", () => {
    ...
    renderGroupedMessage(
      container,
      {
        ...
        role: "user",
        content: "",
        MediaPath: "/tmp/juno/user-upload.png",
        timestamp: Date.now(),
      },
      "user",
      {
        ...
        localMediaPreviewRoots: ["/tmp/juno"],
      },
    );

    const image = container.querySelector<HTMLImageElement>(".chat-message-image");
    expect(image?.getAttribute("src")).toBe(
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Fuser-upload.png&token=session-token",
    );
  });
  ...
  it("keeps plural user transcript images visible after history reload", () => {
    ...
    renderGroupedMessage(
      container,
      {
        ...
        role: "user",
        content: "",
        MediaPaths: ["/tmp/juno/first.png", "/tmp/juno/second.jpg"],
    ...
    expect(imageSources).toEqual([
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ffirst.png&token=session-token",
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Fsecond.jpg&token=session-token",
    ]);
  });
  ...
  it("skips non-image transcript media paths after history reload", () => {
    ...
    renderGroupedMessage(
      container,
      {
        ...
        role: "user",
        content: "",
        MediaPath: "/tmp/juno/user-upload.pdf",
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/juno/test image.png\nMEDIA:/tmp/juno/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ftest+image.png&token=session-token&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ftest+image.png&token=session-token",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ftest-doc.pdf&token=session-token",
    );
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ftest+image.png&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/juno/__juno__/assistant-media?source=%2Ftmp%2Fjuno%2Ftest+image.png&token=fresh-token&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(container.querySelector(".chat-message-image")).not.toBeNull();
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    expect(docLink?.getAttribute("href")).toBe("/__juno__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/juno/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/juno/__juno__/assistant-media?source=%2FC%3A%2Ftmp%2Fjuno%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/juno/__juno__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/juno/__juno__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__juno__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__juno__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__juno__/cap/cap_123/__juno__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
              ...
              content: [
                {
                  ...
                  text: JSON.stringify({
                    ...
                    view: {
                      ...
                      url: "/__juno__/canvas/documents/cv_canvas_live_history/index.html",
                ...
                },
                {
                  ...
                  text: JSON.stringify({
                    ...
                    view: {
                      ...
                      url: "/__juno__/canvas/documents/cv_streamed_artifact/index.html",
                      title: "Streamed demo",
                      preferred_height: 320,
                    },
                    presentation: {
