...
export type SkillsProps = {
  ...
  junohubQuery: string;
  junohubResults: JunoHubSearchResult[] | null;
  ...
  onDetailClose: () => void;
  onJunoHubQueryChange: (query: string) => void;
  onJunoHubDetailOpen: (slug: string) => void;
  onJunoHubDetailClose: () => void;
  onJunoHubInstall: (slug: string) => void;
};
...
export function renderSkills(props: SkillsProps) {
  ...
  return html`
    <section class="card">
      ...
      <div style="margin-top: 16px; border-top: 1px solid var(--border); padding-top: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
          <div style="font-weight: 600;">JunoHub</div>
        ...
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          <label class="field" style="flex: 1; min-width: 180px;">
            <input
              .value=${props.junohubQuery}
              @input=${(e: Event) =>
                props.onJunoHubQueryChange((e.target as HTMLInputElement).value)}
              placeholder="Search JunoHub skills…"
              autocomplete="off"
              name="junohub-search"
            />
          </label>
          ${props.junohubSearchLoading ? html`<span class="muted">Searching…</span>` : nothing}
        </div>
        ${props.junohubSearchError
          ? html`<div class="callout danger" style="margin-top: 8px;">
              ${props.junohubSearchError}
            </div>`
          : nothing}
        ${props.junohubInstallMessage
          ? html`<div
              class="callout ${props.junohubInstallMessage.kind === "error" ? "danger" : "success"}"
              style="margin-top: 8px;"
            >
              ${props.junohubInstallMessage.text}
            </div>`
          : nothing}
        ${renderJunoHubResults(props)}
      </div>
    ...
    ${props.junohubDetailSlug ? renderJunoHubDetailDialog(props) : nothing}
  `;
}

function renderJunoHubResults(props: SkillsProps) {
  const results = props.junohubResults;
  if (!results) {
    return nothing;
  }
  if (results.length === 0) {
    return html`<div class="muted" style="margin-top: 8px;">No skills found on JunoHub.</div>`;
  }
  return html`
      ...
      ${results.map(
        (r) => html`
          <div
            ...
            @click=${() => props.onJunoHubDetailOpen(r.slug)}
          >
            <div class="list-main">
            ...
            <div class="list-meta" style="display: flex; align-items: center; gap: 8px;">
              ...
              <button
                class="btn btn--sm"
                ?disabled=${props.junohubInstallSlug !== null}
                @click=${(e: Event) => {
                  e.stopPropagation();
                  props.onJunoHubInstall(r.slug);
                }}
              >
                ${props.junohubInstallSlug === r.slug ? "Installing\u2026" : "Install"}
              </button>
            </div>
          </div>
        `,
...
function renderJunoHubDetailDialog(props: SkillsProps) {
  const detail = props.junohubDetail;
  ...
  return html`
    <dialog
      ...
      @close=${props.onJunoHubDetailClose}
    >
      <div class="md-preview-dialog__panel">
        <div class="md-preview-dialog__header">
          <div class="md-preview-dialog__title">
            ${detail?.skill?.displayName ?? props.junohubDetailSlug}
          </div>
          <button
            class="btn btn--sm"
            @click=${(e: Event) => {
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ${props.junohubDetailLoading
            ? html`<div class="muted">${t("common.loading")}</div>`
            : props.junohubDetailError
              ? html`<div class="callout danger">${props.junohubDetailError}</div>`
              : detail?.skill
                ? html`
                    ...
                    <button
                      ...
                      ?disabled=${props.junohubInstallSlug !== null}
                      @click=${() => {
                        if (props.junohubDetailSlug) {
                          props.onJunoHubInstall(props.junohubDetailSlug);
                        }
                      }}
                    >
                      ${props.junohubInstallSlug === props.junohubDetailSlug
                        ? "Installing\u2026"
...
function renderSkillDetail(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const showBundledBadge = Boolean(skill.bundled && skill.source !== "juno-bundled");
