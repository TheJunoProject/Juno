...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://juno.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://juno.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://juno.ai/abc-123", baseHref)).toBe(
      "blob:https://juno.ai/abc-123",
    );
  });
