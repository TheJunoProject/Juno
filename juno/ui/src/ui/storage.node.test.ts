  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __JUNO_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__JUNO_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__JUNO_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /juno/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/juno"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/juno/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/juno"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    sessionStorage.setItem("juno.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "juno.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/juno",
    ...
    const scopedKey = "juno.control.settings.v1:wss://gateway.example:8443/juno";
  ...
  it("does not persist gateway tokens when saving settings", async () => {
    ...
    const scopedKey = `juno.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "juno",
      themeMode: "system",
  ...
  it("caps persisted session scopes to the most recent gateways", async () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `juno.control.settings.v1:wss://gateway.example:8443`;
